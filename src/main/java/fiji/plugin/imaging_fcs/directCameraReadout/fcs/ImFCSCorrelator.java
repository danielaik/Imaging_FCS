 ...
 */
package fiji.plugin.imaging_fcs.directCameraReadout.fcs;
...
import fiji.plugin.imaging_fcs.imfcs.gpu.GpuParameters;
import ij.IJ;
import ij.ImagePlus;
...
public class ImFCSCorrelator {
    ...
    private final int fitMaxIterations = 2000;
    // maximum number of iterations allowed in the fits; the maximum number is Integer.MAX_VALUE but that can
    // lead to very long evaluations with typically little improvement on the results
    private final int fitMaxEvaluations = 2000;
    ...
    private final int panelPosX = 10;
    ...
    private final int sdWindowPosX = acfWindowPosX + acfWindowDimX + 115;
    // fit standard deviation window position and size
    private final int sdWindowPosY = acfWindowPosY;
    // Parameter Blocking Plot Window position and size
    private final int blockingWindowPosY = sdWindowPosY;
    private final int sdWindowDimX = acfWindowDimX;
    private final int blockingWindowPosX = sdWindowPosX + sdWindowDimX + 110;
    ...
    // remember the current model that is used for fitting; this is not necessarily the cbFitModel setting as
    // in some cases multiple
    PlotWindow blockingWindow;
    ...
    private double correlatorp;
    // parameters for the correlator structure; correlatorp refers to the number of bins in the first channel
    // group.
    private double correlatorq;
    // all higher groups have correlatorp/2 channels; correaltorq refers to the number of higher groups
    ...
    private double[] paraminitval;
    // intitial values for the parameters used in the fit (can be the same as initparam or can be continously
    // set; both options are given but only the first is used momentarily)
    private double q2;
    // fit parameter, ratio of brightness of second component to first component (if it exists)
    private double q3;
    // fit parameter, ratio of brightness of third component to first component (if it exists)
    ...
    private double fitobsvol;
    // normalization factor including the observation volume; for ACFs this is correct; note that N for
    // spatial cross-correaltions has no physical meaning
    ...
    private boolean[][][] pixfitted;
    // whether pixel has been successfully fitted or not; in the absence of user-defined thresholds, this
    // array determines pixvalid[][][]
    private int isdlawcalculatedingpu = 0;
    // maximum number of evaluations allowed in the fits; the maximum number is Integer.MAX_VALUE but that
    // can lead to very long evaluations with typically little improvement on the result
    ...
    private String[] panelSettings = new String[noSettings];
    // array to store the individual settings, the settings are stored in the same order as used to create
    // the results table
    ...
    // microscope objective magnification; note the variable 'magnification' is used for the magnification of
    // the stack image in ImageJ
    ...
    // temporary array which stores the values along two pixel columns to be correlated through the stack
    private int background;
    // background is determined from the smallest count value from the stack; this can be maually changed in
    // the control panel
    private int background2;
    // dummy array used for initilizing plot (should be removed later on)
    // for FCCS background for the second region is determined as the minimum value for that region
    private int cfXshift;
    // x,y distance used in fitting of FCS correlations between two pixels; (0, 0) is for ACFs
    ...
    private int pixbinX;
    // this is a multiplication factor to determine positions in the image window; it is 1 for overlap and
    // equal to binning for non-overlap
    private int pixbinY;
    private int parcormode;
    private int cfXDistance;
    // x,y distance between pixels to be correlated; in DC-FCCS this is the distance between corresponding
    // pixels in red and green channels
    private int cfYDistance;
    private int pixelWidthX;
    // determines the number of pixels that can be correlated, depending on whether overlap is allowed for
    ...
    private int maxcposx;
    // max positions allowed in image depending on actual image width/height, binning, and distance between
    // pixels;
    private int maxcposy;
    private int mincposx;
    // min positions allowed in image depending on actual image width/height, binning, and distance between
    // pixels;
    private int mincposy;
    // arrays and parameters used for computation of correlations and storage of results
    private double[][] currentCovmats;
    // the regularized covariance matrix is at the moment not stored but only updated for the current pixel
    // if "GLS Fit" is selected
    private double[][][][] sdacf;
    // standerd deviation of the ACF; dimensions [ccf: ACF1, ACF2, CCF][width][height][chanum]
    private double[][][][] acf = null;
    // ACF array to store all FCS functions for the pixels in the image; dimensions [ccf: ACF1, ACF2,
    // CCF][width][height][chanum]
    private double[][][][] varacf;
    // Index whether block Transformation is succesful (1) or maximum blocking is used (0)
    // standerd deviation of the ACF; dimensions [ccf: ACF1, ACF2, CCF][width][height][chanum]
    ...
    private double[][][][] msd;
    // MSD array to store all MSD plots for the pixels in the image; dimensions [ccf: MSD1, MSD2, MSD of
    // CCF][width][height][chanum]
    ...
    private int[] lag;
    // lag for each correlation channel counted as multiples of the smallest basic time; independent of time;
    // lagtime = lag * frametime
    private int base;                    // base = number of channels in first group
    private int hbase;                // hbase = number of channels in all higher groups
    // read whether to calculate cumulative or non-cumulative CF; update parameter from GUI
    private double[] mtab;
    // number of samples for each correlation channel; dimensions [chanum]; defined in setParameters()
    private double[] samp;
    // sampletime (or bin width) of each channel; dimensions [chanum]; defined in setParameters()
    ...
    // lagtime array which stores the correlation times; dimensions [chanum]; defined in setParameters()
    ...
    private boolean isCalibFixScale;
    // setting true will remove autoscaling for 3 calibration plot: diffusion, amplitude, and intensity
    private String tempAnalysisMode;
    // Window for blocking analysis; not shown by default; to show, set plotBlockingCurve = true
    ...
    public boolean settingPlotOption(boolean plotACF, boolean plotTrace, boolean plotAverage, boolean plotJustCCF,
                                     int CCFdistX, int CCFdistY, boolean plotCalibrationAmplitude,
                                     boolean plotCalibrationDiffusion, boolean isResetCalibPlot, int noaver,
                                     int InputBackground, boolean plotCalibrationIntensity, boolean isCalibFixScale,
                                     String tempAnalyMode, int fitstart) {
        ...
        if (tempAnalysisMode.equals($amode[2])) { //cumulative //improve Q with more frame
            correlatorq = DirectCapturePanel.multiTauCorrelatorCalculator.getQgivenFrame(
                    DirectCapturePanel.Common.correlator_p, DirectCapturePanel.Common.correlator_q, frames,
                    DirectCapturePanel.Common.dataPtsLastChannel); //adjust Q according to avaialble frame (
            // recommended Q <= specified Q
        } else { // Q indepenent of no frame
            correlatorq = DirectCapturePanel.multiTauCorrelatorCalculator.getQgivenFrame(
                    DirectCapturePanel.Common.correlator_p, DirectCapturePanel.Common.correlator_q, frames,
                    DirectCapturePanel.Common.dataPtsLastChannel_calibration);
            //                correlatorq = EMCCD.correlator_q; //correlate constant q regardless of currently
            //                available frame
        }
        ...
        if (InputBackground >= 1000000) {
            impmin = minDetermination(
                    imp);//calculate the minimum of the image stack; this will be used as the default background value
        } else {
    ...
    public void runPlotACF() {
        ...
        if (imp != null) {
            ...
            obtainImage();
            //            IJ.log("ObtainImage called, lastframe: " + lastframe + ", time elapse: " + (System.nanoTime
            //            () - time)/1000000 + " ms");
        }
        printParam();
        if (setParameters()) {
            ...
            if (tempAnalysisMode.equals($amode[1])) { //non-cumulative
                ...
                impRoi1 = new Roi(DirectCapturePanel.Common.lLeft - 1, DirectCapturePanel.Common.lTop - 1,
                        DirectCapturePanel.Common.lWidth, DirectCapturePanel.Common.lHeight);
            }
            ...
            //            }
            checkroi = true;
            // a Swing Worker class is used to call the parent method runPlotACF instead. This will let us know when
            // the ACF analysis has been compelted.
    ...
    public void correlateROI(Roi improi) {
        ...
        if (!IsGPUCalculationOK) {
            System.out.println(
                    "An error was encountered while performing calculations on GPU. Calculations will be done on CPU " +
                            "instead.");
        }

        // CPU calculations
        if (!DirectCapturePanel.Common.useGpu || !IsGPUCalculationOK ||
                !DirectCapturePanel.Common.RunLiveReadOutOnGPU) {
            ...
            //            IJ.log("startXmap: " + startXmap + ", endXmap: " + endXmap + ", startYmap: " + startYmap +
            //            ", endymap: " + endYmap);
            ...
            if ("FCS".equals(fitModel)) {
                if (doFit) {
                    ...
                    //                            if (filterArray[x * pixbinX][y * pixbinY] >= filterLL * binningX *
                    //                            binningY && filterArray[x * pixbinX][y * pixbinY] <= filterUL *
                    //                            binningX * binningY) {
                    //                                calcIntensityTrace(imp, x * pixbinX, y * pixbinY, x * pixbinX +
                    //                                cfXDistance, y * pixbinY + cfYDistance, firstframe, lastframe);
                    //                                correlate(imp, x * pixbinX, y * pixbinY, x * pixbinX +
                    //                                cfXDistance, y * pixbinY + cfYDistance, 0, firstframe, lastframe);
                    //                                if (!fit(x, y, 0, "FCS")) {
                    //                                    return;
                    //                                }
                    //                            }
            ...
            if ("DC-FCCS".equals(fitModel)) { //DAN only 2D DC-FCCS implemented
                if (doFit) {
                    ...
                    //                            binningX * binningY) {
                    //                                calcIntensityTrace(imp, x * pixbinX, y * pixbinY, x * pixbinX,
                    //                                y * pixbinY, firstframe, lastframe);
                    //                                correlate(imp, x * pixbinX, y * pixbinY, x * pixbinX, y *
                    //                                pixbinY, 0, firstframe, lastframe);
                    //                                if (!fit(x, y, 0, "ITIR-FCS (2D)")) {
                    //                                    return;
                    //                                }
                    //                                calcIntensityTrace(imp, x * pixbinX + cfXDistance, y * pixbinY
                    //                                + cfYDistance, x * pixbinX + cfXDistance, y * pixbinY +
                    //                                cfYDistance, firstframe, lastframe);
                    //                                correlate(imp, x * pixbinX + cfXDistance, y * pixbinY +
                    //                                cfYDistance, x * pixbinX + cfXDistance, y * pixbinY +
                    //                                cfYDistance, 1, firstframe, lastframe);
                    ...
                    //                                cfXDistance, y * pixbinY + cfYDistance, firstframe, lastframe);
                    //                                correlate(imp, x * pixbinX, y * pixbinY, x * pixbinX +
                    //                                cfXDistance, y * pixbinY + cfYDistance, 2, firstframe, lastframe);
                    ...
                    //                                }
                    //                                if (CCFq[x][y] > 1) {
                    //                                    CCFq[x][y] = Double.NaN;	//cross-correlation > 1 is
                    //                                    non-physical, most likely due to very noisy CFs, better
                    //                                    discarded
                    //                                }
                    //                            }
                    //                        }
                        ...
                        for (int y = startYmap; y <= endYmap; y++) {
                            if (filterArray[x * pixbinX][y * pixbinY] >= 0) {
                                if (use2imp) {
                                    ...
                                    calcIntensityTrace(imp, x * pixbinX, y * pixbinY, x * pixbinX, y * pixbinY,
                                    ...
                                    calcIntensityTrace(imp, x * pixbinX + cfXDistance, y * pixbinY + cfYDistance,
                                            x * pixbinX + cfXDistance, y * pixbinY + cfYDistance, firstframe,
                                            lastframe);
                                    correlate(imp, x * pixbinX + cfXDistance, y * pixbinY + cfYDistance,
                                            x * pixbinX + cfXDistance, y * pixbinY + cfYDistance, 1, firstframe,
                                            lastframe);
                                    calcIntensityTrace(imp, x * pixbinX, y * pixbinY, x * pixbinX + cfXDistance,
                                            y * pixbinY + cfYDistance, firstframe, lastframe);
                                    correlate(imp, x * pixbinX, y * pixbinY, x * pixbinX + cfXDistance,
            ...
            if (pixcount > 0) {
                ...
                if (plotAverage) {
                    ...
                    if (cfXDistance != 0 || cfYDistance != 0) {
                        if (plotJustCCF) {
                            calcAveCF(0);//acquisition mode CCF just CCF plot; calibration mode CCF (single function)
                            // just CCF
                            plotCF(tmproi, 4, false);//plot average CCF, or ACF
                        } else {
                            calcAveCF(2);//acquisition CCF multiple functions averaged; calibration mode CCF multiple
                            // functions
                    ...
                    if ((cfXDistance != 0 || cfYDistance != 0) && !plotJustCCF) {
                        plotCF(improi, 3,
                                false); //acquisition CCF multiple functions; calibration CCF multiple functions
                    } else {
                        if (use2imp) {
                            plotCF(improi, 6, false); //calibration CCF single function
                        } else {
                            plotCF(improi, 2,
                                    false); //calibration and acquisition mode ACF, acquisition mode CCF justCCF,
                        }
                    }
                }
                ...
                if (plotIntensityCurves) {
                    if (cfXDistance != 0 || cfYDistance != 0) {
                        ...
                        calcAverageIntensityTrace(filterArray, startX, startY, endX, endY, firstframe,
                                lastframe);//calibration and acquisition mode ACF
                        plotIntensityTrace(improi, 2);//calibration and acquisition mode ACF
                    }
                }

                if ((plotCalibrationAmplitude || plotCalibrationDiffusion || plotCalibrationIntensity) &&
                        (tempAnalysisMode.equals($amode[1]))) {//non-cumulative
                ...
                //if calibration mode is true, check if either plot amplitude and plot diffusion is true. if so
                // calculate averageacf array and check flag restart plot is false, then do plotting
                ...
                IJ.log("ROI does not cover a whole single pixel in the binned image. Select larger area or reduce " +
                        "pixel binning");
            }
    ...
    public void plotCF(Roi plotroi, int cormode, boolean map) {
        ...
        //            if (fitModel == "DC-FCCS" && cormode == 1) { 	// index ct ensures that in cormode 1 when DC-FCCS
        //            is selected the CCF is plotted
        //                ct = 2;
        //            }
        ...
        if (cormode == 1) {                    // if single ACF or CCF
            for (int z = 1; z <= (chanum -
                    1); z++) {    // find minimum and maximum values in correlation functions that will be plotted
            ...
            if (cfXDistance + cfYDistance != 0) {
                acfPlot.addLabel(0.5, 0,
                        " CCF of (" + cpx1 * pixbinX + ", " + cpy1 * pixbinY + ")" + " and (" + cpx2 + ", " + cpy2 +
                                ")" + " at (" + binningX + "x" + binningY + "binning");
                ...
                acfPlot.addLabel(0.5, 0,
                        " ACFss of (" + cpx1 * pixbinX + ", " + cpy1 * pixbinY + ") at " + binningX + "x" + binningY +
                                "binning");
            }
            acfPlot.draw();
            int num1 = 0;
            ...
            //                plot.addPoints(Arrays.copyOfRange(lagtime, fitstart, fitend + 1), Arrays.copyOfRange
            //                (fitacf[ct][cpx1][cpy1], fitstart, fitend + 1), Plot.LINE);
        ...
        if (cormode == 2) {                    // if multiple ACF or CCF
            ...
            for (int x = cpx1; x <=
                    cpxf; x++) {        // find minimum and maximum values in correlation functions that will be plotted
                for (int y = cpy1; y <= cpyf; y++) {
                    for (int z = 1; z <= (chanum - 1); z++) {
                        if ((!map && plotroi.contains(x * pixbinX, y * pixbinY) &&
                            ...
                            if (acf[0][x][y][z] != 0 && minsc >
                                    acf[0][x][y][z]) { // make sure minsc is not set to 0 because of a missing CF
                                minsc = acf[0][x][y][z];
                            }
                        }
                    }
            ...
            // If this is a DC-FCCS plot where all functions are supposed to be plotted then plot the
            ...
            if (cfXDistance + cfYDistance != 0 && cftype != 2) {
                acfPlot.addLabel(0.5, 0,
                        $titlePrefix + " - CCFs of pixels in the ROIs at " + binningX + "x" + binningY + "binning");
            } else if (cfXDistance + cfYDistance != 0 && cftype == 2) {
                acfPlot.addLabel(0.5, 0,
                        $titlePrefix + " - CCFs of pixels in the ROIs at " + binningX + "x" + binningY + "binning");
            } else {
                acfPlot.addLabel(0.5, 0,
                        $titlePrefix + " - ACFs from the ROI at " + binningX + "x" + binningY + "binning");
            }
            ...
            if (doFit) {
                ...
                //                        if ((!map && plotroi.contains(x * pixbinX, y * pixbinY) && plotroi.contains
                //                        (x * pixbinX, y * pixbinY + binningY - 1) && plotroi.contains(x * pixbinX +
                //                        binningX - 1, y * pixbinY) && plotroi.contains(x * pixbinX + binningX - 1,
                //                        y * pixbinY + binningY - 1)) || (map == true && plotroi.contains(x, y))) {
                //                            plot.setColor(java.awt.Color.BLUE);
                ...
                //                            Arrays.copyOfRange(fitacf[cftype][x][y], fitstart, fitend + 1), Plot
                //                            .LINE);
                //                        }
                //                    }
                //                }
        ...
        if (cormode == 3) {
            ...
            for (int x = cpx1; x <= cpxf; x++) {
                for (int y = cpy1; y <= cpyf; y++) {
                    if ((!map && plotroi.contains(x * pixbinX, y * pixbinY) &&
                            ...
                            (map == true && plotroi.contains(x, y))) {
                        for (int z = 1; z <= (chanum -
                                1); z++) {    // find minimum and maximum values in correlation functions that will
                            // be plotted
                            if (maxsc < acf[0][x][y][z]) {
                                maxsc = acf[0][x][y][z];
                            }
            ...
            acfPlot.addLabel(0.5, 0,
                    $titlePrefix + " - auto and cross CFs" + " at (" + binningX + "x" + binningY + "binning");
            //            acfPlot.addLabel(0.5, 0, " ACF1, ACF2, CCF of (" + cpx1 * pixbinX + ", " + cpy1 * pixbinY +
            //            ")" + " and (" + cpx2 * pixbinX + ", " + cpy2 * pixbinY + ")" + " at (" + binningX + "x" +
            //            binningY + "binning");
            acfPlot.draw();
            ...
            if (doFit) {
                ...
                //                plot.addPoints(Arrays.copyOfRange(lagtime, fitstart, fitend + 1), Arrays
                //                .copyOfRange(fitacf[0][cpx1][cpy1], fitstart, fitend + 1), Plot.LINE);
                //                plot.addPoints(Arrays.copyOfRange(lagtime, fitstart, fitend + 1), Arrays
                //                .copyOfRange(fitacf[1][cpx1][cpy1], fitstart, fitend + 1), Plot.LINE);
                //                plot.addPoints(Arrays.copyOfRange(lagtime, fitstart, fitend + 1), Arrays
                //                .copyOfRange(fitacf[2][cpx1][cpy1], fitstart, fitend + 1), Plot.LINE);
                //                plot.draw();
        ...
        if (cormode == 4) {                    // if average ACF
            ...
            if (doFit) {
                //                plot.setColor(java.awt.Color.RED);
                //                plot.addPoints(Arrays.copyOfRange(lagtime, fitstart, fitend + 1), Arrays
                //                .copyOfRange(fitaveacf, fitstart, fitend + 1), Plot.LINE);
                //                plot.setColor(java.awt.Color.BLUE);
                //                plot.draw();
                //                /*if (plotResCurves) {
                //					plotResiduals(cpx1, cpy1); // and add a residual window
        ...
        if (cormode == 5) {
            ...
            // create plot label for ACF of CCF
            acfPlot.addLabel(0.5, 0,
                    $titlePrefix + " - Average auto and cross CF" + " at (" + binningX + "x" + binningY + "binning");
            //            acfPlot.addLabel(0.5, 0, " Average ACF1, ACF2, CCF of (" + cpx1 * pixbinX + ", " + cpy1 *
            //            pixbinY + ")" + " and (" + cpx2 * pixbinX + ", " + cpy2 * pixbinY + ")" + " at (" +
            //            binningX + "x" + binningY + "binning");
            acfPlot.draw();
                ...
                for (int y = cpy1; y <= cpyf; y++) {
                    for (int z = 1; z <= (chanum - 1); z++) {
                        if ((!map && plotroi.contains(x * pixbinX, y * pixbinY) &&
                            ...
                            if (acf[2][x][y][z] != 0 && minsc >
                                    acf[2][x][y][z]) { // make sure minsc is not set to 0 because of a missing CF
                                minsc = acf[2][x][y][z];
                            }
                        }
                    }
    ...
    public void plotSD(int sdpx1, int sdpy1, int cormode) {
        ...
        sdminsc -= sdminsc *
                0.1;        // maximum scales are to be 10% larger than maximum value and 10% smaller than minimum value
        sdmaxsc += sdmaxsc * 0.1;
        ...
        if (sdWindow == null ||
                sdWindow.isClosed() == true) {    // create new plot if window doesn't exist, or reuse existing window
    ...
    public void plotIntensityTrace(Roi plotroi, int cormode) {
        ...
        // cormode = 2: plot all intensity traces in a ROI; ACFs or CCFs depending on the setting of cfXDistance and
        // cfYDistance
        ...
        if (cormode == 1) {
            ...
            for (int x = 0;
                 x < nopit; x++) {    // find minimum and maximum values in intensity trace 1 that will be plotted
            ...
            if ((ipx1 - ipx2) != 0 || (ipy1 - ipy2) !=
                    0) { // in the case of a CCF find also the max and min value in the intensity trace 2
                for (int x = 0;
                     x < nopit; x++) {    // find minimum and maximum values in intensity trace 1 that will be plotted
            ...
            iminsc -=
                    iminsc * 0.1;        // maximum scales are to be 10% larger than maximum value and 10% smaller than
            // minimum value
            imaxsc += imaxsc * 0.1;
            ...
            if (intWindow == null || intWindow.isClosed() ==
    ...
    //            // map: is roi selected in parameter window (reproduction of values only) or is it from a new
    //            calculation?
    //
    //            int cutoff = chanum - 1;
    //            int ct = 0;
    ...
    //            if (map == true) {										// if the ROI is selected in the parameter
    //            map
    ...
    //                }
    //
    //                msdminsc -= msdminsc * 0.1;		// maximum scales are to be 10% larger than maximum value and
    //                10% smaller than minimum value
    ...
    //                msdplot.draw();
    //
    //                if (msdWindow == null || msdWindow.isClosed() == true) {	// create new plot if window doesn't
    //                exist, or reuse existing window
    ...
    //                            if (((!map && MSDroi.contains(x * pixbinX, y * pixbinY) && MSDroi.contains(x *
    //                            pixbinX, y * pixbinY + binningX - 1) && MSDroi.contains(x * pixbinX + binningX - 1,
    //                            y * pixbinY) && MSDroi.contains(x * pixbinX + binningX - 1, y * pixbinY + binningY
    //                            - 1)) || (map && MSDroi.contains(x, y))) && (msd[ct][x][y][i] == 0.0)) {
    //                                zerofound = true;
    //                            }
    //                        }
    //                    }
    ...
    //                double[] msdvalue = new double[cutoff];
    //
    //                for (int x = msdpx1; x <= msdpxf; x++) {		// find minimum and maximum values in MSD plots
    //                that will be plotted
    //                    for (int y = msdpy1; y <= msdpyf; y++) {
    //                        for (int z = 1; z < cutoff; z++) {
    //                            if ((!map && MSDroi.contains(x * pixbinX, y * pixbinY) && MSDroi.contains(x *
    //                            pixbinX, y * pixbinY + binningY - 1) && MSDroi.contains(x * pixbinX + binningX - 1,
    //                            y * pixbinY) && MSDroi.contains(x * pixbinX + binningX - 1, y * pixbinY + binningY
    //                            - 1)) || (map == true && MSDroi.contains(x, y))) {
    ...
    //                msdplot.setColor(java.awt.Color.BLUE);
    //                msdplot.setJustification(Plot.CENTER);
    //                msdplot.addLabel(0.5, 0, "MSDs of pixels in the ROIs at " + binningX + "x" + binningY +
    //                "binning");
    //                msdplot.draw();
    //
    //                if (msdWindow == null || msdWindow.isClosed() == true) {	// create new plot if window doesn't
    ...
    //                for (int y = msdpy1; y <= msdpyf; y++) {
    //                    for (int x = msdpx1; x <= msdpxf; x++) {
    //                        if ((!map && MSDroi.contains(x * pixbinX, y * pixbinY) && MSDroi.contains(x * pixbinX,
    //                        y * pixbinY + binningY - 1) && MSDroi.contains(x * pixbinX + binningX - 1, y * pixbinY)
    //                        && MSDroi.contains(x * pixbinX + binningX - 1, y * pixbinY + binningY - 1)) || (map ==
    //                        true && MSDroi.contains(x, y))) {
    ...
    //
    //                int i = chanum - 1;
    //                while ((msd[0][msdpx1][msdpy1][i] == 0 || msd[1][msdpx1][msdpy1][i] == 0 ||
    //                msd[2][msdpx1][msdpy1][i] == 0) && (i > 2)) {
    //                    i--;
    //                }
    //                cutoff = i + 1;
    //
    ...
    //                ReadFilteringFrame(); 									// reads the current Thresholding
    //                settings
    //            } else {
    ...
    //            create a stack for the fit parameters plus chi2, blocking status, filtering mask plus q map
    //
    //            for (int m = 0; m < nochannels; m++) {							//loop over individual correlation
    //            channels and the cross-correlation
    //                cm = (m + cshift) % 3;
    //                for (int x = 0; x < wx; x++) {
    //                    for (int y = 0; y < hy; y++) {
    //                        if (doFiltering) {											//if thresholding on,
    ...
    //                if q map calculated set the stack to the last frame
    ...
    //                        if (userThreshold[4]) {
    //                            if ((pixvalid[2][x + xm][y + ym] != 1.0) && (pixvalid[0][x + xm][y + ym] *
    //                            pixvalid[1][x + xm][y + ym] == 1)) {
    ...
    //                cm = (m + cshift) % 3;
    //
    //                for (int p = 0; p < noparam; p++) {					// put pixel values for fit parameters in
    //                the maps
    //                    ImageProcessor ipPara1 = impPara1.getStack().getProcessor((m * noparam) + p + 1);
    //                    for (int x = 0; x < wx; x++) {
    //                        for (int y = 0; y < hy; y++) {
    //                            ipPara1.putPixelValue(x, y, fitres[cm][x + xm][y + ym][p] * pixvalid[cm][x + xm][y
    //                            + ym]);
    //                        }
    //                    }
    //                }
    //
    //                ImageProcessor ipPara1 = impPara1.getStack().getProcessor(nochannels * noparam + m + 1);	// set
    //                the stack to the frame for chi2
    ...
    //                }
    //
    //                ipPara1 = impPara1.getStack().getProcessor(nochannels * (noparam + 1) + m + 1);	// set the
    //                stack to the frame for blocked values
    ...
    //                }
    //
    //                ipPara1 = impPara1.getStack().getProcessor(nochannels * (noparam + 2) + m + 1);	// set the
    //                stack to the frame for filtering mask
    ...
    //            IJ.run(impPara1, "Set... ", "zoom=" + scimp + " x=" + (int) Math.floor(wx / 2) + " y=" + (int) Math
    //            .floor(hy / 2)); //then zoom to fit within application
    //            IJ.run("In [+]", ""); 	// This needs to be used since ImageJ 1.48v to set the window to the right
    //            size;
    //            // this might be a bug and is an ad hoc solution for the moment; before only the "Set" command was
    //            necessary
    //
    ...
    //            Component[] impPara1comp = impPara1Win.getComponents();	// check which component is the scrollbar
    //            and add an AdjustmentListener
    ...
    public void calcAveCF(int mode) { //mode 0-average of one functions; 1-average of 3 functions
        ...
        try {
                    ...
                    if (doFit) {
                        for (int i = 0; i < (correlatedwidth); i++) {
                            for (int j = 0; j < (correlatedheight); j++) {
                                if (acf[m][i][j][k] != 0 && !Double.isNaN(
                                        acf[m][i][j][k])) { // use this if only fitted pixels to be averaged:
                                    // pixvalid[0][i][j] == 1.0
            ...
            if (doFit) {
                //                    prepareFit();
                //                    if (initparam[1] < 0 || initparam[6] < 0 || initparam[8] < 0 || initparam[5] <
                //                    0 || initparam[7] < 0) {
                //                        JOptionPane.showMessageDialog(null, "Fit parameters are out of range.
                //                        Either D < 0 or F < 0.");
                //                        return;
                //                    }
    ...
    private void fillCalibList(int nopts) {
        ...
        if (cfXDistance != 0 || cfYDistance != 0) {
            ...
            if (plotCalibrationDiffusion) {
                ...
                //not implemented: for DC-FCCS(kcf2) or spatial FCS(kcf2) //todo fix cfxshift and cfyshift currently
                // set to 0
            }
    ...
    private double getAverageInt() {
        ...
        //        IJ.log("inside getAverageInt check parameter: startXmap: " + startXmap + ", startYmap: " +
        //        startYmap + ", endXmap: " + endXmap + ", endYmap" + endYmap);
        int counter = 0;
    ...
    private void plotCalibTrace(int pmode) {
        ...
        if (pmode == 0) {
            ...
            CalibAmplitudePlot.addPoints(calibParamListofList.get(0), calibParamListofList.get(pmode + 1),
                    CalibAmplitudePlot.CONNECTED_CIRCLES);
            CalibAmplitudePlot.setFrameSize(tempXdim, tempYdim);
            CalibAmplitudePlot.setLimits(minxx, calibParamListofList.get(0).get(calibParamListofList.get(0).size() - 1),
                    minamp, maxamp);
            ...
            if (ampCalibWindow == null || ampCalibWindow.isClosed() ==
                    true) {    // create new plot if window doesn't exist, or reuse existing window
        ...
        if (pmode == 1) {
            ...
            if (DiffCalibWindow == null || DiffCalibWindow.isClosed() == true) {
                ...
                //                //                //Setting border size (if user chose to maximis or minimize plot
                //                for clarity)
            ...
            CalibDiffusionPlot.addPoints(calibParamListofList.get(0), calibParamListofList.get(pmode + 1),
                    CalibDiffusionPlot.CONNECTED_CIRCLES);
            CalibDiffusionPlot.setFrameSize(tempXdim, tempYdim);
            CalibDiffusionPlot.setLimits(minxx, calibParamListofList.get(0).get(calibParamListofList.get(0).size() - 1),
                    mind, maxd);
            ...
            if (DiffCalibWindow == null || DiffCalibWindow.isClosed() ==
                    true) {    // create new plot if window doesn't exist, or reuse existing window
        ...
        if (pmode == 2) {
            ...
            if (intCalibWindow == null || intCalibWindow.isClosed() == true) {
                ...
                minint -= minint *
                        0.1;        // maximum scales are to be 10% larger than maximum value and 10% smaller than
                // minimum value
                maxint += maxint * 0.1;
                minxx = calibParamListofList.get(0).get(0);
                ...
                if (isCalibFixScale) {
                    ...
                    minint -= minint *
                            0.1;        // maximum scales are to be 10% larger than maximum value and 10% smaller
                    // than minimum value
                    maxint += maxint * 0.1;
                }
            ...
            CalibIntensityPlot.addPoints(calibParamListofList.get(0), calibParamListofList.get(pmode + 1),
                    CalibIntensityPlot.CONNECTED_CIRCLES);
            CalibIntensityPlot.setFrameSize(tempXdim, tempYdim);
            CalibIntensityPlot.setLimits(minxx, calibParamListofList.get(0).get(calibParamListofList.get(0).size() - 1),
                    minint, maxint);
            ...
            if (intCalibWindow == null || intCalibWindow.isClosed() ==
                    true) {    // create new plot if window doesn't exist, or reuse existing window
    ...
    public boolean setParameters() {
        ...
        boolean proceed =
                true;                                    // whether program should proceed resetting the Results
        boolean onlySigmaOrBinChanged =
                true;                            // whether sigma0 is the only parameter changed in the panel - PSF
        // calibration is not reset in that case
        boolean onlyBinChanged =
                true;                            // whether binning is the only parameter changed in the panel - diff
        // law is not reset in that case
        String[] newPanelSettings =
                new String[noSettings];        // an array to temporarily hold the settings from the Panel
        ...
        if (proceed) {
            ...
            // set maximum, and minimum cursor positions possible in the image, depending on binning and whether
            // overlap is allowed
            // parameters need to be checked according to these settings as allowed parameter ranges differ in
            // overlap and non-overlap mode
            if (overlap) {
            ...
            //            }
            //
            //            // check that the distance between pixels to be correlated is smaller than the size of the
            //            image
            ...
            //            if (checkroi) {
            //                if ((cfXDistance < 0 && Math.ceil((double) roi1StartX / pixbinX) * pixbinX <
            //                cfXDistance * (-1)) || (cfYDistance < 0 && Math.ceil((double) roi1StartY / pixbinY) *
            //                pixbinY < cfYDistance * (-1))) {
            //                    JOptionPane.showMessageDialog(null, "Correlation points are not within image.");
            //                    return false;
            //                }
            //
            //                if ((cfXDistance >= 0 && Math.floor(((double) roi1StartX + roi1WidthX - binningX) /
            //                pixbinX) * pixbinX + cfXDistance > width - binningX) || (cfYDistance >= 0 && Math.floor
            //                (((double) roi1StartY + roi1HeightY - binningY) / pixbinY) * pixbinY + cfYDistance >
            //                height - binningY)) {
            //                    JOptionPane.showMessageDialog(null, "Correlation points are not within image.");
            //                    return false;
            //                }
            ...
            //                JOptionPane.showMessageDialog(null, "Warning: CF distance is not a multiple of pixel
            //                bin.");
            //            }
            //
            ...
            //                JOptionPane.showMessageDialog(null, "Warning: Background 1 value is larger than
            ...
            //                JOptionPane.showMessageDialog(null, "Warning: Background 2 value is larger than
            //                smallest pixel value");
            //            }
            //
            ...
            //            }
            //
            //            // check whether there are enough frames for the correlation and give a warning if there
            //            are less than minFrameReq frames for the last correlation channel
            ...
            //                JOptionPane.showMessageDialog(null, "Warning: Less than " + minFrameReq + " data point
            //                for last correlation channel.");
            //            }
            // set common arrays and parameters according to user settings in the panel
            if ((lastframe - firstframe + 1) <
                    1000) {    // use 1000 points for the intensity, except when less than 1000 frames are present
                nopit = (lastframe - firstframe + 1);
            } else {
                nopit = 1000;
            }

            // if sliding window correction is needed then the correlator structure has to be adapted to the smaller
            // number of lagtimes
            ...
            //                lagnum = (int) Math.floor((Math.log(slidingWindowLength / (swMinFrameReq + correlatorp)
            //                ) + 1) / Math.log(2));
            //                if (lagnum < correlatorq) {	// allow smaller correlatorq values as minimum but not
            //                larger
            //                    correlatorq = lagnum;
            ...
            // initialize arrays required for calculations; they change with each new paramter setting and are thus
            // re-initialized
            intTrace1 = new double[nopit];
            intTrace2 = new double[nopit];
            intTime = new double[nopit];

            //            // the arrays for CFs and results of fitting are reinitialized only if key parameters have
            //            changed to prevent mixing results for different settings in a single parameter map
            //            if (resetResults) {
            //                initializeArrays();
            //                tfFitEnd.setText(Integer.toString(chanum - 1));			// reset the fitting range for
            //                data
            //                fitstart = Integer.parseInt(tfFitStart.getText());
            //                fitend = Integer.parseInt(tfFitEnd.getText());
            //                if (!onlyBinChanged && !expload) {					// if other parameter than binning
            //                has been changed, reset diff. law data and close diff. law window if it exists; but
            //                don't do anything if htis is an experimetn load
            ...
            //                        diffLawFitLim[k] = 0;
            //                    }
            //                    if (!onlySigmaOrBinChanged) {				// if other parameter than sigma or bin
            //                    have been changed, reset PSF data and close PSF window if it exists
            ...
            for (int x = 0;
                 x <= hbase; x++) {    // calculate lag and lagtimes for the 0 lagtime channel and the first 8 channels
                lag[x] = x;
            ...
            for (int x = 0;
                 x <= base; x++) {    // calculate sampletimes (bin width) for the 0 lagtime channel and the first 8
                // channels
                samp[x] = 1;
            }
            ...
            // calculate the number of samples for each channel including 0 lagtime; this differs for sliding window
            // correction
            ...
            if (cfXDistance >= 0) {
                maxcposx = pixelWidthX - (int) Math.ceil(
                        ((double) cfXDistance - (width - (pixelWidthX * pixbinX + binningX))) / pixbinX);
                mincposx = 0;
            } else {
            ...
            if (cfYDistance >= 0) {
                maxcposy = pixelHeightY - (int) Math.ceil(
                        ((double) cfYDistance - (height - (pixelHeightY * pixbinY + binningY))) / pixbinY);
                mincposy = 0;
            } else {
            ...
            //            IJ.log("psfsize: " + psfsize + ", NA: " + NA + ", emlambda: " + emlambda + ", sigma: " +
            //            sigma);
            ...
            //
            //            tfParamRx.setText(IJ.d2s(pixelsize * 1000 / objmag * cfXshift, decformat)); // set the
            //            value of rx
            //            tfParamRy.setText(IJ.d2s(pixelsize * 1000 / objmag * cfYshift, decformat)); // set the
            //            value of ry
    ...
    public void obtainImage() {
        // define array for this picture which will be used to store the results; as these arrays should stay with
        // the window they are defined here;
        // other changeable arrays are defined in setParameters(); however, if parameters are changed the arrays are
        // re-initialized.
        //  chanum = (int) (16 + (Integer.parseInt($CorrelQ) - 1) * 8 + 1); 	// value of chanum based on default Q
        //  and P, used for arrays initialization
        // NOTE: calculation of chanum has been revised. It is now based on values/settings on GUI. Instead of
        // default values as the commented line above.
        // Otherwise, there may be a downstream error, ie. array out of bound error, example in plotCF function,
        // cormode 2, fitacf array
        chanum = (int) (correlatorp + (correlatorq - 1) * correlatorp / 2 + 1);
        lagnum = (int) correlatorq;
    ...
    public void printParam() {
        ...
        //        printlog("width: " + width + ", height: " + height + ", frames: " + frames + ", firstframe: " +
        //        firstframe + ", lastframe: " + lastframe + ", frametime: " + frametime);
        //        printlog("polyorder: " + polyOrder + ",bleachCorMem: " + bleachCorMem + ", binningX: " + binningX +
        //        ", binningY: " + binningY + ", correlator p: " + correlatorp + ", correlator q: " + correlatorq +
        //        ", doFit: " + doFit + ", overlap: " + overlap);
        //        printlog("useGpu: " + Common.useGpu + ", runongpu: " + Common.RunLiveReadOutOnGPU + ", impmin: " +
        //        impmin);
        //        printlog("--fromImFCSCPUGPU class");
    }
    ...
    // calculate reduced intensity traces for plotting average and use not more than 'nopit' (defined in
    // setParameters()) points for a trace
    public void calcIntensityTrace(ImagePlus image, int ipx1, int ipy1, int ipx2, int ipy2, int initialframe,
        ...
        // initialframe and finalframe provide the range of frames to be used
        int ave = (int) Math.floor(
                (finalframe - initialframe + 1) / nopit); // calculate number of data points which are averaged
        int sum1;
        int sum2;
        int bckg1 = background; // artificially setted to zero to make sure we dont subrtract background when displaying
        // intensity trance; does not affect ACF
    ...
    public void correlate(ImagePlus image, int px1, int py1, int px2, int py2, int kcf, int initialframe,
        ...
        int swinitialframe;                    // if sliding window (bleach) correction is selected these are the
        // initial and final frames of the sub-windows
        int swfinalframe;
        int pxm1;                                // pixel coordinates on the binned grid used to store the output
        // and map it to the parameter map
        int pym1;

        if (kcf == 1 && px1 == px2 && py1 ==
                py2) {    // if red channel in the DC-FCCS mode, map the output to the corresponding green-channel
            // pixels on the binned grid
        ...
        datac[0] = getIntensity(image, px1, py1, 1, initialframe,
                finalframe);        // getIntensity for first pixel; performs a bleach correction if indicated in the
        // panel
        if (px1 != px2 ||
                py1 != py2) {                        // if the two pixels are not equal (i.e. for a cross-correlation)
        ...
        if (doMSD) {
            if (!MSDmode) { // 2D if MSDmode is false, otherwise 3D
                msd[kcf][pxm1][pym1] = correlationToMSD(acf[kcf][pxm1][pym1], pixeldimx * Math.pow(10, 6), psfsize *
                Math.pow(10, 6));
            } else {
                msd[kcf][pxm1][pym1] = correlationToMSD3D(acf[kcf][pxm1][pym1], pixeldimx * Math.pow(10, 6), psfsize
                * Math.pow(10, 6), lsthickness * Math.pow(10, 6));
            }
        }
         */
    }
    ...
    public void correlate(ImagePlus image, ImagePlus image2, int px1, int py1, int kcf, int initialframe,
                          int finalframe) {
        // image: the imp to be used
    ...
    public Map correlator(double[][] intcor, int initialframe, int finalframe) {
        ...
        // initialframe and finalframe provide the range of frames to be used for the correlation
        int num = (finalframe - initialframe + 1);            // total number of frames to be correlated
        int blockIndex;                                    // index at which optimal blocking is reached; if it
        // fails maximal blocking is used

        blockIndex = blockTransform(intcor, num,
                1);            // perform blocking on the first channel to determine when intensity bins are independent

        Map result;
        result = calculateCF(intcor, num, blockIndex,
                1);            // perform optimal blocking and return the CF, SD and covariance matrix
    ...
    public Map calculateCF(double[][] intcor, int num, int ind, int blocklag) {
        ...
        int binct;
        int pnum = (int) Math.floor(mtab[chanum - 1] / Math.pow(2,
                Math.max(ind - Math.log(samp[chanum - 1]) / Math.log(2),
                        0)));    // minimum number of prodcuts given the used correlator structure and the blockIndex
        // ind
        int[] prodnum = new int[chanum];
        double sumprod;        // sum of all intensity products; divide by num to get the average <i(n)i(n+del)>
        double sumprod2;    // sum of all intensity products squared; divide by num to get the average <(i(n)i
        // (n+del))^2>
        ...
        double[][] covmats = new double[chanum - 1][chanum -
                1];    //the final results does not contain information about the zero lagtime channel
        ...
        blocksd[0] =
                0;                // we do not calcualte the SD for the 0 lagtime as it is not used for fitting (shot
        // noise)
        ...
        for (int x = 0; x < chanum; x++) {                        // run over all channels except the 0 lag time
            ...
            if (currentIncrement != samp[x]) {                    // check whether the channel width has changed
                numbin =
                        (int) Math.floor(numbin / 2);                // if yes, correct the number of actual data points
                currentIncrement = (int) samp[x];                    // set the currentIncrement accordingly
                ctbin++;                                            // count how often the data was binned
                for (int y = 0;
                     y < numbin; y++) {                    // and bin the data according to the width of the current
                    // channel
            ...
            del = lag[x] /
                    currentIncrement;                            // calculate the delay, i.e. the correlation time ...
            prodnum[x] =
                    numbin - del;                                // and the number of products for that delay; //(int)
            ...
            directm[x] /=
                    prodnum[x];        // calculate average of direct and delayed monitor, i.e. the average intensity
            // <n(0)> and <n(tau)>
            ...
            for (int y = 0; y < prodnum[x]; y++) {                    // calculate the correlation
                ...
                sumprod += prod[x][y];                                // calculate the sum of prod, i.e. the raw
                // correlation value ...
                sumprod2 += Math.pow(prod[x][y], 2);                // ... and the sum of the squares
            }

            corav[x] = sumprod / (prodnum[x] * directm[x] *
                    delayedm[x]);    // calculate the ACF, i.e. the mean for the later calculations of the
            // variance-covariance matrix

            binct = ind -
                    ctbin; // determine whether data needs to be further binned or is already exceeding the blocking
            // number
            sumprod = 0;
            sumprod2 = 0;
            ...
            for (int y = 1; y <= binct; y++) {                    // bin the data until block time is reached
                prodnum[x] =
                        (int) Math.floor(prodnum[x] / 2);        // for each binning the number of data points is halfed
                for (int z = 0;
                     z < prodnum[x]; z++) {            // do the binning and divide by 2 so that average value does
                    // not change
                    prod[x][z] = (prod[x][2 * z] + prod[x][2 * z + 1]) / 2;
                }
            }

            prodnum[x] = pnum;                                        // use only the minimal number of products to
            // achieve a symmetric variance matrix
            for (int z = 0; z < prodnum[x]; z++) {
                sumprod += prod[x][z];                                // calculate the sum of prod, i.e. the raw
                // correlation value ...
                sumprod2 += Math.pow(prod[x][z], 2);                // ... and the sum of the squares
            }

            blockvar[x] = (sumprod2 / prodnum[x] - Math.pow(sumprod / prodnum[x], 2)) / ((prodnum[x] - 1) *
                    Math.pow(directm[x] * delayedm[x],
                            2));    // variance after blocking; extra division by prodnum to obtain SEM
            blocksd[x] = Math.sqrt(
                    blockvar[x]);                                                                                                    // standard deviation after blocking
        }
        ...
        //            for (int x = 1; x < chanum; x++) {
        //                for (int y = 1; y <= x; y++) {	// calculate only the upper triangular part as the matrix
        //                is symmetric
        //                    for (int z = 0; z < pnum; z++) {
        //                        covmat[x][y] += (prod[x][z] / (directm[x] * delayedm[x]) - mcov[x]) * (prod[y][z] /
        //                        (directm[y] * delayedm[y]) - mcov[y]);
        //                    }
        ...
        //            for (int x = 1; x < chanum; x++) {			// get the variance (diagonal of covariance
        //            matrix) ...
        ...
        //                tmpnum = 0;
        //                for (int z = 0; z < pnum; z++) {
        //                    tmpnum += Math.pow((Math.pow(prod[x][z] / (directm[x] * delayedm[x]) - mcov[x], 2) -
        //                    covmat[x][x]), 2);
        //                }
        ...
        //            for (int x = 1; x < chanum; x++) {						// calculate the sample correlation
        //            matrix
        ...
        //                tmpnum = 0;
        //                for (int y = 1; y < x; y++) {				//sum only over the upper triangle as the matrix
        //                is symmetric
        //                    for (int z = 0; z < pnum; z++) {
        ...
        //                    tmpnum *= (pnum) / Math.pow(pnum - 1, 3);
        //                    numerator += tmpnum;
        //                    denominator += Math.pow(cormat[x][y], 2);		// sum of squares of off-diagonal elements
        ...
        //            for (int x = 1; x < chanum; x++) {		// do not include zero lagtime channel as we don't use
        //            it for fitting
        //                for (int y = 1; y < x; y++) {
        ...
        //                }
        //            }
        //            for (int x = 1; x < chanum; x++) {	// diagonal elements of the regularized
        //            variance-covariance matrix
        ...
        //                    for (int y = 1; y < chanum; y++) {
        //                        impCovIp.putPixelValue(x - 1, y - 1, covmats[x - 1][y - 1]);			// regularized
        //                        var-cov matrix
        //                        //impCovIp.putPixelValue(x, y, Math.sqrt(covmat[x][y]));	// non-regularized var-cov
        //                        matrix
        //                        //impCovIp.putPixelValue(x, y, Math.sqrt(cormat[x][y]));	// sample correlation
        //                        matrix
        //                    }
        //                }
        //
        ...
        //                IJ.run(impCov, "Set... ", "zoom=" + 200 + " x=" + 0 + " y=" + 0); //then zoom to fit within
        //                application
        //                IJ.run("In [+]", ""); 	// This needs to be used since ImageJ 1.48v to set the window to
        //                the right size;
        //            }
        //        } // end of if GLS selected statement
        Map<String, Object> map = new HashMap<>();
        if (GLS) {    //hand over either the correlation function corav or the actual function used to calcualte the
            // covariance matrix; they differ only slightly
            map.put("corav", mcov);
        } else {
            map.put("corav", corav);
        }
    ...
    public int blockTransform(double[][] intcor, int num, int blocklag) {
        ...
        // num is the number of frames which are correlated
        // blocklag defines for which lag the blocking will be done; typically we use the smalles, i.e. 1
        int blocknum = (int) Math.floor(Math.log(mtab[blocklag]) / Math.log(2)) -
                2;    // number of blocking operations that can be performed given blocklag
        int numbin = num;        // number of data points when they are binned
        int del;                // delay or correlation time expressed in lags
        int currentIncrement;
        ...
        double sumprod2 =
                0.0;    // sum of all intensity products squared; divide by num to get the average <(i(n)i(n+del))^2>
        ...
        double[] blocksd;
        int[] crt;
        int[] cr12;            // do neighbouring points have overlapping error bars; together with crwin=2 this
        // tests for three points that have overlapping erro bars
        int[] cr3;
        int[] diffpos;
        int last0 = 0;
        int ind = 0;
        ...
        currentIncrement =
                blocklag;        // at the moment we always do blocking for smallest lag which is 1 but in general it
        // can be used freely

        for (int x = 1; x < chanum; x++) {                                    // run over all channels
            if (currentIncrement !=
                    samp[x]) {                                // check whether the channel width has changed
                currentIncrement = (int) samp[x];                                // set the currentIncrement accordingly
                numbin = (int) Math.floor(numbin /
                        2);                            // and correct the number of actual data points accordingly
                for (int y = 0;
                     y < numbin; y++) {                                // if yes, bin the data according to the width
            ...
            if (x == blocklag) {                                        // if the channel number is equal to the
                // blocklag ...
                del = lag[x] /
                        currentIncrement;                            // calculate the delay, i.e. the correlation time
                ...
                directm /= prodnum[0];                                    // calculate average of direct and delayed
                // monitor,
                delayedm /=
                        prodnum[0];                                    // i.e. the average intesity <n(0)> and <n(tau)>
                ...
                for (int y = 0; y < prodnum[0]; y++) {                    // calculate the correlation
                    ...
                    sumprod += prod[y];                                    // calculate the sum of prod, i.e. the raw
                    // correlation value ...
                ...
                for (int y = 1; y < blocknum; y++) {                    // perform blocking operations
                    prodnum[y] = (int) Math.floor(prodnum[y - 1] /
                            2);    // the number of samples for the blocking curve decreases by a factor 2 with every
                    // step
                    sumprod = 0;
                    sumprod2 = 0;
                    for (int z = 0;
                         z < prodnum[y]; z++) {            // bin the correlation data and calculate the blocked
                        // values for the SD
        ...
        for (int x = 0;
             x < blocknum - 1; x++) {                            // do neighboring points have overlapping error bars?
        ...
        for (int x = 0; x <
                blocknum - 2; x++) {                            // do three adjacent points have overlapping error bars?
            if (crt[x] * crt[x + 1] == 1) {
                cr12[x] = 1;
            }
        }

        for (int x = 0; x < blocknum -
                1; x++) {                            // do neighboring points have a positive difference (increasing
            // SD)?
        ...
        for (int x = 0;
             x < blocknum - 2; x++) {                            // do three neighboring points monotonically increase?
        ...
        for (int x = 0; x < blocknum -
                2; x++) {                            // find the last triple of points with monotonically increasing
        ...
        for (int x = 0;
             x <= last0; x++) {                                // indices of two pairs that pass criterion 1 an 2
            cr12[x] = 0;
        }

        cr12[blocknum - 3] =
                0;                                                // criterion 3, the last two points can't be part
        // of the blocking triple
        cr12[blocknum - 4] = 0;

        for (int x = blocknum - 5;
             x > 0; x--) {                            // index of triplet with overlapping error bars and after which
            // no other triplet has a significant monotonic increase
            if (cr12[x] == 1) {                                            // or 4 increasing points
                ind = x + 1;                                                // take the middle of the three points as
                // the blocking limit
            }
        }

        if (ind == 0) {                                                    // if optimal blocking is not possible, use
            // maximal blocking
            blockIndS = 0;
            if (blocknum - 3 > 0) {
                ind = blocknum -
                        3;                                            // maximal blocking is performed for the 3rd
                // last point in the blocking curve if that exists
            } else {
                ind = blocknum - 1;
            }
        } else {
    ...
    // get intensity data for correlate() and correct for bleaching if required; note that you should call
    // calcIntensityTrace() before to obtain intTrace1 and 2
    public double[] getIntensity(ImagePlus image, int px, int py, int mode, int initialframe, int finalframe) {
        ...
        for (int x = 1;
             x <= num; x++) {    //read data from all relevant pixels, depending on the selected frames and binning
            for (int i = 0; i < binningX; i++) {
                for (int k = 0; k < binningY; k++) {
                    if (bgrloaded) {
        ...
        String $bcmode =
                (String) bleachCorMem;    // perform single or double exponential bleach corrections if selected

        //        if ("Single Exp".equals($bcmode)) {
        //            SingleExpFit efit = new SingleExpFit();
        //            if (mode == 1) {
        //                res = efit.doFit(intTrace1);			// note that the bleach correction is performed on the
        //                averaged intensity traces to make it faster
        //            } else {									// while you may have 20,000 intensity points,
        //            intTrace1 and 2 contain only 1,000 points
        //                res = efit.doFit(intTrace2);			// see definition in setParameters()
        //            }
        //            if (res[0] * res[1] != 0) {				// correct the full intensity trace if the fit was
        //            succesful
        //                for (int x = 1; x <= num; x++) {
        //                    intdat[x] = intdat[x] / Math.sqrt((res[0] * Math.exp(-frametime * (x + 0.5) / res[1]) +
        //                    res[2]) / (res[0] + res[2])) + (res[0] + res[2]) * (1 - Math.sqrt((res[0] * Math.exp
        //                    (-frametime * (x + 0.5) / res[1]) + res[2]) / (res[0] + res[2])));
        //                }
        //                if (mode == 1) {
        //                    for (int x = 0; x < nopit; x++) {
        //                        intTrace1[x] = intTrace1[x] / Math.sqrt((res[0] * Math.exp(-intTime[x] / res[1]) +
        //                        res[2]) / (res[0] + res[2])) + (res[0] + res[2]) * (1 - Math.sqrt((res[0] * Math
        //                        .exp(-intTime[x] / res[1]) + res[2]) / (res[0] + res[2])));
        //                    }
        //                }
        //                if (mode == 2) {
        //                    for (int x = 0; x < nopit; x++) {
        //                        intTrace2[x] = intTrace2[x] / Math.sqrt((res[0] * Math.exp(-intTime[x] / res[1]) +
        //                        res[2]) / (res[0] + res[2])) + (res[0] + res[2]) * (1 - Math.sqrt((res[0] * Math
        //                        .exp(-intTime[x] / res[1]) + res[2]) / (res[0] + res[2])));
        //                    }
        //                }
        ...
        //            }
        //        }
        //        if ("Double Exp".equals($bcmode)) {		// same as single exponential fit only for double
        //        exponential
        ...
        //                for (int x = 1; x <= num; x++) {
        //                    intdat[x] = intdat[x] / Math.sqrt((res[0] * Math.exp(-frametime * (x + 0.5) / res[1]) +
        //                    res[2] * Math.exp(-frametime * (x + 0.5) / res[3]) + res[4]) / (res[0] + res[2] +
        //                    res[4])) + (res[0] + res[2] + res[4]) * (1 - Math.sqrt((res[0] * Math.exp(-frametime *
        //                    (x + 0.5) / res[1]) + res[2] * Math.exp(-frametime * (x + 0.5) / res[3]) + res[4]) /
        //                    (res[0] + res[2] + res[4])));
        //                }
        //                if (mode == 1) {
        //                    for (int x = 0; x < nopit; x++) {
        //                        intTrace1[x] = intTrace1[x] / Math.sqrt((res[0] * Math.exp(-intTime[x] / res[1]) +
        //                        res[2] * Math.exp(-intTime[x] / res[3]) + res[4]) / (res[0] + res[2] + res[4])) +
        //                        (res[0] + res[2] + res[4]) * (1 - Math.sqrt((res[0] * Math.exp(-intTime[x] /
        //                        res[1]) + res[2] * Math.exp(-intTime[x] / res[3]) + res[4]) / (res[0] + res[2] +
        //                        res[4])));
        //                    }
        //                }
        //                if (mode == 2) {
        ...
        if ("Polynomial".equals($bcmode)) {        //fitting with a polynomial of selected order
            ...
            if (mode == 1) {
                res = polfit.doFit(
                        intTrace1);            // note that the bleach correction is performed on the averaged
                // intensity traces to make it faster
            } else {                                    // while you may have 20,000 intensity points, intTrace1 and
                // 2 contain only 1,000 points
            ...
            if (mode == 1) {
                for (int x = 0; x < nopit; x++) {
                    ...
                    intTrace1[x] =
                            intTrace1[x] / Math.sqrt(corfunc / res[0]) + res[0] * (1 - Math.sqrt(corfunc / res[0]));
                }
            }
            if (mode == 2) {
                for (int x = 0; x < nopit; x++) {
                    ...
                    intTrace2[x] =
                            intTrace2[x] / Math.sqrt(corfunc / res[0]) + res[0] * (1 - Math.sqrt(corfunc / res[0]));
                }
            }
        }

        //        if ("Lin Segment".equals($bcmode)) {		// approximating bleaching by a partially linear function
        //            int ave = (int) Math.floor((finalframe - initialframe + 1) / nopit); // number of points
        //            averaged in intTrace
        ...
        //            }
        //
        //            bcInt0 = bcTrace[0] + (bcTrace[0] - bcTrace[1]) / 2;	// Initial intensity obtained by
        //            extrapolating the line between average intensities in 1st and second segment to 0
        //
        //            for (int x = 1; x < (int) Math.floor(bcAve / 2); x++) {
        //                intdat[x] = intdat[x] / Math.sqrt((bcTrace[0] + (bcTrace[0] - bcTrace[1]) / bcAve * (bcAve
        //                / 2 - x)) / bcInt0) + bcInt0 * (1 - Math.sqrt((bcTrace[0] + (bcTrace[0] - bcTrace[1]) /
        //                bcAve * (bcAve / 2 - x)) / bcInt0));
        //            }
        //
        //            for (int x = 1; x < bcNopit; x++) {
        //                for (int y = 0; y < bcAve; y++) {
        //                    nf = (x - 1) * bcAve + (int) Math.floor(bcAve / 2) + y;
        //                    intdat[nf] = intdat[nf] / Math.sqrt((bcTrace[x - 1] + (bcTrace[x] - bcTrace[x - 1]) * y
        //                    / bcAve) / bcInt0) + bcInt0 * (1 - Math.sqrt((bcTrace[x - 1] + (bcTrace[x] - bcTrace[x
        //                    - 1]) * y / bcAve) / bcInt0));
        //                }
        //            }
        //
        ...
        //                intdat[x] = intdat[x] / Math.sqrt((bcTrace[bcNopit - 2] + (bcTrace[bcNopit - 1] -
        //                bcTrace[bcNopit - 2]) * nf / bcAve) / bcInt0) + bcInt0 * (1 - Math.sqrt((bcTrace[bcNopit -
        //                2] + (bcTrace[bcNopit - 1] - bcTrace[bcNopit - 2]) * nf / bcAve) / bcInt0));
        //            }
        //
        //            if (mode == 1) {
        //                for (int x = 0; x < nopit; x++) {
    ...
    public double[] correlationToMSD(double[] corrfunc, double psize, double psfwidth) {
        ...
        for (int i = 1; i < cutoffInd; i++) {
            d = pi * corrfunc[i] / corrfunc[1] / (obsvolFCS_ST2D1p(2) * Math.pow(10, 12)) * Math.pow(psize, 2) *
                    1260.0 / 29.0;
        ...
        for (int i = 2; i < cutoffInd; i++) {
            ...
            UnivariateFunction f = (double x) -> {
                ...
                double corrp = temp_msd_3d * sqrpi * temp_psize * temp_psize * temp_psfwidthz /
                        (obsvolFCS_ST2D1p(3) * Math.sqrt(pi) * temp_psfwidthz * Math.pow(10, 12)) / temp1_msd_3d;
    ...
    //This calculates the solution to a third order polynomial which is obtained from the fourth order polynomial.
    // The cubic which appears is called resolvent cubic equation
    ...
    public void prepareFit() {
        ...
        //        IJ.log("paraminitval[0]: " + paraminitval[0] + ", paraminitval[1]: " + paraminitval[1] + ",
        //        paraminitval[2]: " + paraminitval[2] + ", paraminitval[3]: " + paraminitval[3]);
        //        IJ.log("paraminitval[4]: " + paraminitval[4] + ", paraminitval[5]: " + paraminitval[5] + ",
        //        paraminitval[6]: " + paraminitval[6] + ", paraminitval[7]: " + paraminitval[7]);
        //        IJ.log("paraminitval[8]: " + paraminitval[8] + ", paraminitval[9]: " + paraminitval[9] + ",
        //        paraminitval[10] : " + paraminitval[10]);
        //        IJ.log("q2: " + q2 + ", q3: " + q3);
        //        IJ.log("pixeldimx: " + pixeldimx + ", pixelsize: " + pixelsize + ", objmag: " + objmag + ",
    ...
    public double obsvolFCS_ST2D1p(int dim) {
        ...
        double szeff = Math.sqrt(1 / (Math.pow(sz, -2.0) + Math.pow(psfz,
                -2.0))); // convolution of two Gaussians depending on illumination profile and detection PSF
            ...
            for (int t = startFrame; t < this_framediff; t++) {
                for (int j = startY; j < this_height; j++) {
                    for (int i = startX; i < this_width; i++) {
                        pixels[t * this_height * this_width + j * this_width + i] =
                                (float) imp.getStack().getVoxel(i, j, t);
                    }
                }
            }
        }
     ...
     */
    private boolean GPU_Calculate_ACF(float pixels[], double pixels1[], double blockvararray[], double NBmeanGPU[],
                                      double NBcovarianceGPU[], double bleachcorr_params[], GpuParameters GPUparams) {
        ...
        // run GPU code
        try {
            GpuCorrelator.calcACF(pixels, pixels1, blockvararray, NBmeanGPU, NBcovarianceGPU, blocked1D,
        ...
        for (int y = 0; y < GPUparams.height; y++) {
            for (int x = 0; x < GPUparams.width; x++) {
                blocked[0][x + roi1StartX][y + roi1StartY] =
                        (int) blocked1D[GPUparams.width * GPUparams.height + y * GPUparams.width + x];
            }
        }
        return IsGPUCalculationOK;
    }

    public void initializeFitres(int a, int b, int c,
                                 int d) {    //initializes arrays for fit parameters, chi2 and cross-correlation
        // amount with NaN
    ...
    private boolean GPU_Initialize_GPUparams(GpuParameters GPUparams, boolean isNBcalculation) {
        //pixbinX and pixbinY in the cpu plugin are strides in width and height respectively. They  are determined
        // automatically depending on the overlap in the plugin.
        int w_out = isNBcalculation ? width : (int) Math.floor(
                (Math.min(roi1WidthX + cfXDistance, width) - cfXDistance) / pixbinX);
        int h_out = isNBcalculation ? height : (int) Math.floor(
                (Math.min(roi1HeightY + cfYDistance, height) - cfYDistance) / pixbinY);

        if (w_out <= 0 || h_out <= 0) {
            return false;
        } else {
            // win_star and hin_star determines the necessary area/pixels of all required data for GPU calculations.
            // See function getVoxels too.
            ...
            // so that we have all the necessary data for all scenarios of our GPU calculations. It
            // simplifies/generalizes the indexing task in our CUDA kernels.
            ...
            GPUparams.mtabchanumminus1 =
                    mtab[chanum - 1]; // mtab[chanum-1], used to calculate pnumgpu[counter_indexarray]
            GPUparams.sampchanumminus1 =
                    samp[chanum - 1]; // samp[chanum-1], used to calculate pnumgpu[counter_indexarray]
    ...
    private boolean GPU_get_pixels(GpuParameters GPUparams, float[] pixels, boolean isNBcalculation) {
        ...
        if (GPUparams.binningY == 1 && GPUparams.binningX == 1) {
            ...
            // We found that the JNI function SetFloatArrayRegion fails when the output array is too huge.
            // Tentatively, we set a limit of 96*96*50000.
            boolean WithinSizeLimit = (GPUparams.w_temp * GPUparams.h_temp * GPUparams.framediff) < 96 * 96 * 50000;

            if (WithinSizeLimit && GpuParameters.isBinningMemorySufficient(GPUparams)) {
                // Binning on GPU
                try {
                    GpuParameters.calcBinning(pixels_approach2, pixels, GPUparams);
                } catch (Exception e) {
                    IsGPUCalculationOK = false;
                    e.printStackTrace(System.out);
                }
        ...
        if (bgrloaded) {
            ...
            for (int k = 0; k < GPUparams.framediff; k++) {
                for (int j = 0; j < GPUparams.h_temp; j++) {
                    for (int i = 0; i < GPUparams.w_temp; i++) {
                        pixels[k * GPUparams.h_temp * GPUparams.w_temp + j * GPUparams.w_temp + i] -=
                                bgrmeangpu[j * GPUparams.w_temp + i];
                    }
                }
            }
        } else {
    ...
    private boolean GPU_Calculate_BleachCorrection(GpuParameters GPUparams, float[] pixels,
                                                   double[] bleachcorr_params) {
        boolean IsGPUCalculationOK = true;

        try {
            float[] datableach_correction = new float[GPUparams.w_temp * GPUparams.h_temp * nopit];
            GpuParameters.calcDataBleachCorrection(pixels, datableach_correction, GPUparams);
            int numberFitsbleach_correction = GPUparams.w_temp * GPUparams.h_temp, numberPoints_bleach_correction =
                    nopit;
            ...
            // NOTE: initialization of 0-th term is different from CPU code, where in class PolynomFit (extends
            // AbstractCurveFitter), the last point in target array is used as offset estimate.
            float[] initialParameters_bleachcorrection =
                    new float[numberFitsbleach_correction * model_bleachcorrection.numberParameters];
            for (int i = 0; i < numberFitsbleach_correction; i++) {
                ...
                for (int j = 0; j < model_bleachcorrection.numberParameters; j++) {
                    initialParameters_bleachcorrection[offset + j] = (j == 0) ? datableach_correction[(i + 1) * nopit -
                            1] : (float) 0; // last value of every nopit points. This works the best.
                }
            }
            ...
            GpuFitModel fitModel_bleachcorrection =
                    new GpuFitModel(numberFitsbleach_correction, numberPoints_bleach_correction, true,
                            model_bleachcorrection, tolerance_bleachcorrection, maxNumberIterations_bleachcorrection,
                            GPUparams.bleachcorr_order, parameters_to_fit_bleachcorrection, estimator_bleachcorrection,
                            nopit * Float.SIZE / 8);
            ...
            FitResult fitResult_bleachcorrection = Gpufit.fit(fitModel_bleachcorrection);
            ...
            for (int y1 = 0; y1 < GPUparams.h_temp; y1++) {
                for (int x1 = 0; x1 < GPUparams.w_temp; x1++) {
                    for (int ii = 0; ii < GPUparams.bleachcorr_order; ii++) {
                        bleachcorr_params[counter4] = fitResult_bleachcorrection.parameters.get(
                                (y1 * GPUparams.w_temp + x1) * model_bleachcorrection.numberParameters + ii);
                        counter4 += 1;
                    }
                }
            }
    ...
    private boolean GPU_ACF_Fit(GpuParameters GPUparams, double[] pixels1, double[] blockvararray) {
        boolean IsGPUCalculationOK = true;
        ...
        try {
            ...
            for (int y = 0; y < GPUparams.height; y++) {
                for (int x = 0; x < GPUparams.width; x++) {
                    for (int z = 0; z < GPUparams.chanum - 1; z++) {
                        data[counter] =
                                (float) pixels1[(z + 1) * GPUparams.width * GPUparams.height + y * GPUparams.width + x];
                        counter += 1;
                    }
                }
            }
            ...
            for (int y = 0; y < GPUparams.height; y++) {
                for (int x = 0; x < GPUparams.width; x++) {
                    for (int z = 0; z < GPUparams.chanum - 1; z++) {
                        weights[counter1] = (float) 1.0 /
                                (float) blockvararray[(z + 1) * GPUparams.width * GPUparams.height +
                                        y * GPUparams.width + x];
                        varacf[0][x][y][z] =
                                blockvararray[(z + 1) * GPUparams.width * GPUparams.height + y * GPUparams.width + x];
            ...
            GpuFitModel fitModel =
                    new GpuFitModel(numberFits, numberPoints, true, model, tolerance, maxNumberIterations,
                            GPUparams.bleachcorr_order, parameters_to_fit, estimator,
                            (GPUparams.chanum - 1) * Float.SIZE / 8);
            ...
            for (int i1 = 0; i1 < GPUparams.width; i1++) {
                for (int i2 = 0; i2 < GPUparams.height; i2++) {
                    ...
                    if (converged[i2 * GPUparams.width + i1]) {
                        ...
                        for (int j = 0; j < model.numberParameters; j++) {
                            gpuresultarray[i1 + roi1StartX][i2 + roi1StartY][j] = (double) fitResult.parameters.get(
                                    (i2 * GPUparams.width + i1) * model.numberParameters + j);
                        }
                    }
                }
            }
    ...
    public boolean GPU_Calculate_ACF_All(Roi improi) {
                        ...
                        for (int x3 = 0; x3 < binningX; x3++) {
                            for (int x4 = 0; x4 < binningY; x4++) {
                                if (improi.contains(x1, x2) && improi.contains(x1, x2 + binningY - 1) &&
                                        ...
                                        improi.contains(x1 + binningX - 1, x2 + binningY - 1)) {
                                    filterArray[x1][x2] +=
                                            imp.getStack().getProcessor(firstframe).get(x1 + x3, x2 + x4);
                                } else {
                                    filterArray[x1][x2] = Float.NaN;
                                }
                            }
        ...
        GpuParameters GPUparams = new GpuParameters();
        ...
        // if (!GpuParameters.isACFmemorySufficient(GPUparams)) {
        //     IsGPUCalculationOK = false;
        ...
        if (IsGPUCalculationOK) {
            ...
            try {
                IsGPUCalculationOK =
                        GPU_Calculate_ACF(pixels, pixels1, blockvararray, NBmeanGPU, NBcovarianceGPU, bleachcorr_params,
                                GPUparams);
                ...
                for (int z = 0; z < GPUparams.chanum; z++) {
                    for (int y = 0; y < GPUparams.height; y++) {
                        for (int x = 0; x < GPUparams.width; x++) {
                            //                            acf[0][x][y][z] = pixels1[z * GPUparams.width * GPUparams
                            //                            .height + y * GPUparams.width + x];
                            acf[0][x + roi1StartX][y + roi1StartY][z] =
                                    pixels1[z * GPUparams.width * GPUparams.height + y * GPUparams.width + x];
                        }
                    }
                }
    ...
    // FCS model: 3D fit assuming 3 components and flow in x and y direction; this is the general fit formula;
    // parameters can be set to 0 to obtain simpler models
    ...
    class FCS_3p implements ParametricUnivariateFunction {
        ...
        double sz = lsthickness;
        double psfz = 2 * emlambda / Math.pow(10, 9.0) * 1.33 / Math.pow(NA, 2.0); // size of PSF in axial direction
        //double szeff = Math.sqrt( 1 / ( Math.pow(sz, -2.0) + Math.pow(psfz, -2.0) ) ); // convolution of two
        // Gaussians depending on illumination profile and detection PSF
        double szeff = sz;
        double rx = ax * cfXshift / binningX;
        double ry = ay * cfYshift / binningY;
        ...
        public double[] gradient(double x, double[] params) {
            ...
            // note that x is used here as the time variable instead of t; that can be come confusing as x and y are
            // used in the names for the paramaters to indicate spatial directions
            // pareq[0] = N
            // pareq[1] = D
            // pareq[2] = vx
            // pareq[3] = vy
            ...
            double p4xt =
                    2 * Math.pow(ax, 2) + 3 * Math.pow(rx, 2) - 6 * x * rx * pareq[2] + 3 * Math.pow(x * pareq[2], 2);
            ...
            double p4yt =
                    2 * Math.pow(ay, 2) + 3 * Math.pow(ry, 2) - 6 * x * ry * pareq[3] + 3 * Math.pow(x * pareq[3], 2);
            ...
            double dDpexpxt = 2 * Math.exp(-p4xt / Math.pow(p0t, 2)) *
                    (Math.exp(p5xt / Math.pow(p0t, 2)) + Math.exp(p6xt / Math.pow(p0t, 2)) -
                            2 * Math.exp(p7xt / Math.pow(p0t, 2)));
            ...
            double dDpexpyt = 2 * Math.exp(-p4yt / Math.pow(p0t, 2)) *
                    (Math.exp(p5yt / Math.pow(p0t, 2)) + Math.exp(p6yt / Math.pow(p0t, 2)) -
                            2 * Math.exp(p7yt / Math.pow(p0t, 2)));
            ...
            double perfxt2 =
                    p1xt2 * Erf.erf(p1xt2 / p0t2) + p2xt2 * Erf.erf(p2xt2 / p0t2) - 2 * p3xt2 * Erf.erf(p3xt2 / p0t2);
            double dDpexpxt2 = 2 * Math.exp(-p4xt2 / Math.pow(p0t2, 2)) *
                    (Math.exp(p5xt2 / Math.pow(p0t2, 2)) + Math.exp(p6xt2 / Math.pow(p0t2, 2)) -
                            2 * Math.exp(p7xt2 / Math.pow(p0t2, 2)));
            ...
            double dDpexpyt2 = 2 * Math.exp(-p4yt2 / Math.pow(p0t2, 2)) *
                    (Math.exp(p5yt2 / Math.pow(p0t2, 2)) + Math.exp(p6yt2 / Math.pow(p0t2, 2)) -
                            2 * Math.exp(p7yt2 / Math.pow(p0t2, 2)));
            double dvyperfyt2 = (Erf.erf(p2yt2 / p0t2) + 2 * Erf.erf(p3yt2 / p0t2) - Erf.erf(p1yt2 / p0t2)) * x;
            double perfyt2 =
                    p1yt2 * Erf.erf(p1yt2 / p0t2) + p2yt2 * Erf.erf(p2yt2 / p0t2) - 2 * p3yt2 * Erf.erf(p3yt2 / p0t2);

            //CF for the lateral dimension (x, y) and its derivative for D
            double plat2 = (p0t2 / sqrpi * pexpxt2 + perfxt2) * (p0t2 / sqrpi * pexpyt2 + perfyt2) /
                    (4 * Math.pow(ax * ay, 2) / fitobsvol);
            double dDplat2 = (1 / (sqrpi * p0t2)) * (dDpexpyt2 * x * (p0t2 / sqrpi * pexpxt2 + perfxt2) +
                    dDpexpxt2 * x * (p0t2 / sqrpi * pexpyt2 + perfyt2)) / (4 * Math.pow(ax * ay, 2) / fitobsvol);
            ...
            double perfxt3 =
                    p1xt3 * Erf.erf(p1xt3 / p0t3) + p2xt3 * Erf.erf(p2xt3 / p0t3) - 2 * p3xt3 * Erf.erf(p3xt3 / p0t3);
            double dDpexpxt3 = 2 * Math.exp(-p4xt3 / Math.pow(p0t3, 2)) *
                    (Math.exp(p5xt3 / Math.pow(p0t3, 2)) + Math.exp(p6xt3 / Math.pow(p0t3, 2)) -
                            2 * Math.exp(p7xt3 / Math.pow(p0t3, 2)));
            ...
            double dDpexpyt3 = 2 * Math.exp(-p4yt3 / Math.pow(p0t3, 2)) *
                    (Math.exp(p5yt3 / Math.pow(p0t3, 2)) + Math.exp(p6yt3 / Math.pow(p0t3, 2)) -
                            2 * Math.exp(p7yt3 / Math.pow(p0t3, 2)));
            double dvyperfyt3 = (Erf.erf(p2yt3 / p0t3) + 2 * Erf.erf(p3yt3 / p0t3) - Erf.erf(p1yt3 / p0t3)) * x;
            double perfyt3 =
                    p1yt3 * Erf.erf(p1yt3 / p0t3) + p2yt3 * Erf.erf(p2yt3 / p0t3) - 2 * p3yt3 * Erf.erf(p3yt3 / p0t3);

            // TRIPLET
            double triplet = 1 + pareq[9] / (1 - pareq[9]) * Math.exp(-x / pareq[10]);
            double dtripletFtrip =
                    Math.exp(-x / pareq[10]) * (1 / (1 - pareq[9]) + pareq[9] / Math.pow(1 - pareq[9], 2));
            double dtripletTtrip =
                    Math.exp(-x / pareq[10]) * (pareq[9] * x) / ((1 - pareq[9]) * Math.pow(pareq[10], 2));

            //CF for the lateral dimension (x, y) and its derivative for D
            double plat3 = (p0t3 / sqrpi * pexpxt3 + perfxt3) * (p0t3 / sqrpi * pexpyt3 + perfyt3) /
                    (4 * Math.pow(ax * ay, 2) / fitobsvol);
            double dDplat3 = (1 / (sqrpi * p0t3)) * (dDpexpyt3 * x * (p0t3 / sqrpi * pexpxt3 + perfxt3) +
                    dDpexpxt3 * x * (p0t3 / sqrpi * pexpyt3 + perfyt3)) / (4 * Math.pow(ax * ay, 2) / fitobsvol);
            ...
            double pacf = (1 / pareq[0]) * ((1 - pareq[5] - pareq[7]) * acf1 + Math.pow(q2, 2) * pareq[5] * acf2 +
            ...
            double[] grad = new double[]{
                    ...
                    (1 / pareq[0]) * (pf1 * ((p0t / sqrpi * pexpyt + perfyt) * dvxperfxt) * pspim /
                            ...
                            pf3 * ((p0t3 / sqrpi * pexpyt3 + perfyt3) * dvxperfxt3) * pspim3 /
                                    (4 * Math.pow(ax * ay, 2) / fitobsvol)) * triplet,
            ...
            double[] gradret =
                    new double[num]; // return the gradients of the fit model in respect to the fit parameters
            num = 0;
            for (int i = 0; i < noparam; i++) {
                if (paramfit[i] == true) {
                    gradret[num] = grad[i];
        ...
        public double value(double x, double[] params) {
            ...
            // pareq[9] = Ftrip
            // pareq[10] = Dtrip
            //q2 and q3, the brightness of the second and third components are fixed parameters and have been globaly
            // defined; see prepareFit()
            // COMPONENT 1
            // help variables, which are dependent on time, to write the full function
            double p0t = Math.sqrt(4 * pareq[1] * x + Math.pow(s, 2));
            double p1xt = ax + rx - pareq[2] * x;
            ...
            double pexpxt2 = Math.exp(-Math.pow(p1xt2 / p0t2, 2)) + Math.exp(-Math.pow(p2xt2 / p0t2, 2)) -
                    2 * Math.exp(-Math.pow(p3xt2 / p0t2, 2));
            double perfxt2 =
                    p1xt * Erf.erf(p1xt2 / p0t2) + p2xt2 * Erf.erf(p2xt2 / p0t2) - 2 * p3xt2 * Erf.erf(p3xt2 / p0t2);
            double pexpyt2 = Math.exp(-Math.pow(p1yt2 / p0t2, 2)) + Math.exp(-Math.pow(p2yt2 / p0t2, 2)) -
                    2 * Math.exp(-Math.pow(p3yt2 / p0t2, 2));
            double perfyt2 =
                    p1yt2 * Erf.erf(p1yt2 / p0t2) + p2yt2 * Erf.erf(p2yt2 / p0t2) - 2 * p3yt2 * Erf.erf(p3yt2 / p0t2);
            ...
            double pexpxt3 = Math.exp(-Math.pow(p1xt3 / p0t3, 2)) + Math.exp(-Math.pow(p2xt3 / p0t3, 2)) -
                    2 * Math.exp(-Math.pow(p3xt3 / p0t3, 2));
            double perfxt3 =
                    p1xt * Erf.erf(p1xt3 / p0t3) + p2xt3 * Erf.erf(p2xt3 / p0t3) - 2 * p3xt3 * Erf.erf(p3xt3 / p0t3);
            double pexpyt3 = Math.exp(-Math.pow(p1yt3 / p0t3, 2)) + Math.exp(-Math.pow(p2yt3 / p0t3, 2)) -
                    2 * Math.exp(-Math.pow(p3yt3 / p0t3, 2));
            double perfyt3 =
                    p1yt3 * Erf.erf(p1yt3 / p0t3) + p2yt3 * Erf.erf(p2yt3 / p0t3) - 2 * p3yt3 * Erf.erf(p3yt3 / p0t3);
            ...
            return (1 / pareq[0]) * ((1 - pareq[5] - pareq[7]) * acf1 + Math.pow(q2, 2) * pareq[5] * acf2 +
                    Math.pow(q3, 2) * pareq[7] * acf3) /
                    Math.pow(1 - pareq[5] - pareq[7] + q2 * pareq[5] + q3 * pareq[7], 2) * triplet + pareq[4];
        }
    }
    ...
    public class averageFit extends AbstractCurveFitter {
        ...
        protected LeastSquaresProblem getProblem(Collection<WeightedObservedPoint> points) {
            ...
            final AbstractCurveFitter.TheoreticalValuesFunction model =
                    new AbstractCurveFitter.TheoreticalValuesFunction(fitfunction, points);

            return new LeastSquaresBuilder().maxEvaluations(fitMaxIterations)
        ...
        public Map doFit(String model) {
            ...
            // currentmodel;    //set the presently used model
            ...
            if (use2imp) { //DAN
                for (int i = 1; i < chanum; i++) {
                    WeightedObservedPoint point =
                            new WeightedObservedPoint(1 / varaveacf[2][i], lagtime[i], aveacf[2][i]);
                    points.add(point);
                }
            } else {
                for (int i = 1; i < chanum; i++) {
                    WeightedObservedPoint point =
                            new WeightedObservedPoint(1 / varaveacf[0][i], lagtime[i], aveacf[0][i]);
                    points.add(point);
                }
            }
            ...
            try {
                ...
                for (int i = 1; i < chanum; i++) {
                    chi2aveacf += Math.pow(resaveacf[i], 2) / (chanum - numfreefitpar -
                            1);    // calculate chi2 value; do not include the 0 lagtime channel which contains shot
                    // noise
                }
