...
void copyToJavaV2(JNIEnv *env, jshortArray outarr, int new_c, int size,
                  int frameSize, int transferFrame);
int getADnumber();
void setSettingsParameterA();
void setSettingsParameterB(); // Set EMgain, ADnumber, Vertical Shift Speed
                              // (usecs), Vertical Clock Amplitude, Horizontal
                              // Speed/Readout (Mhz), Pre-Amp Gains
                  ...
                  int frameSize, int transferFrame)
{
    ...
    {
        if (new_c + transferFrame <= imgobj.size_b_)
        { // more than enough space in 1D array
            env->SetShortArrayRegion(outarr, (new_c * frameSize), size,
                                     imgobj.pImageArrayBuf_);
        }
        else
        {
            int sizeFirstSection =
                (new_c + transferFrame - imgobj.size_b_) * frameSize;
            int sizeSeconSection =
                (transferFrame - sizeFirstSection) * frameSize;
            env->SetShortArrayRegion(outarr, (new_c * frameSize),
                                     sizeFirstSection, imgobj.pImageArrayBuf_);
            env->SetShortArrayRegion(outarr, 0, sizeSeconSection,
                                     imgobj.pImageArrayBuf_
                                         + (sizeFirstSection));
        }
    }
...
void setSettingsParameterA()
{
    ...
    {
        assert((imgobj.acquisitionMode != 1)
               && "error 393 does not run single capture mode under Crop Mode");
        // crop mode
        imgobj.errorValue =
            SetReadMode(4); // 0 Full Vertical Binning   1 Multi-Track   2
                            // Random-Track   3 Single-Track   4 Image
        assert((imgobj.errorValue == DRV_SUCCESS) && "error 21 SetReadMode");
        imgobj.errorValue = SetAcquisitionMode(
            imgobj.acquisitionMode); // 1 - single scan; 2 - accumulate; 3 -
                                     // kinetics; 4 - fast kinetics; 5 - run
                                     // till abort;
        assert((imgobj.errorValue == DRV_SUCCESS)
               && "error 22 SetAcquisitionMode");
        imgobj.errorValue = SetFrameTransferMode(imgobj.isFrameTransfer);
        assert((imgobj.errorValue == DRV_SUCCESS)
               && "error 23 SetFrameTransferMode");
        if (imgobj.CameraModel == 860)
        {
            imgobj.errorValue = SetIsolatedCropMode(
                imgobj.isCropMode, imgobj.cHeight, imgobj.cWidth, imgobj.vbin,
                imgobj.hbin); // only centralized
            if (imgobj.errorValue != DRV_SUCCESS)
            {
                std::cout << "error SetIsolatedCropMode: " << imgobj.errorValue
                          << std::endl;
            }

            assert((imgobj.errorValue == DRV_SUCCESS)
                   && "error 24 SetIsolatedCropModeEx");
        }
        else
        {
            imgobj.errorValue = SetIsolatedCropModeEx(
                imgobj.isCropMode, imgobj.cHeight, imgobj.cWidth, imgobj.vbin,
                imgobj.hbin, imgobj.cLeft, imgobj.cTop); // variable position
            if (imgobj.errorValue != DRV_SUCCESS)
            {
                std::cout << "error SetIsolatedCropModeEx: "
                          << imgobj.errorValue << std::endl;
            }
    ...
    else
    {
        if (imgobj.CameraModel == 860)
        {
            imgobj.errorValue = SetIsolatedCropMode(
                ...
                imgobj.hbin); // only centralized
            assert((imgobj.errorValue == DRV_SUCCESS)
                   && "error 24 turning off crop mode SetIsolatedCropModeEx");
        }
        else
        {
            imgobj.errorValue = SetIsolatedCropModeEx(
                ...
                imgobj.hbin, imgobj.cLeft, imgobj.cTop); // variable position
            assert((imgobj.errorValue == DRV_SUCCESS)
                   && "error 24 turning off crop modeSetIsolatedCropModeEx");
        }
        imgobj.errorValue =
            SetReadMode(4); // 0 Full Vertical Binning   1 Multi-Track   2
                            // Random-Track   3 Single-Track   4 Image
        ...
        assert((imgobj.errorValue == DRV_SUCCESS)
               && "error 23 SetFrameTransferMode");
        imgobj.errorValue = SetImage(imgobj.hbin, imgobj.vbin, imgobj.hstart,
                                     imgobj.hend, imgobj.vstart, imgobj.vend);
        if (imgobj.errorValue != DRV_SUCCESS)
        {
    ...
    JNIEnv *env, jclass cls)
{
    ...
    std::cout << aBuffer << "\n";
    imgobj.errorValue =
        Initialize(aBuffer); // Initialize driver in current directory
    if (imgobj.errorValue != DRV_SUCCESS)
    {
    ...
    if (imgobj.errorValue != DRV_SUCCESS)
    {
        std::cout << "Turning cooler on Error: " << imgobj.errorValue
                  << std::endl;
    }
                                                                ...
                                                                jint iscool)
{
    ...
    if (iscool == 1)
    {
        imgobj.errorValue = CoolerON();
        assert((imgobj.errorValue == DRV_SUCCESS)
               && "error 31 Turning on cooler");
    }
    else
    {
        imgobj.errorValue = CoolerOFF(); // Switch off cooler (if used)
        assert((imgobj.errorValue == DRV_SUCCESS)
               && "error 51 Turning cooler off");
    }
}
    ...
    JNIEnv *env, jclass cls, jint desiredtemp)
{
    ...
    if (imgobj.errorValue != DRV_SUCCESS)
    {
        std::cout << "Set temperature error: " << imgobj.errorValue
                  << std::endl;
    }
    ...
    JNIEnv *env, jclass cls)
{
    ...
    // 20037 temp not reached //20035 temp not stabilized //20034 temp off
    // //20036 temp stabilized
                                                                    ...
                                                                    jclass cls)
{
    ...
    imgobj.errorValue = CoolerOFF(); // Switch off cooler (if used)
    if (imgobj.errorValue != DRV_SUCCESS)
        std::cout << "Error switching cooler off: " << imgobj.errorValue
                  << std::endl;
    ...
    JNIEnv *env, jclass cls)
{
    ...
    if (imgobj.errorValue != DRV_SUCCESS)
    {
        ...
        assert((imgobj.errorValue != DRV_INIERROR) && "Unable to load
        'Detector.INI"); assert((imgobj.errorValue != DRV_COFERROR) && "Unable
        to load COF"); assert((imgobj.errorValue != DRV_FLEXERROR) && "Unable to
        load RBF"); assert((imgobj.errorValue != DRV_ERROR_ACK) && "Unable to
        communicate with card"); assert((imgobj.errorValue !=
        DRV_ERROR_FILELOAD) && "Unable to load COF and RBF files");
        assert((imgobj.errorValue != DRV_ERROR_PAGELOCK)
        && "Unable to acquire lock on requested memory");
        assert((imgobj.errorValue
        != DRV_USBERROR) && "Unable to detect USB device or not USB2.0");*/

        // return false;
    }

    /* I call ShutDown() directly from java instead, this is to allow copying
    serial number and camera series
...
Java_fiji_plugin_imaging_1fcs_directCameraReadout_andorsdk2v3_AndorSDK2v3_setParameterSingleSDK2(
    ...
    jint isCropMode, jint cWidth, jint cHeight, jint cLeft, jint cTop)
{
    ...
    // std::cout << "cpp SetParameter hbin: " << imgobj.hbin << ", vbin: " <<
    // imgobj.vbin << ", hstart: " << imgobj.hstart << ", hend: " << imgobj.hend
    // << ", vstart: " << imgobj.vstart << ", vend: " << imgobj.vend << ",
    // nopixelX: " << imgobj.nopixelX_ << ", nopixelY: " << imgobj.nopixelY_ <<
    // std::endl;

    setSettingsParameterA();

    setSettingsParameterB(); // set ADchannel, Vertical shift speed, Vertical
                             // amp, Horizontal speed, Preamp, emgain
    ...
    // set trigger mode
    int trigger_mode = 0; // 0 - internal; 1 - external; 6 - external start; 7 -
                          // external exposure (bulb); 9 - external FVB EM; 10 -
                          // software trigger; 12 - external charge shifting;
                                                                   ...
                                                                   jclass cls)
{
    ...
    */
    ...
    assert((imgobj.errorValue == DRV_SUCCESS)
           && "error 45 SingleScan StartAcquisition()");
    ...
    imgobj.errorValue =
        GetAcquiredData(imgobj.pImageArray1_, imgobj.arraysize_);
    if (imgobj.errorValue != DRV_SUCCESS)
    {
    ...
    assert((imgobj.errorValue == DRV_SUCCESS)
           && "error 46 SingleScan GetAcquiredData()");
    ...
    env->SetShortArrayRegion(outArray, 0, imgobj.arraysize_,
                             imgobj.pImageArrayBuf1_);
...
Java_fiji_plugin_imaging_1fcs_directCameraReadout_andorsdk2v3_AndorSDK2v3_setParameterInfiniteLoopSDK2(
    ...
    jint iVamp, jint iHspeed, jint iPreAmp, jint isCropMode, jint cWidth,
    jint cHeight, jint cLeft, jint cTop, jint arraysize)
{
    imgobj.CameraModel = cammodel;
    imgobj.isCropMode = isCropMode;
    imgobj.setExposureTime(exposureTime);
    ...
    assert(((imgobj.size_b_ % imgobj.frameTransferInterval_) == 0)
           && "error 26; total frame is not a multiple of frame inteval");
    ...
    */
    ...
    /*
        // return maximum number of images circular buffer can store based on
       current acquisiton setting long buffer; imgobj.errorValue =
       GetSizeOfCircularBuffer(&buffer); assert((imgobj.errorValue) && "error
       41; GetSizeOfCircularBuffer"); if (imgobj.errorValue != DRV_SUCCESS) {
                std::cout << "Get size of circular buffer Error \n";
                std::cout << "Error: " << imgobj.errorValue << "\n";
        }
        else {
                std::cout << "Number of images circular buffer can hold: " <<
       buffer << std::endl;
        }
        */
    ...
    JNIEnv *env, jclass cls, jshortArray outArray, jobject framecounterObj)
{
    ...
    int c = 0;
    while (!imgobj.GetIsStopPressed())
    { ////c != 100 Tmporary stop after total
      /// frame = 99+1 * frameinterval
        ...
        } while (totalImAcq < (c + 1) * imgobj.frameTransferInterval_
                     + imgobj.discaredZeroFramecounter);
        ...
        lastTRUE = first + imgobj.frameTransferInterval_
            - 1; // TODO: confirmed correct allocation by passing -1 into
                 // element in pImageArray
        ...
        /*
        // The following is necassary to discard first frames. Fix absurd number
        appearing on first frames of dimension larger than 64 x 64 pixels.
        UNUSED. if (i == 0) { first = 2; lastTRUE = 2 +
        imgobj.acquisitionFrameInterval_ - 1;
        }
        */
        ...
        {
            first_ref = c * imgobj.frameTransferInterval_ + 1
                + imgobj.discaredZeroFramecounter;
        ...
        imgobj.errorValue = GetImages(first, lastTRUE, imgobj.pImageArray_,
                                      sizee, &validfirst, &validlast);
        if (imgobj.errorValue != DRV_SUCCESS)
        {
            std::cout << "error 55 getimages: " << imgobj.errorValue
                      << std::endl;
        }
        assert((imgobj.errorValue == DRV_SUCCESS)
               && "error 55; GetImages error(camera buffer to cpp)");
      ...
      */
            ...
            tk3.setTimeStart();
            std::copy(imgobj.pImageArray_, imgobj.pImageArray_ + sizee,
                      imgobj.pImageArrayBuf_); // find out if faster
            if (!imgobj.isPointerNull())
            {
                env->SetShortArrayRegion(outArray, (new_c * sizee), sizee,
                                         imgobj.pImageArrayBuf_);
            }
        ...
        else
        {
            ...
            std::copy(imgobj.pImageArray_, imgobj.pImageArray_ + sizee,
                      imgobj.pImageArrayBuf_); // find out if faster
            std::copy(imgobj.pImageArrayBuf_, imgobj.pImageArrayBuf_ + sizee,
                      carray + (new_c * sizee));
            env->CallVoidMethod(framecounterObj, increment_id);
        }
    ...
    jint arraysize)
{
    ...
    */
    ...
    // return maximum number of images circular buffer can store based on
    current acquisiton setting long buffer; imgobj.errorValue =
    GetSizeOfCircularBuffer(&buffer); assert((imgobj.errorValue) && "error 41;
    GetSizeOfCircularBuffer"); if (imgobj.errorValue != DRV_SUCCESS) { std::cout
    << "Get size of circular buffer Error \n"; std::cout << "Error: " <<
    imgobj.errorValue << "\n";
    }
    else {
            std::cout << "Number of images circular buffer can hold: " << buffer
    << std::endl;
    }
    */
...
Java_fiji_plugin_imaging_1fcs_directCameraReadout_andorsdk2v3_AndorSDK2v3_setParameterInfiniteLoopV2SDK2(
    ...
    jint iPreAmp, jint isCropMode, jint cWidth, jint cHeight, jint cLeft,
    jint cTop)
{
    imgobj.CameraModel = cammodel;
    imgobj.isCropMode = isCropMode;
    ...
    JNIEnv *env, jclass cls, jshortArray outArray)
{
    ...
    while (last < imgobj.totalFrame_)
    {
        ...
        if (imgobj.errorValue != DRV_SUCCESS)
        {
            std::cout << "GetNumebrneImages error: " << imgobj.errorValue
                      << std::endl;
        }
        assert((imgobj.errorValue == DRV_SUCCESS)
               && "error 45 GetNumebrNewImages");
        ...
        // Andor get image buffer to long array
        // imgobj.errorValue = GetImages(first, last, imgobj.pImageArray_ +
        // (new_c * frameSize), size, &validFirst, &validLast);
        imgobj.errorValue = GetImages(first, last, imgobj.pImageArray_, size,
                                                                ...
                                                                jclass cls)
{
    ...
    result = (jobjectArray)env->NewObjectArray(
        3, env->FindClass("java/lang/String"), env->NewStringUTF(""));
    for (int i = 0; i < 3; i++)
    {
        env->SetObjectArrayElement(
            result, i, env->NewStringUTF(std::to_string(i).c_str()));
    }
    return result;
}
    ...
    JNIEnv *env, jclass cls)
{
    ...
    jobjectArray result;
    result = (jobjectArray)env->NewObjectArray(
        numberAvailVS, env->FindClass("java/lang/String"),
        env->NewStringUTF(""));
    for (int i = 0; i < numberAvailVS; i++)
    {
        env->SetObjectArrayElement(result, i,
                                   env->NewStringUTF(VSAmpArray[i].c_str()));
    }
    ...
    JNIEnv *env, jclass cls)
{
    ...
    jobjectArray result;
    result = (jobjectArray)env->NewObjectArray(
        numberAvailVSpeed, env->FindClass("java/lang/String"),
        env->NewStringUTF(""));
    for (int i = 0; i < numberAvailVSpeed; i++)
    {
        env->SetObjectArrayElement(result, i,
                                   env->NewStringUTF(VSpeedArray[i].c_str()));
    }
    ...
    JNIEnv *env, jclass cls)
{
    ...
    jobjectArray result;
    result = (jobjectArray)env->NewObjectArray(
        numberAvailHSpeed, env->FindClass("java/lang/String"),
        env->NewStringUTF(""));
    for (int i = 0; i < numberAvailHSpeed; i++)
    {
        env->SetObjectArrayElement(result, i,
                                   env->NewStringUTF(HSpeedArray[i].c_str()));
    }
...
{
    int noGains;
    int ihs = imgobj.iHspeedFastest; // index hspeed to cheack (iset this to
                                     // maximum horizontal speed just
    int status;
    float gain;
    ...
    jobjectArray result;
    result = (jobjectArray)env->NewObjectArray(
        noGains, env->FindClass("java/lang/String"), env->NewStringUTF(""));
    for (int i = 0; i < noGains; i++)
    {
        env->SetObjectArrayElement(result, i,
                                   env->NewStringUTF(GainsArray[i].c_str()));
    }
    ...
    jint binH, jint cropL, jint cropT)
{
    ...
    if (imgobj.errorValue != DRV_SUCCESS)
    {
        std::cout << "error SetFrameTransferMode: " << imgobj.errorValue
                  << std::endl;
    }

    imgobj.errorValue = SetIsolatedCropModeEx(isCropMode, cropH, cropW, binH,
                                              binW, cropL, cropT);
    if (imgobj.errorValue != DRV_SUCCESS)
    {
        std::cout << "error SetIsolatedCropModeEx: " << imgobj.errorValue
                  << std::endl;
    }
