...
While the information is accurate for slower acquisitions and most legacy CCD
cameras, these timestamps are still obtained from the operating systems. For
that reason, the timestamps may not always represent the time of actual,
in-camera acquisition. This is especially true for fast sCMOS cameras that
implement an internal frame buffer. Such cameras often report both the BOF and
EOF timestamps as identical numbers with the readout time reported as 0. For
accurate hardware timestamps please see @ref EmbeddedFrameMetadata feature.
*/
...
typedef enum PL_IMAGE_FORMATS
{
    ...
    PL_IMAGE_FORMAT_BAYER16, /**< 16bit bayer masked image, 2 bytes per pixel.
                                See also #PL_COLOR_MODES. */
    PL_IMAGE_FORMAT_MONO8, /**< 8bit mono, 1 byte per pixel. */
    PL_IMAGE_FORMAT_BAYER8, /**< 8bit bayer masked image, 1 byte per pixel. See
                               also #PL_COLOR_MODES. */
    PL_IMAGE_FORMAT_MONO24, /**< 24bit mono, 3 bytes per pixel. */
    PL_IMAGE_FORMAT_BAYER24, /**< 24bit bayer masked image, 3 bytes per pixel.
                                See also #PL_COLOR_MODES. */
    PL_IMAGE_FORMAT_RGB24, /**< 8bit RGB, 1 byte per sample, 3 bytes per pixel.
                            */
    PL_IMAGE_FORMAT_RGB48, /**< 16bit RGB, 2 bytes per sample, 6 bytes per
                            * pixel.
                            */
    PL_IMAGE_FORMAT_RGB72, /**< 24bit RGB, 3 bytes per sample, 9 bytes per
                            * pixel.
                            */
    PL_IMAGE_FORMAT_MONO32, /**< 32bit mono, 4 bytes per pixel. */
    PL_IMAGE_FORMAT_BAYER32, /**< 32bit bayer masked image, 4 bytes per pixel.
                                See also #PL_COLOR_MODES. */
    PL_IMAGE_FORMAT_RGB96 /**< 32bit RGB, 4 bytes per sample, 12 bytes per
                           * pixel.
                           */
} PL_IMAGE_FORMATS;
...
image is decompressed, the pixels can be accessed according to #PL_IMAGE_FORMATS
type. Used with the #PARAM_IMAGE_COMPRESSION parameter ID. Treated as @c #int32
type (but should not exceed a value of 255 due to
#md_frame_header.imageCompression field).
*/
typedef enum PL_IMAGE_COMPRESSIONS
{
    PL_IMAGE_COMPRESSION_NONE = 0, /**< No compression, the pixels can be
                                      accessed according to #PL_IMAGE_FORMATS */
...
/**
Frame summing formats. Used with the #PARAM_HOST_FRAME_SUMMING_FORMAT parameter
ID. Treated as @c #int32 type.
*/
...
typedef enum PL_PARAM_ATTRIBUTES
{
    ...
    */
    ...
    calculated. The first valid value is the value reported for the attribute
    #ATTR_MIN, the second value is the minimum value plus increment
    (#ATTR_INCREMENT), and so on up to the maximum value (#ATTR_MAX). The data
    type for this attribute is defined by #ATTR_TYPE.
    */
    ATTR_INCREMENT,
    /**
    Reports if the parameter with ID param_id can be written to and/or read from
    or in case it cannot be written to and/or read, it tells whether a feature
    exists. If the param_id can be either written to or read from, the next step
    is to determine its data type. The access types are enumerated:
...
typedef enum PL_IO_TYPE
{
    ...
    IO_TYPE_DAC /**< The value of the desired analog output written to the DAC
                   on this address.*/
} PL_IO_TYPE;
...
typedef enum PL_CLEAR_MODES
{
    ...
    */
    ...
    compatibility with existing applications, the 'auto' clearing mode is
    reported under the same value as the 'clear never' mode.
    */
...
typedef enum PL_EXPOSURE_MODES
{
    ...
    */
    ...
    Use #pl_set_param with #PARAM_EXP_TIME to set new exposure time before
    calling #pl_exp_start_seq.
    */
    VARIABLE_TIMED_MODE,
    /** @deprecated Not supported by any modern camera. */
    INT_STROBE_MODE,
    ...
    */
    ...
    /**
    Trigger controls the start and duration of each exposure. This mode is
    similar to the legacy #BULB_MODE. This value allows the exposure mode to be
    "ORed" with #PL_EXPOSE_OUT_MODES for #pl_exp_setup_seq and
    #pl_exp_setup_cont functions.
    */
    EXT_TRIG_LEVEL = (7 + 3) << 8,
    /**
    Exposure is triggered with software trigger using the #pl_exp_trigger call.
    Similarly to #EXT_TRIG_TRIG_FIRST, the trigger starts the first exposure
    only. Subsequent exposures are controlled by the camera internal timer. This
    value allows the exposure mode to be "ORed" with #PL_EXPOSE_OUT_MODES for
    #pl_exp_setup_seq and #pl_exp_setup_cont functions.
    */
    EXT_TRIG_SOFTWARE_FIRST = (7 + 4) << 8,
    /**
    Exposure is triggered with software trigger using the #pl_exp_trigger call.
    Similarly to #EXT_TRIG_EDGE_RISING, each call to #pl_exp_trigger triggers
    one exposure. This value allows the exposure mode to be "ORed" with
    #PL_EXPOSE_OUT_MODES for #pl_exp_setup_seq and #pl_exp_setup_cont functions.
    */
    ...
    */
    ...
    For N frames specified in the software, there must be N+1 trigger pulses in
    the series with individual exposure times being equal to the time interval
    between the trigger pulses. This value allows the exposure mode to be "ORed"
    with #PL_EXPOSE_OUT_MODES for #pl_exp_setup_seq and #pl_exp_setup_cont
    functions.
    */
...
{
    /**
    Expose Out high when first row is exposed (from first row begin to first row
    end)
    */
    EXPOSE_OUT_FIRST_ROW = 0,
    /**
    Expose Out high when all rows are exposed simultaneously (from last row
    begin to first row end). The duration of the signal equals the exposure
    value entered which means the actual exposure time is longer - use this mode
    with triggered light source only.
    */
    EXPOSE_OUT_ALL_ROWS,
    /**
    Expose Out high when any row is exposed (from first row begin to last row
    end)
    */
    EXPOSE_OUT_ANY_ROW,
    /**
    Similar to ALL_ROWS. Actual exposure duration matches the value entered, but
    the Expose Out is only high while all rows are exposing simultaneously. If
    the exposure time entered is shorter than the readout time, the Expose Out
    signal will remain at low level.
    */
    EXPOSE_OUT_ROLLING_SHUTTER,
    /**
    Expose Out signal pulses for every line readout as configured via the
    #PARAM_SCAN_MODE and related parameters. If #PARAM_SCAN_MODE is not
    available or enabled, the Expose Out behaves as #EXPOSE_OUT_ANY_ROW.
    */
    EXPOSE_OUT_LINE_TRIGGER,
    /**
    Similar to EXPOSE_OUT_ROLLING_SHUTTER but using sensor's global shutter
    mode.
    */
...
{
    PL_TRIGTAB_SIGNAL_EXPOSE_OUT /**< Control the expose out hardware signal
                                    multiplexing */
} PL_TRIGTAB_SIGNALS;
...
{
    /**
    Camera will sample and deliver the frames as fast as possible while
    considering the interface bandwidth. This mode may result in uneven frame
    intervals, but possibly higher frame rates.
    */
    PL_FRAME_DELIVERY_MODE_MAX_FPS = 0,
    /**
    Camera will sample and deliver the frames in constant intervals. In this
    mode the camera samples the sensor in constant time intervals but if the
    interface is busy and all frame buffers are full, the frame will be skipped
    and the sensor will be sampled in the next interval (unlike the MAX_FPS mode
    where the sensor would be sampled immediately once the buffer/interface is
    unclogged). This results in even frame intervals, but there might be gaps
    in between the frame deliveries and thus reduced frame rate.
    */
...
32-bit enum where:
- Upper 24 bits are interface classes, flags, 1bit = one class, 24 possible
classes.
- Lower 8 bits are interface revisions with 254 possible revisions per each
interface class.
...
typedef enum PL_CAM_INTERFACE_TYPES
{
    ...
    PL_CAM_IFC_TYPE_1394 =
        0x100, /**< A generic 1394 in case we cannot identify the sub type. */
    PL_CAM_IFC_TYPE_1394_A, /**< FireWire 400. */
    PL_CAM_IFC_TYPE_1394_B, /**< FireWire 800. */

    PL_CAM_IFC_TYPE_USB =
        0x200, /**< A generic USB in case we cannot identify the sub type. */
    ...
    PL_CAM_IFC_TYPE_VIRTUAL =
        0x1000, /**< Base for all Virtual camera interfaces. */

    PL_CAM_IFC_TYPE_ETHERNET =
        0x2000 /**< Base for all Ethernet-based cameras. */
...
typedef enum PL_SCAN_MODES
{
    ...
    reads each row in succession without inserting additional delays between
    rows
  */
    PL_SCAN_MODE_AUTO = 0,
...
typedef enum PP_PARAMETER_IDS
{
    PP_PARAMETER_RF_FUNCTION =
        (PP_FEATURE_RING_FUNCTION * PP_MAX_PARAMETERS_PER_FEATURE),
    ...
    PP_FEATURE_QUANT_VIEW_ENABLED =
        (PP_FEATURE_QUANT_VIEW * PP_MAX_PARAMETERS_PER_FEATURE),
    PP_FEATURE_QUANT_VIEW_E,
    PP_FEATURE_BLACK_LOCK_ENABLED =
        (PP_FEATURE_BLACK_LOCK * PP_MAX_PARAMETERS_PER_FEATURE),
    PP_FEATURE_BLACK_LOCK_BLACK_CLIP,
    PP_FEATURE_TOP_LOCK_ENABLED =
        (PP_FEATURE_TOP_LOCK * PP_MAX_PARAMETERS_PER_FEATURE),
    PP_FEATURE_TOP_LOCK_WHITE_CLIP,
    PP_FEATURE_VARI_BIT_ENABLED =
        (PP_FEATURE_VARI_BIT * PP_MAX_PARAMETERS_PER_FEATURE),
    ...
    PP_FEATURE_DENOISING_ENABLED =
        (PP_FEATURE_DENOISING * PP_MAX_PARAMETERS_PER_FEATURE),
    ...
    PP_FEATURE_LARGE_CLUSTER_CORRECTION_ENABLED =
        (PP_FEATURE_LARGE_CLUSTER_CORRECTION * PP_MAX_PARAMETERS_PER_FEATURE),
    PP_FEATURE_FRAME_AVERAGING_ENABLED =
        (PP_FEATURE_FRAME_AVERAGING * PP_MAX_PARAMETERS_PER_FEATURE),
...
{
    /** The system is @b idle, no data is expected. If any data arrives, it will
       be discarded. */
    READOUT_NOT_ACTIVE,
    /**
    The data collection routines are @b active. They are waiting for data to
    arrive, but none has arrived yet.
    */
    EXPOSURE_IN_PROGRESS,
    /** The data collection routines are @b active. The data has started to
       arrive. */
    READOUT_IN_PROGRESS,
    ...
    /** Something went wrong. The function returns a #PV_FAIL and #pl_error_code
       is set. */
    READOUT_FAILED,
...
The resolution defines units in which the exposure time is passed to
#pl_exp_setup_seq and #pl_exp_setup_cont calls and for the Variable Timed Mode
(#PARAM_EXP_TIME). Treated as @c #int32 type.
*/
typedef enum PL_EXP_RES_MODES
{
    EXP_RES_ONE_MILLISEC =
        0, /**< Exposure value is defined in milli-seconds. */
...
/**
Used with the functions pl_cam_register_callback* and
...
In most cases, the <tt>s1, s2</tt> coordinates correspond to <tt>x1, x2</tt>
coordinates. The sensor region width is then calculated as <tt>s2 - s1 + 1</tt>.
The resulting image width would be <tt>(s2 - s1 + 1) / sbin</tt>. Similarly, the
<tt>p1, p2</tt> coordinates correspond to <tt>y1, y2</tt> coordinates. The
sensor region height is then calculated as <tt>(p2 - p1 + 1)</tt>. The resulting
...
{
    uns16 shutter_close_delay; /**< Number of milliseconds for the shutter to
                                  close. */
    uns16
        shutter_open_delay; /**< Number of milliseconds for the shutter to open.
                             */
...
{ /* TOTAL: 48 bytes */
    ...
    /** The final exposure time = exposureTime * exposureTimeResNs
     * (nanoseconds).
     */
    uns32 exposureTime; /**< 4B - Depends on resolution. */
    uns32 exposureTimeResNs; /**< 4B - 1=1ns, 1000=1us, 5000000=5ms, ... */

    /** ROI timestamp resolution is stored here, no need to transfer with each
     * ROI. */
    ...
    uns8 imageCompression; /**< 1B - Image pixel data compression, see
                              #PL_IMAGE_COMPRESSIONS */
...
/**
Version 3 of the frame header with improved timestamp and exposure time
accuracy. When the md_frame_header.version is reported as 3, users are expected
to use reinterpret_cast to cast the md_frame.header pointer to
md_frame_header_v3 type.
*/
...
{ /* TOTAL: 48 bytes */
    ...
    ulong64
        timestampBOF; /**< 8B - Beginning of frame timestamp, in picoseconds. */
...
{ /* TOTAL: 32 bytes */
    ...
    uns16 extendedMdSize; /**< 2B - Must be 0 or actual ext metadata size in
                             bytes. */
...
{
    md_ext_item list[PL_MD_EXT_TAGS_MAX_SUPPORTED]; /**< List of extended
                                                       metadata tags */
    md_ext_item *
        map[PL_MD_EXT_TAGS_MAX_SUPPORTED]; /**< Map of extended metadata tags */
...
buffer. Once the buffer is deleted, the contents of the structure become
invalid.
*/
typedef struct md_frame_roi
{
    md_frame_roi_header
        *header; /**< Points directly to the header within the buffer. */
    void *data; /**< Points to the ROI image data. */
    uns32 dataSize; /**< Size of the ROI image data in bytes. */
    void *
        extMdData; /**< Points directly to ext. metadata data within the buffer.
                    */
...
/******************************************************************************/
/**
The following macros can be used to extract a single byte from multi-byte
integers, or to combine multiple bytes into a larger integer value.

@{
*/

/** Extracts the most significant byte from a two-uns8 integer input. */
#define MS16_BYTE(two_byte_value)                                              \
    ((uns8)(((uns16)(two_byte_value) >> 8) & 0xFF))
/** Extracts the least significant byte from a two-uns8 integer input. */
#define LS16_BYTE(two_byte_value)                                              \
    ((uns8)(((uns16)(two_byte_value) >> 0) & 0xFF))

/** Extracts the most significant byte from a four-uns8 integer input. */
#define MS32_BYTE(four_byte_value)                                             \
    ((uns8)(((uns32)(four_byte_value) >> 24) & 0xFF))
/** Extracts the middle-high significant byte from a four-uns8 integer input. */
#define MH32_BYTE(four_byte_value)                                             \
    ((uns8)(((uns32)(four_byte_value) >> 16) & 0xFF))
/** Extracts the middle-low significant byte from a four-uns8 integer input. */
#define ML32_BYTE(four_byte_value)                                             \
    ((uns8)(((uns32)(four_byte_value) >> 8) & 0xFF))
/** Extracts the least significant byte from a four-uns8 integer input. */
#define LS32_BYTE(four_byte_value)                                             \
    ((uns8)(((uns32)(four_byte_value) >> 0) & 0xFF))

/** Produces a 16-bit unsigned integer value from 2 input bytes. */
#define VAL_UNS16(ms_byte, ls_byte)                                            \
    (((uns16)((uns8)(ms_byte)) << 8) | ((uns16)((uns8)(ls_byte)) << 0))

/** Produces a 32-bit unsigned integer value from 4 input bytes. */
#define VAL_UNS32(ms_byte, mh_byte, ml_byte, ls_byte)                          \
    (((uns32)((uns8)(ms_byte)) << 24) | ((uns32)((uns8)(mh_byte)) << 16)       \
     | ((uns32)((uns8)(ml_byte)) << 8) | ((uns32)((uns8)(ls_byte)) << 0))

/** Produces a 64-bit signed integer value from 8 input bytes. */
#define VAL_INT64(b0, b1, b2, b3, b4, b5, b6, b7)                              \
    (((long64)((uns8)(b0)) << 56) | ((long64)((uns8)(b1)) << 48)               \
     | ((long64)((uns8)(b2)) << 40) | ((long64)((uns8)(b3)) << 32)             \
     | ((long64)((uns8)(b4)) << 24) | ((long64)((uns8)(b5)) << 16)             \
     | ((long64)((uns8)(b6)) << 8) | ((long64)((uns8)(b7)) << 0))
...
typedef int16 pm_script_hook(int16 hcam, uns16 exp_total, uns16 rgn_total,
                             const rgn_type *rgn_array, int16 mode,
                             uns32 exposure_time, uns32 *pixels,
...
Some devices have no message. In other words, they return a value of 0 for
bytes.
...
This for example returns whether the interface is fully capable of imaging or if
it has limitations. Use the #ATTR_CURRENT to retrieve the mode of the currently
...
(the conversion rate should be linear, but may differ from system to system),
but a lower offset voltage will yield a lower value for all output pixels.
Pixels brought below zero by this method will be clipped at zero. Pixels raised
above saturation will be clipped at saturation. Before you can change the offset
...
/**
@brief This is the size of the active area of a pixel, in the parallel
direction.
...
The full size of the parallel register is actually (par_size + premask +
postmask).
...
/**
@brief Returns @c TRUE for #ATTR_AVAIL if the camera has accumulation
capability.
...
This parameter is for Teledyne Photometrics internal use only. It is largely
unused because we simply do it on all cameras now, thus it can become deprecated
in future.
...
For pre-exposure clearing mode and the first frame in pre-sequence clearing
mode, the frame cycle time is the sum of #PARAM_EXPOSURE_TIME,
#PARAM_PRE_TRIGGER_DELAY, #PARAM_POST_TRIGGER_DELAY and #PARAM_CLEARING_TIME.
...
temperature is too high, it is difficult to get sufficient cooling on an
air-cooled camera.
...
The serial number for Teledyne Photometrics-branded cameras has a maximum length
of #MAX_ALPHA_SER_NUM_LEN.
...
Note that the camera can automatically adjust the fan speed to higher level due
to sensor overheating or completely shut down power to the sensor board to
protect camera from damage. The default fan speed is supposed to be changed only
...
scanning camera capabilities after opening (especially with Retiga R1/R3/R6
cameras). To work around this limitation ensure there is no communication with
camera one second before and one second after this parameter is used!
@note The availability is camera-dependent.
*/
...
/**
@brief *Native* pixel sample range in number of bits for the currently
configured acquisition.

The image bit depth may depend on currently selected #PARAM_READOUT_PORT,
#PARAM_SPDTAB_INDEX, #PARAM_GAIN_INDEX and other acquisition parameters. Please,
make sure to check this parameter before starting the acquisition, ideally after
calling #pl_exp_setup_seq or #pl_exp_setup_cont.

Most Teledyne Photometrics cameras transfer pixels in 16-bit words (#uns16
type). However, some cameras are capable of transferring 8-bit data (#uns8
type). Make sure to also check the #PARAM_IMAGE_FORMAT parameter to discover the
current camera pixel format.

This parameter indicates the number of valid bits within the transferred word or
byte.
...
This parameter differs from the *native* #PARAM_BIT_DEPTH in a way that it
reports the bit depth of the *output* frame - a frame that is delivered to the
*host*. Since PVCAM supports various *host* side post processing features, the
*host* bit depth may differ from the *native* camera bit depth, depending on
what *host*-side post processing features are active.

For example, the camera *native* bit depth may be reported as 12-bit for current
port/speed/gain selection. However, when #PARAM_HOST_FRAME_SUMMING_ENABLED is
enabled, the *host* frame bit depth may be reported as 16 or 32-bit.

As a general rule, the application should always rely on the '_HOST'-specific
parameters when identifying the output data format. The *native* parameters
should be used only for informational purposes, e.g. to show the camera native
format in the GUI.

Data type: @c #int16

@see @ref SpeedTable, #PARAM_HOST_FRAME_SUMMING_ENABLED,
#PARAM_HOST_FRAME_SUMMING_FORMAT.
*/
#define PARAM_BIT_DEPTH_HOST ((CLASS2 << 16) + (TYPE_INT16 << 24) + 551)
/**
@brief *Native* image format of the camera pixel stream.

This parameter is used to retrieve the list of camera-supported image formats.
The image format may depend on current camera configuration. It is advised to
check the image format before starting the acquisition. Some cameras allow the
native format to be selected. Use the #ATTR_ACCESS to check the write
permissions.
...
This parameter differs from the *native* #PARAM_IMAGE_FORMAT in a way that it
reports the format of the *output* frame - a frame that is delivered to the
*host*. Since PVCAM supports various *host* side post processing features, the
*host* image format may differ from the *native* camera bit depth, depending on
what *host*-side post processing features are active.

For example, the camera *native* image format may be reported as 16-bit MONO for
the current port/speed/gain selection. However, when
#PARAM_HOST_FRAME_SUMMING_ENABLED is enabled, the *host* image format may need
to be upgraded to a wider, 32-bit MONO format.
...
@see #PL_IMAGE_FORMATS enum for all possible values and related
#PARAM_HOST_FRAME_SUMMING_ENABLED, #PARAM_HOST_FRAME_SUMMING_FORMAT parameters.
*/
...
compression modes. The compression mode may depend on currently selected port
and speed combination. It is advised to check the compression mode before
starting the acquisition. Some cameras allow the native compression to be
selected. Use the #ATTR_ACCESS to check the write permissions.
...
This parameter differs from the *native* #PARAM_IMAGE_COMPRESSION in a way that
it reports the compression of the *output* frame - a frame that is delivered to
the *host*. For some camera models, various compression methods may be used to
increase the interface bandwidth.

In general, applications do not have to support any of the compression-related
parameters because decompression is automatically and seamlessly done by PVCAM.
However, in specific cases the automatic decompression may be disabled.
...
#PARAM_SCAN_WIDTH can be controlled and the #PARAM_SCAN_LINE_DELAY becomes
read-only. If the mode is set to #PL_SCAN_MODE_PROGRAMMABLE_LINE_DELAY, then the
#PARAM_SCAN_LINE_DELAY can be controlled and the #PARAM_SCAN_WIDTH becomes
read-only. In both cases the #PARAM_SCAN_LINE_TIME reports the total time it
takes to read one sensor line (row), including any delay added with the
parameters above.
...
This parameter is used to retrieve scan line time of camera. The time is
...
#PARAM_COLOR_MODE. For example, if a 90-degree rotation is requested, the
affected parameters will return appropriately swapped or rotated values.
Application should re-read the affected parameters after every change in
#PARAM_FRAME_ROTATE.

If #PARAM_METADATA_ENABLED is enabled, the frame-embedded metadata is also
adjusted automatically if needed. This applies to the metadata-reported ROI and
color mask.

Legacy applications can set a system environment variable
"PVCAM_FRAME_ROTATE_MODE" to one of the following values: 90, 180, 270 or 0. If
set, sensors of all PVCAM cameras in the system will become virtually rotated
and all sensor-related parameters will be reported appropriately rotated as
well.

A multi-threaded block-rotate algorithm is used for image rotation. The delay
introduced by the algorithm depends on CPU performance. An Intel Xeon W-2123
processor can rotate a 2048x2048 16-bit image in approximately 1.5ms.

This parameter can be used together with #PARAM_FRAME_FLIP. When both parameters
are enabled, PVCAM uses an algorithm that processes the rotation/flipping
combination using only one pass, keeping the performance withing 1-2ms on the
same CPU.
...
If #PARAM_METADATA_ENABLED is enabled, the frame-embedded metadata is also
adjusted automatically if needed. This applies to the metadata-reported color
mask value.

Legacy applications can set a system environment variable
"PVCAM_FRAME_FLIP_MODE" to one of the following values: X, Y, XY or NONE. If
set, sensors of all PVCAM cameras in the system will become virtually flipped
and all sensor-related parameters will be reported appropriately flipped as
well.

A multi-threaded algorithm is used for image flipping. The delay introduced
by the algorithm depends on CPU performance. An Intel Xeon W-2123 processor can
flip a 2048x2048 16-bit image in approximately 1ms.

This parameter can be used together with #PARAM_FRAME_ROTATE. When both
parameters are enabled, PVCAM uses an algorithm that processes the
rotation/flipping combination using only one pass, keeping the performance
withing 1-2ms on the same CPU.
...
/**
@brief This parameter is used to enable or disable the host frame summing
feature.

Use the #PARAM_HOST_FRAME_SUMMING_COUNT to set the number of frames to sum and
the #PARAM_HOST_FRAME_SUMMING_FORMAT to discover and set the desired frame
output format. After setting up the acquisition with #pl_exp_setup_seq or
#pl_exp_setup_cont, check the #PARAM_BIT_DEPTH_HOST and #PARAM_IMAGE_FORMAT_HOST
to discover the correct output frame format. This format may differ from the
native #PARAM_BIT_DEPTH and #PARAM_IMAGE_FORMAT.

By enabling this feature, PVCAM will start summing incoming frames and provide
an output frame for every N-th frame (defined by
#PARAM_HOST_FRAME_SUMMING_COUNT). Please note that when sequences are used
(#pl_exp_setup_seq), the number of frames in sequence may be limited. When
M-frames are requested in #pl_exp_setup_seq function and N-frames are configured
for summing, PVCAM has to internally acquire a total of MxN frames to deliver
correct amount of frames to the host. The camera hardware signals will also
correspond to the MxN count because the frame summing is done on the host side
without any knowledge of the camera.

Data type: @c #rs_bool

@see #PARAM_BIT_DEPTH_HOST, #PARAM_IMAGE_FORMAT_HOST
*/
#define PARAM_HOST_FRAME_SUMMING_ENABLED                                       \
    ((CLASS2 << 16) + (TYPE_BOOLEAN << 24) + 258)
/**
...
Data type: @c #uns32
*/
#define PARAM_HOST_FRAME_SUMMING_COUNT                                         \
    ((CLASS2 << 16) + (TYPE_UNS32 << 24) + 259)
/**
@brief This parameter is used to set the desired output format for the summed
frame.

When host frame summing feature is enabled, this parameter can be used to set
the desired output frame format. Depending on the average image intensity and
dynamic range requirements, the output format can be widened as needed. For
example, when summing 12-bit images, the output format may be set to 16-bit
width, allowing to sum up to 16 saturated images into one. When wider dynamic
range is required, the output format can be switched to 32-bit mode, allowing to
sum up to 1M 12-bit frames without reaching the 32-bit saturation level.

Data type: @c enum (@c #int32)

@see #PARAM_HOST_FRAME_SUMMING_ENABLED and #PL_FRAME_SUMMING_FORMATS for
details.
*/
#define PARAM_HOST_FRAME_SUMMING_FORMAT                                        \
    ((CLASS2 << 16) + (TYPE_ENUM << 24) + 260)
/**
@brief This parameter is used to enable or disable the automated frame
decompression feature.

In general, applications do not have to support any of the compression-related
parameters because decompression is automatically and seamlessly done by PVCAM.
However, in specific cases the automatic decompression may be disabled. In such
case, the application is expected to provide the frame decompression.

Please note that when this parameter is disabled for cameras that require frame
decompression, other *host* post processing parameters such as
#PARAM_HOST_FRAME_ROTATE, #PARAM_HOST_FRAME_FLIP,
#PARAM_HOST_FRAME_SUMMING_ENABLED and similar cannot be enabled because they
require uncompressed pixels to work properly. The #pl_exp_setup_seq and
#pl_exp_setup_cont functions will fail.

@see #PARAM_IMAGE_COMPRESSION, #PARAM_IMAGE_COMPRESSION_HOST

Data type: @c #rs_bool
*/
#define PARAM_HOST_FRAME_DECOMPRESSION_ENABLED                                 \
    ((CLASS2 << 16) + (TYPE_BOOLEAN << 24) + 261)
/**
...
This is the number of milliseconds required for the shutter to close. Since
physical shutters are no longer shipped or used with modern Teledyne
Photometrics cameras, this signal can be used for controlling other devices such
as light sources.
...
This is the number of milliseconds required for the shutter to open. Since
physical shutters are no longer shipped or used with modern Teledyne
Photometrics cameras, this signal can be used for controlling other devices such
as light sources.
...
This parameter is read-only and its value is always @c TRUE if it is available
on the camera.
...
/**
@brief This gets or sets the post-processing parameter for the currently
selected post-processing parameter at the index.
...
don't. It helps to identify similarities between camera post-processing
features.
...
@note The availability is camera-dependent.
*/
#define PARAM_SMART_STREAM_MODE_ENABLED                                        \
    ((CLASS2 << 16) + (TYPE_BOOLEAN << 24) + 700)
/**
...
@see @ref SmartStreaming
@note The availability is camera-dependent.
@note The parameter type is incorrectly defined. The actual type is
TYPE_SMART_STREAM_TYPE_PTR.
*/
#define PARAM_SMART_STREAM_EXP_PARAMS                                          \
    ((CLASS2 << 16) + (TYPE_VOID_PTR << 24) + 702)
/**
...
TYPE_SMART_STREAM_TYPE_PTR.
*/
#define PARAM_SMART_STREAM_DLY_PARAMS                                          \
    ((CLASS2 << 16) + (TYPE_VOID_PTR << 24) + 703)
...
known, which does not rule out exposure limits. In such case limits may be
specified e.g. in camera manual. Please note the reported value unit depends on
...
In normal operation, the timestamp is reset upon camera power up. Use this
parameter to reset the timestamp when needed. This is a write-only paremeter,
use #pl_set_param with @c TRUE value to reset the timestamp.
...
@note The availability is camera-dependent.
*/
#define PARAM_METADATA_RESET_TIMESTAMP                                         \
    ((CLASS3 << 16) + (TYPE_BOOLEAN << 24) + 176)
/**
...
/**
@brief This parameter is used to enable or disable the Centroids/Object
detection feature.
...
The radius defines the distance from the center pixel. For example, if the
camera reports the radius range between 1 and 5, it means that the resulting
ROIs can be configured to the following sizes: 1=3x3, 2=5x5, 3=7x7, 4=9x9,
5=11x11.

The parameter interpretation also depends on the selected Centroid/Object
detection mode. In some modes, the radius is used to set the size of the
...
The camera usually supports a limited number of Centroids/objects to be
detected. Use this parameter to limit the number of objects to be detected. For
example, in particle tracking mode, the parameter will limit number of particles
to be tracked
- if the camera finds more particles than the current limit, the remaining
...
dynamic background removal. The processing is optimized only for the selected
set of frames. Thus there is no related enumeration type with supported values
for this parameter.
...
/**
@brief Used to configure threshold multiplier for specific object detection
modes.

For 'particle tracking' mode, the value is a fixed-point real number in Q8.4
format. E.g. the value 1234 (0x4D2) means 77.2 (0x4D hex = 77 dec).
...
void MyCamera::initialize()
{
    ...
    pl_cam_register_callback_ex3(m_hCam, PL_CALLBACK_EOF,
(void*)pvcamCallbackEofEx3, this);
}
...
void PV_DECL MyCamera::pvcamCallbackEofEx3(const FRAME_INFO* pFrameInfo, void*
pContext)
{
...
@param[in]  pContext    A pointer to the user defined context that was
previously passed into the #pl_cam_register_callback_ex function.
*/
...
@param[in]  pFrameInfo  A pointer to a FRAME_INFO structure uniquely identifying
                        the incoming frame.
@param[in]  pContext    A pointer to the user defined context that was
previously passed into the #pl_cam_register_callback_ex3 function.
*/
typedef void(PV_DECL *PL_CALLBACK_SIG_EX3)(const FRAME_INFO *pFrameInfo,
                                           void *pContext);
...
#    endif
    ...
    */
    ...
    and trivial change 1. A major release is defined as anything that alters the
    interface, calling sequence, parameter list, or interpretation of any
    function in the library. This includes new functions and alterations to
    existing functions, but it does not include alterations to the optional
    libraries, which sit on top of PVCAM (each optional library includes its
    own, independent version number). A new major release often requires a
    change in the PVCAM library, but wherever possible, major releases are
    backwards compatible with earlier releases. A minor release should be
    completely transparent to higher-level software (PVCAM) but may include
    internal enhancements. The trivial version is reserved for use by the
    software staff to keep track of extremely minor variations. The last digit
    is used for build numbers and should be ignored. Minor and trivial releases
    should require no change in the calling software.

    @param[out] pvcam_version   Version number of PVCAM installer.

    @return #PV_OK for success, #PV_FAIL for failure. Failure sets
    #pl_error_code.
    ...
    */
    ...
    The PVCAM library requires significant system resources: memory, hardware
    access, etc. #pl_pvcam_init prepares these resources for use, as well as
    allocating whatever static memory the library needs. Until #pl_pvcam_init is
    called, every PVCAM function (except for the error reporting functions) will
    fail and return an error message that corresponds to <em>"library has not
    been initialized"</em>.

    @return #PV_OK for success, #PV_FAIL for failure. Failure sets
    #pl_error_code.

    @note If this call fails, #pl_error_code contains the code that lists the
    reason for failure.
    ...
    */
    ...
    It will also unlock and free memory, and clean up after itself as much as
    possible.
    ...
    */
    ...
    Second, it performs all cleanup, close-down, and shutdown preparations
    needed by the hardware. A camera can only be closed if it was previously
    opened.
    ...
    @note #pl_pvcam_uninit automatically calls the #pl_cam_close on all cameras
    ...
    */
    ...
    This function allows users to learn the string identifier associated with
    every camera on the current system. This is a companion to the
    #pl_cam_get_total function. cam_num input can run from 0 to (@c
    totl_cams-1), inclusive. The user must pass in a string that is at least
    #CAM_NAME_LEN characters long, #pl_cam_get_name then fills that string with
    an appropriate null-terminated string. @c camera_name can be passed directly
    into the #pl_cam_open function. It has no other use, aside from providing a
    ...
    @note This call reports the names of all cameras on the system, even if all
    the cameras are not available. If the hardware is turned off, or if another
    user has a camera open, the camera name is reported, but the camera will not
    be available. #pl_cam_get_name returns a name and #pl_cam_open gives
    information on availability of that camera. To build a complete list of
    every camera on the system, it is necessary to cycle through all the entries
    as shown below:
    @code{.cpp}
    int total_cameras;
    ...
    */
    ...
    All the listed cameras may not be available on multi-tasking systems, some
    cameras may already be in use by other users. A companion function
    #pl_cam_get_name can be used to get the string identifier associated with
    each camera.
    ...
    @note This function actually searches for all device drivers on the system,
    without checking hardware. The list of cameras is obtained during
    #pl_pvcam_init. Thus, if a new camera (new device driver) is added after the
    library was opened, the system won't know that the new camera is there. The
    system will also not notice if a camera is removed. (Obviously, this is only
    important on multi-tasking systems). A cycle of @c uninit / @c init
    regenerates the list of available cameras, updating the system for any
    ...
    */
    ...
    The string @a camera_name should be identical to one of the valid camera
    names returned by #pl_cam_get_name. If the name is valid, #pl_cam_open
    completes a short set of checks and diagnostics as it attempts to establish
    communication with the camera electronics unit. If successful, the camera is
    opened and a valid camera handle is passed back in hcam. Otherwise,
    #pl_cam_open returns with a failure. An explanation is shown in
    #pl_error_code.

    The @a o_mode setting controls the mode under which the camera is opened.
    Currently, the only possible choice is #OPEN_EXCLUSIVE. On multi-user
    systems, opening a camera under the exclusive mode reserves it for the
    current user, locking out all other users on the system. If #pl_cam_open is
    successful, the user has exclusive access to that camera until the camera is
    closed or #pl_pvcam_uninit is called.

    @param[in]  camera_name Text name assigned to a camera (via legacy RSConfig
    tool if available).
    @param[out] hcam        Camera handle returned from #pl_cam_open.
    @param[in]  o_mode      Mode to open the camera in (must be
    #OPEN_EXCLUSIVE).

    @return #PV_OK for success, #PV_FAIL for failure. Failure sets
    #pl_error_code.

    @see pl_cam_get_name, pl_cam_get_total, pl_cam_close, pl_pvcam_init,
    pl_pvcam_uninit

    @warning Despite the above paragraph, a successful #pl_cam_open does not
    mean that the camera is in working order. It does mean that you can
    communicate with the device driver associated with the camera. After a
    successful #pl_cam_open, call #pl_error_message, which reports any error
    conditions.
    */
    ...
    @{
    */
    DEPRECATED rs_bool PV_DECL pl_cam_register_callback(int16 hcam,
                                                        int32 callback_event,
                                                        void *callback);
    DEPRECATED rs_bool PV_DECL pl_cam_register_callback_ex(int16 hcam,
                                                           int32 callback_event,
                                                           void *callback,
                                                           void *context);
    DEPRECATED rs_bool PV_DECL pl_cam_register_callback_ex2(
        int16 hcam, int32 callback_event, void *callback);
    /** @} */ /* grp_pm_deprecated_functions */
    ...
    */
    ...
    /**
    @brief Installs a handler function that will be called when an event occurs
    in a camera providing information about frame via #FRAME_INFO type and with
    user context information.

    This function combines functionality provided by
    #pl_cam_register_callback_ex and #pl_cam_register_callback_ex2.

    Use this API call to install a function that will be called when the
    specified event occurs providing additional frame information. Input
    parameter of the callback function must be of #FRAME_INFO* type in order to
    receive information about the frame (timestamp with precision of 0.1ms,
    frame counter number, ID (handle) of the camera that produced the frame).
    Also, pointer to a context that will be echoed back when the callback is
    invoked can be passed to PVCAM in this function.
    ...
    @param[in]   hcam           Camera handle returned from #pl_cam_open.
    @param[in]   callback_event Callback event to register for (see
    #PL_CALLBACK_EVENT).
    ...
    after all frames are read out and before new exposure is started by calling
    #pl_exp_start_seq. Not all callbacks will be available for all camera
    systems/interfaces.

    @note Only #PL_CALLBACK_BOF and #PL_CALLBACK_EOF are fully supported by all
    camera types. Do not use other callback types in generic-purpose software.
    */
    ...
    Use this API call to uninstall a function for the specified camera system
    event.
    ...
    @see pl_cam_register_callback, pl_cam_register_callback_ex,
    pl_cam_register_callback_ex2, pl_cam_register_callback_ex3.

    @note Only #PL_CALLBACK_BOF and #PL_CALLBACK_EOF are fully supported by all
    camera types. Do not use other callback types in generic-purpose software.
    See callback descriptions section under #pl_cam_register_callback_ex3 for
    details.
    */
    ...
    If an error occurs later in the function, the error code is set to a
    corresponding value.
    ...
    */
    ...
    This function fills in the character string msg with a message that
    corresponds to the value in @a err_code. The msg string is allocated by the
    user, and should be at least #ERROR_MSG_LEN elements long.
    ...
    and #PV_FAIL if the code does not have a corresponding message
    ...
    All error messages are standalone sentences without ending period having
    error code name appended in parenthesis.
    */
    ...
    @a param_value points to the value of the requested attribute for the
    parameter. It is a @c void* because it can be of different data types. The
    user is responsible for passing in the correct data type (see attribute
    ...
    Reading of values for attributes #ATTR_AVAIL, #ATTR_ACCESS and #ATTR_TYPE
    should always succeed and return correct value. Values of other attributes
    can be read only if #ATTR_ACCESS reports either #ACC_READ_ONLY or
    #ACC_READ_WRITE.
    ...
    @note The data type of @a param_value is documented in pvcam.h for each
    param_id.
    */
    ...
    @a param_id is an enumerated type that indicates the parameter in question.
    @a param_value points to the new value of the parameter. For the enumerated
    type, this value is the value assigned to current enum item, not the item
    index.
    ...
    @note The data type of @a param_value is documented in pvcam.h for each @a
    param_id. It can be retrieved using the #pl_get_param function and
    #ATTR_TYPE attribute. The user should call the #pl_get_param function with
    the attribute #ATTR_ACCESS to verify that the parameter ID is writable
    (settable) before calling the #pl_set_param function.
    */
    ...
    This function will return the enumerated value of the parameter param_id at
    index. It also returns a string associated with the enumerated type (desc).
    @b length indicates the maximum length allowed for the returned description.

    @param[in]  hcam        Camera handle returned from #pl_cam_open.
    @param[in]  param_id    ID of the parameter to get or set (PARAM_...).
    @param[in]  index       Index of enumeration Range: 0 through N-1 ... where
    N is retrieved with get_param(...,#ATTR_COUNT,...).
    ...
    @note The user should call the #pl_get_param function with the attribute
    #ATTR_TYPE, to verify that the parameter ID is an enumerated data type
    before calling the #pl_get_enum_param. The user should also call the
    #pl_get_param function with the attribute #ATTR_COUNT to determine how many
    valid enumerated values the parameter ID has. Example: Suppose there is a
    parameter for camera readout speed. This parameter can be set to 1MHz, 5MHz
    or 10MHz with the appropriate values 1, 5 and 10. If the readout speed is
    currently set to 5MHz, a call to #pl_get_param with #ATTR_CURRENT returns a
    value of 5. A call to #pl_get_enum_param for the readout speed parameter at
    index 1 (the second item) returns the enumerated type 5MHz with the value
    equal to 5 and the desc would contain "5MHz".
    */
    rs_bool PV_DECL pl_get_enum_param(int16 hcam, uns32 param_id, uns32 index,
                                      int32 *value, char *desc, uns32 length);

    /**
    @brief Returns the length of the descriptive string for the parameter @a
    param_id at index.

    This function will return the length (length) of the descriptive string for
    the parameter
    @a param_id at index. The length includes the terminating null ('\0')
    character.
    ...
    @note This function can be used to determine the amount of memory to
    allocate for the descriptive string when calling the #pl_get_enum_param
    function. Using the example in #pl_get_enum_param, the length returned would
    be 5 (4 printable characters plus 1 null character).
    */
    rs_bool PV_DECL pl_enum_str_length(int16 hcam, uns32 param_id, uns32 index,
                                       uns32 *length);

    /**
    @brief This function will reset all post-processing modules to their default
    values.

    @param[in]  hcam    Camera handle returned from #pl_cam_open.

    @warning Fails if post-processing modules are not available in the current
    camera or if @a hcam is not the handle of an open camera.

    @return #PV_OK for success, #PV_FAIL for failure. Failure sets
    #pl_error_code.

    @see PARAM_PP_FEAT_NAME, PARAM_PP_PARAM_INDEX, PARAM_PP_PARAM_NAME,
    PARAM_PP_PARAM, PARAM_PP_FEAT_ID, PARAM_PP_PARAM_ID
    */
    rs_bool PV_DECL pl_pp_reset(int16 hcam);

    /**
    @brief Creates and allocates variable of smart_stream_type type with the
    number of entries passed in via the entries parameter and returns pointer to
    it.

    This function will create a variable of smart_stream_type type and return a
    pointer to access it. The entries parameter passed by the user determines
    how many entries the structure will contain.
    ...
    */
    ...
    /**
    @brief Frees the space previously allocated by the
    #pl_create_smart_stream_struct function.
    ...
    */
    ...
    /**
    @brief Creates and allocates variable of #FRAME_INFO type and returns
    pointer to it.

    This function will create a variable of #FRAME_INFO type and
    return a pointer to access it. The GUID field of the #FRAME_INFO structure
    is assigned by this function. Other fields are updated by PVCAM at the time
    of frame reception.
    ...
    @see pl_release_frame_info_struct, pl_exp_get_latest_frame_ex,
    pl_exp_get_oldest_frame_ex, pl_exp_check_cont_status_ex,
    pl_cam_register_callback_ex2, pl_cam_register_callback_ex3
    */
    ...
    This function will deallocate #FRAME_INFO variable created by
    ...
    @see pl_create_frame_info_struct, pl_exp_get_latest_frame_ex,
    pl_exp_get_oldest_frame_ex, pl_exp_check_cont_status_ex,
    pl_cam_register_callback_ex2, pl_cam_register_callback_ex3
    */
    ...
    The pointer @a rgn_array points to @a rgn_total region definitions, @a
    exp_mode specifies the bitwise OR combination of exposure mode and expose
    out mode (see chapter Extended Exposure Modes), @a exposure_time specifies
    the exposure time in the currently selected exposure time resolution (see
    #PARAM_EXP_RES and #PARAM_EXP_RES_INDEX). The pointer @a exp_bytes points to
    a variable that will be filled with number of bytes in the pixel stream. The
    settings are then uploaded to the camera. If there is any problem
    (overlapping regions or a frame-transfer setting for a camera that lacks
    that capability), this function aborts and returns with a failure.
    #pl_error_code indicates the definition problem. The @a exp_bytes pointer is
    filled with the number of bytes of memory needed to buffer the full
    sequence. (It is the developer's responsibility to allocate a memory buffer
    for the pixel stream.) When this function returns, the camera is ready to
    begin the exposure. #pl_exp_start_seq initiates exposure and readout.
    ...
    @param[in]  exp_mode        Mode for capture (an OR-ed combination of
    #PL_EXPOSURE_MODES and #PL_EXPOSE_OUT_MODES, if applicable).
    @param[in]  exposure_time   Time to expose in selected exposure resolution.
                                Default is milliseconds (see #PARAM_EXP_RES).
    @param[out] exp_bytes       Value returned from PVCAM specifying the
    required number of bytes to allocate for the capture.
    ...
    @note This function uploads new settings to the camera. After receiving the
    settings, the camera merely waits in an idle state. The #pl_exp_abort
    command may be used to place the camera into some other state, such as
    continuous clearing, but this will not alter or affect the uploaded
    settings. Essentially, the camera is still holding the exposure sequence and
    waiting to start, while it clears the sensor charge. Also, please note that
    PVCAM internally checks whether configuration needs to be sent to the
    camera. If there is no difference in the camera configuration from the
    previous "setup" and other acquisition parameters have not changed in the
    meantime, the PVCAM will not resend the configuration. This may lead to
    shorter execution times of this function and faster start of the subsequent
    acquisition.
    */
    ...
    into the camera. After that, #pl_exp_start_seq may be called one or more
    times. Each time it is called, it starts one sequence and returns
    immediately (a sequence may be one or more exposures). Progress can be
    monitored through #pl_exp_check_status. The next sequence may be started as
    soon as the readout has finished or an abort has been performed
    (#pl_exp_abort).

    The @a hcam parameter defines which camera is used.
    The user must allocate an appropriately sized memory buffer for data
    collection, pointed to by @a pixel_stream. This buffer must be at least @c
    exp_bytes bytes, where @c exp_bytes is the value returned from
    #pl_exp_setup_seq.
    ...
    */
    ...
    This function uses the array of regions, exposure mode, exposure time passed
    in, and circular buffer mode and transmits them to the camera. The pointer
    @a rgn_array points to @a rgn_total region definitions.
    @a exp_mode specifies the bitwise OR combination of the exposure mode and
    expose out mode.
    @a exposure_time specifies the exposure time in the currently selected
    exposure time resolution (see #PARAM_EXP_RES and #PARAM_EXP_RES_INDEX). The
    pointer @a exp_bytes points to a variable that will be filled with number of
    bytes in the pixel stream.
    @a buffer_mode can be set to either #CIRC_OVERWRITE or #CIRC_NO_OVERWRITE.
    This function must be called before calling #pl_exp_start_cont.
    ...
    sequence. (It is the developer's responsibility to allocate a memory buffer
    for the pixel stream.) When this function returns, the camera is ready to
    begin the exposure. #pl_exp_start_cont initiates exposure and readout.
    ...
    #PL_EXPOSURE_MODES and #PL_EXPOSE_OUT_MODES, if applicable).
    @param[in]  exposure_time   Time to expose in selected exposure resolution
    units. Default is milliseconds (see #PARAM_EXP_RES).
    @param[out] exp_bytes       Value returned from PVCAM specifying the
    required number of bytes to allocate for the capture.
    @param[in]  buffer_mode     Circular buffer mode (#CIRC_OVERWRITE,
    #CIRC_NO_OVERWRITE).

    @return #PV_OK for success, #PV_FAIL for failure. Failure sets
    #pl_error_code.

    @see pl_exp_start_cont, pl_exp_check_cont_status, pl_exp_get_oldest_frame,
    pl_exp_get_latest_frame, pl_exp_unlock_oldest_frame, pl_exp_stop_cont

    @note Use the parameter ID #PARAM_CIRC_BUFFER with #pl_get_param to see if
    the system can perform circular buffer operations. The circular buffer is
    ...
    */
    ...
    This function will initiate a continuous readout from the camera into a
    circular buffer.
    ...
    @note If @a pixel_stream points to a buffer that is not an integer-multiple
    of the frame size for the exposure, this function will return #PV_FAIL and
    set an appropriate error code in #pl_error_code. For example, a buffer size
    of 1000 bytes with a frame size of 250 bytes is OK, but a buffer size of 900
    bytes would cause a failure. Use the parameter ID #PARAM_CIRC_BUFFER with
    #pl_get_param to check to see if the system can perform circular buffer
    operations.
    */
    rs_bool PV_DECL pl_exp_start_cont(int16 hcam, void *pixel_stream,
                                      uns32 size);
    ...
    In order to use the software trigger, following preconditions must be met:
     - Acquisition has been configured via a call to #pl_exp_setup_seq or
    #pl_exp_setup_cont using one of the software triggering modes (@see
    PL_EXPOSURE_MODES).
    ...
    @remarks In current implementation the input flags should be set to 0. On
    output, the flags will contain one of the values defined in
    #PL_SW_TRIG_STATUSES enumeration.

    @see pl_exp_setup_seq, pl_exp_start_seq, pl_exp_setup_cont,
    pl_exp_start_cont, pl_exp_check_cont_status, pl_exp_get_oldest_frame,
    pl_exp_get_latest_frame, pl_exp_unlock_oldest_frame, pl_exp_abort
    */
    ...
    This is only useful when data collection has been set up and started, as
    with a call to the functions #pl_exp_setup_seq and #pl_exp_start_seq. In
    general, these functions start an exposure and then immediately return,
    allowing the progress to be monitored. The @a status gives a quick
    evaluation of progress. The argument @a status returns one of the values
    listed here: #PL_IMAGE_STATUSES.

    More detailed information is returned in @a bytes_arrived. This reports on
    exactly how many bytes of data have arrived so far. This level of feedback
    is unimportant to many users.

    @param[in]  hcam          Camera handle returned from #pl_cam_open.
    @param[out] status        Status of the current capture
    (#EXPOSURE_IN_PROGRESS,
    ...).
    @param[out] bytes_arrived Number of bytes that have arrived. For continuous
                              mode, this is the number of bytes that have
    arrived this time through the buffer.
    ...
    */
    rs_bool PV_DECL pl_exp_check_status(int16 hcam, int16 *status,
                                        uns32 *bytes_arrived);

    /**
    @brief Checks the continuous readout status from the camera into the
    circular buffer.
    ...
    @param[out]  status         Status of the current capture
    (#EXPOSURE_IN_PROGRESS,...).
    @param[out]  bytes_arrived  Number of bytes that have arrived. For
    continuous mode, this is the number of bytes that have arrived this time
    through the buffer.
    @param[out]  buffer_cnt     Number of times through the buffer (continuous
    mode).

    @return #PV_OK for success, #PV_FAIL for failure. Failure sets
    #pl_error_code.

    @see pl_exp_setup_cont, pl_exp_start_cont, pl_exp_get_oldest frame,
    pl_exp_get_latest_frame, pl_exp_unlock_oldest_frame, pl_exp_stop_cont

    @note This function only returns meaningful results if a continuous readout
    from the camera has been initiated by a call to #pl_exp_start_cont. Use the
    parameter ID #PARAM_CIRC_BUFFER with #pl_get_param to see if the system can
    ...
    */
    ...
    Values in the variable pointed to by frame_info will be updated with frame
    counters, timestamps (with precision of 0.1ms) and readout time information
    assigned by device driver at the moment of frame reception.
    ...
    @see pl_exp_setup_cont, pl_exp_start_cont, pl_exp_get_oldest_frame,
    pl_exp_get_latest_frame, pl_exp_unlock_oldest_frame, pl_exp_stop_cont,
    pl_create_frame_info_struct, pl_exp_get_latest_frame_ex,
    pl_exp_get_oldest_frame_ex

    @note This function only returns meaningful results if a continuous readout
    from the camera has been initiated by a call to #pl_exp_start_cont. Use the
    parameter ID #PARAM_CIRC_BUFFER with #pl_get_param to see if the system can
    perform circular buffer operations. Variable pointed to by frame_info must
    be created with #pl_create_frame_info_struct.
    */
    ...
    /**
    @brief This function returns a pointer to the most recently acquired frame
    ...
    @note If the camera in use is not able to return the latest frame for the
    current operating mode, this function will fail. For example, some cameras
    cannot return the latest frame in #CIRC_NO_OVERWRITE mode. Use the parameter
    ID #PARAM_CIRC_BUFFER with #pl_get_param to see if the system can perform
    ...
    */
    ...
    Additionally, this function updates values of timestamps (with precision of
    0.1ms), frame counter numbers and readout time in variable of #FRAME_INFO
    type.
    ...
    @see pl_exp_setup_cont, pl_exp_start_cont, pl_exp_check_cont_status,
    pl_exp_stop_cont, pl_exp_get_oldest_frame_ex, pl_exp_check_cont_status_ex,
    pl_cam_register_callback_ex2, pl_create_frame_info_struct,
    ...
    cannot return the latest frame in #CIRC_NO_OVERWRITE mode. Use the parameter
    ID #PARAM_CIRC_BUFFER with #pl_get_param to see if the system can perform
    circular buffer operations. Variable pointed to by frame_info must be
    created with #pl_create_frame_info_struct.
    */
    rs_bool PV_DECL pl_exp_get_latest_frame_ex(int16 hcam, void **frame,
                                               FRAME_INFO *frame_info);

    /**
    @brief This function returns pointer to the oldest unretrieved frame in the
    circular buffer.

    After calling this function, #pl_exp_unlock_oldest_frame has to be called to
    notify PVCAM that the oldest frame can be overwritten with new data.\n This
    function should be used with acquisitions running in #CIRC_NO_OVERWRITE
    mode.
    ...
    @note If the camera in use is not able to return the oldest frame for the
    current operating mode, this function will fail. For example, some cameras
    cannot return the oldest frame in #CIRC_OVERWRITE mode. Use the parameter ID
    #PARAM_CIRC_BUFFER with #pl_get_param to see if the system can perform
    circular buffer operations.
    */
    ...
    After calling this function, #pl_exp_unlock_oldest_frame has to be called to
    notify PVCAM that the oldest frame can be overwritten with new data.\n This
    function should be used in acquisitions running in #CIRC_NO_OVERWRITE mode.

    Additionally, this function updates the values in the variable pointed to by
    @a frame_info with the data collected at the time of frame reception by the
    device driver (e.g. frame counter value).
    @param[in]  hcam        Camera handle returned from #pl_cam_open.
    @param[out] frame       Pointer to the oldest unretrieved frame.
    @param[out] frame_info  Frame info struct pointer.
    ...
    */
    ...
    This function allows PVCAM to overwrite the oldest frame in the circular
    buffer.
    ...
    @see pl_exp_setup_cont, pl_exp_start_cont, pl_exp_check_cont_status,
    pl_exp_get_oldest_frame, pl_exp_unlock_oldest_frame, pl_exp_stop_cont

    @note Failure to call this function after using the frame will cause the
    continuous acquisition progress to halt eventually, because the frame cannot
    be overwritten when it is locked. Use the parameter ID #PARAM_CIRC_BUFFER
    with #pl_get_param to check to see if the system can perform circular buffer
    operations.
    */
    ...
    This function halts a continuous readout acquisition into the circular
    buffer.
    @a cam_state defines the new state of the Camera Control Subsystem,
    as described in the documentation for the #pl_exp_abort function. To
    simplify the code, #pl_exp_abort may be used instead of #pl_exp_stop_cont to
    stop both continuous and sequential acquisitions.
    ...
    @see pl_exp_setup_cont, pl_exp_start_cont, pl_exp_check_cont_status,
    pl_exp_get_oldest_frame, pl_exp_get_latest_frame, pl_exp_unlock_oldest_frame

    @note Use the parameter ID #PARAM_CIRC_BUFFER with #pl_get_param to see if
    the system can perform circular buffer operations.
    */
    ...
    #pl_exp_abort performs two functions: it stops the host device driver, and
    it may halt the camera
    (@a hcam specifies which camera and which device driver are being used.)
    Halting the camera halts
    @a readout, @c clearing, and all other camera activity. On the host side,
    data collection is controlled by a device driver. If data collection is
    currently enabled (the image data @b active state), this function stops
    collection, returns the low-level communication hardware and software to an
    image data @b idle state, and disables collection. In the @b idle state, any
    data that arrives is ignored and discarded. The @b idle state is the normal
    system default. On the camera side, the Camera Control Subsystem (CCS) may
    be in the process of collecting data, or it may be in one of several idle
    states. This function always stops the data collection software. In
    addition, it has the option of forcing the CCS into a new state by setting
    the @a cam_state variable to one of the following constants, which are
    camera-dependent: #PL_CCS_ABORT_MODES.
    ...
    @note This may also be called outside of an exposure. It can explicitly open
    the shutter, close the shutter, or stop the CCS. In the @b idle state, the
    system takes the least possible amount of action when image data arrives. On
    some systems, this involves placing the hardware in reset state, so it is
    inactive. On SCSI systems, the driver does not initiate any data transfers,
    although a buffer on the camera end may be filling up. If the CCS is halted
    and the shutter is closed (#CCS_HALT_CLOSE_SHTR), the current image remains
    on the sensor (although dark charge continues to accumulate). If @c
    clear_cycles is zero or the clear mode is #CLEAR_NEVER, the image may be
    read off by performing a bias readout. In frame transfer mode, you may not
    want to close the shutter when halting the CCS. Some frame transfer systems
    do not include a shutter, in which case an attempt to open or close the
    shutter is ignored, but does not cause an error.
    */
    ...
    This cleans up after an exposure started through #pl_exp_start_seq has
    finished readout. If the exposure has not finished readout, this function
    returns with an error. Argument @a hbuf is not used at the moment.
    ...
    (using #pl_exp_setup_seq and #pl_exp_start_seq) with callback notification
    after a frame is read out and before new exposure is started by calling
    #pl_exp_start_seq.
    */
    rs_bool PV_DECL pl_exp_finish_seq(int16 hcam, void *pixel_stream,
                                      int16 hbuf);

    /**
    @brief Defines control of an I/O line from within a camera script.

    This function allows the application program to define control of the
    available I/O lines from within a script. This allows for more precise
    control of external devices. For example, the application could request that
    a linear stage be indexed immediately after integration, instead of waiting
    until after the data is read out, the shutter is closed, etc.

    @a state has different meanings depending on the I/O type which are
    described here: #PL_IO_TYPE.
    ...
    @param[in]  location    Specifies when to control the I/O
    (#SCR_PRE_FLASH,...). Values are listed under #PL_IO_TYPE.
    ...
    */
    ...
    /**
    @brief Clears the current setup for control of the available I/O lines
    within a camera script.

    This function allows the application program to clear the current setup for
    control of the available I/O lines within the script. This allows the user
    to enter a new setup for these lines.
    ...
    */
    ...
    DEPRECATED rs_bool PV_DECL pl_dd_get_info(int16 hcam, int16 bytes,
                                              char *text);
    ...
    DEPRECATED rs_bool PV_DECL pl_dd_get_retries(int16 hcam,
                                                 uns16 *max_retries);
    /** Use #PARAM_DD_RETRIES instead. */
    ...
    DEPRECATED rs_bool PV_DECL pl_ccd_get_chip_name(int16 hcam,
                                                    char *chip_name);
    /** Use #PARAM_CLEAR_CYCLES instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_clear_cycles(int16 hcam,
                                                       uns16 *clear_cycles);
    /** Use #PARAM_CLEAR_CYCLES instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_set_clear_cycles(int16 hcam,
                                                       uns16 clr_cycles);
    /** Use #PARAM_CLEAR_MODE instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_clear_mode(int16 hcam,
                                                     int16 *clear_mode);
    /** Use #PARAM_CLEAR_MODE instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_set_clear_mode(int16 hcam,
                                                     int16 ccd_clear);
    /** Use #PARAM_COLOR_MODE instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_color_mode(int16 hcam,
                                                     uns16 *color_mode);
    /** Use #PARAM_COOLING_MODE instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_cooling_mode(int16 hcam,
                                                       int16 *cooling);
    /** Use #PARAM_FRAME_CAPABLE instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_frame_capable(int16 hcam,
                                                        rs_bool *frame_capable);
    /** Use #PARAM_FWELL_CAPACITY instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_fwell_capacity(int16 hcam,
                                                         uns32 *fwell_capacity);
    /** Use #PARAM_MPP_CAPABLE instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_mpp_capable(int16 hcam,
                                                      int16 *mpp_capable);
    /** Use #PARAM_PREAMP_DELAY instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_preamp_dly(int16 hcam,
                                                     uns16 *preamp_dly);
    /** Use #PARAM_PREAMP_OFF_CONTROL instead. */
    DEPRECATED rs_bool PV_DECL
    pl_ccd_get_preamp_off_control(int16 hcam, uns32 *preamp_off_control);
    /** Use #PARAM_PREAMP_OFF_CONTROL instead. */
    DEPRECATED rs_bool PV_DECL
    pl_ccd_set_preamp_off_control(int16 hcam, uns32 preamp_off_control);
    /** Use deprecated PARAM_PREFLASH instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_preflash(int16 hcam,
                                                   uns16 *pre_flash);
    ...
    DEPRECATED rs_bool PV_DECL pl_ccd_get_postmask(int16 hcam,
                                                   uns16 *post_mask);
    ...
    DEPRECATED rs_bool PV_DECL pl_ccd_get_serial_num(int16 hcam,
                                                     uns16 *serial_num);
    ...
    DEPRECATED rs_bool PV_DECL pl_ccd_get_summing_well(int16 hcam,
                                                       rs_bool *s_well_exists);
    /** Use #PARAM_TEMP instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_tmp(int16 hcam, int16 *cur_tmp);
    /** Use #PARAM_TEMP instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_tmp_range(int16 hcam,
                                                    int16 *tmp_hi_val,
                                                    int16 *tmp_lo_val);
    /** Use #PARAM_TEMP_SETPOINT instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_tmp_setpoint(int16 hcam,
                                                       int16 *tmp_setpoint);
    /** Use #PARAM_TEMP_SETPOINT instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_set_tmp_setpoint(int16 hcam,
                                                       int16 tmp_setpoint);
    DEPRECATED rs_bool PV_DECL pl_ccd_set_readout_port(int16, int16);
    /** Use #PARAM_PIX_PAR_DIST instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_pix_par_dist(int16 hcam,
                                                       uns16 *pix_par_dist);
    /** Use #PARAM_PIX_PAR_SIZE instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_pix_par_size(int16 hcam,
                                                       uns16 *pix_par_size);
    /** Use #PARAM_PIX_SER_DIST instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_pix_ser_dist(int16 hcam,
                                                       uns16 *pix_ser_dist);
    /** Use #PARAM_PIX_SER_SIZE instead. */
    DEPRECATED rs_bool PV_DECL pl_ccd_get_pix_ser_size(int16 hcam,
                                                       uns16 *pix_ser_size);
    /** Use #PARAM_BIT_DEPTH instead. */
    DEPRECATED rs_bool PV_DECL pl_spdtab_get_bits(int16 hcam,
                                                  int16 *spdtab_bits);
    /** Use #PARAM_GAIN_INDEX instead. */
    DEPRECATED rs_bool PV_DECL pl_spdtab_get_gain(int16 hcam,
                                                  int16 *spdtab_gain);
    /** Use #PARAM_GAIN_INDEX instead. */
    DEPRECATED rs_bool PV_DECL pl_spdtab_set_gain(int16 hcam,
                                                  int16 spdtab_gain);
    /** Use #PARAM_GAIN_INDEX instead. */
    ...
    /** Use #PARAM_SPDTAB_INDEX instead. */
    DEPRECATED rs_bool PV_DECL pl_spdtab_get_entries(int16 hcam,
                                                     int16 *spdtab_entries);
    /** Use #PARAM_READOUT_PORT instead. */
    DEPRECATED rs_bool PV_DECL pl_spdtab_get_port(int16 hcam,
                                                  int16 *spdtab_port);
    /** Use #PARAM_READOUT_PORT instead. */
    DEPRECATED rs_bool PV_DECL pl_spdtab_get_port_total(int16 hcam,
                                                        int16 *total_ports);
    /** Use #PARAM_PIX_TIME instead. */
    DEPRECATED rs_bool PV_DECL pl_spdtab_get_time(int16 hcam,
                                                  uns16 *spdtab_time);
    /** Use #PARAM_SHTR_CLOSE_DELAY instead. */
    DEPRECATED rs_bool PV_DECL pl_shtr_get_close_dly(int16 hcam,
                                                     uns16 *shtr_close_dly);
    /** Use #PARAM_SHTR_CLOSE_DELAY instead. */
    DEPRECATED rs_bool PV_DECL pl_shtr_set_close_dly(int16 hcam,
                                                     uns16 shtr_close_dly);
    /** Use #PARAM_SHTR_OPEN_DELAY instead. */
    DEPRECATED rs_bool PV_DECL pl_shtr_get_open_dly(int16 hcam,
                                                    uns16 *shtr_open_dly);
    /** Use #PARAM_SHTR_OPEN_DELAY instead. */
    DEPRECATED rs_bool PV_DECL pl_shtr_set_open_dly(int16 hcam,
                                                    uns16 shtr_open_dly);
    /** Use #PARAM_SHTR_OPEN_MODE instead. */
    DEPRECATED rs_bool PV_DECL pl_shtr_get_open_mode(int16 hcam,
                                                     int16 *shtr_open_mode);
    /** Use #PARAM_SHTR_OPEN_MODE instead. */
    DEPRECATED rs_bool PV_DECL pl_shtr_set_open_mode(int16 hcam,
                                                     int16 shtr_open_mode);
    /** Use #PARAM_SHTR_STATUS instead. */
    DEPRECATED rs_bool PV_DECL pl_shtr_get_status(int16 hcam,
                                                  int16 *shtr_status);
    ...
    DEPRECATED rs_bool PV_DECL pl_buf_get_exp_date(int16 hbuf, int16 exp_num,
                                                   ...
                                                   uns8 *min, uns8 *sec,
                                                   uns16 *msec);
    DEPRECATED rs_bool PV_DECL pl_buf_set_exp_date(int16 hbuf, int16 exp_num,
                                                   int16 year, uns8 month,
                                                   uns8 day, uns8 hour,
                                                   uns8 min, uns8 sec,
                                                   uns16 msec);
    DEPRECATED rs_bool PV_DECL pl_buf_get_exp_time(int16 hbuf, int16 exp_num,
                                                   uns32 *exp_msec);
    DEPRECATED rs_bool PV_DECL pl_buf_get_exp_total(int16 hbuf,
                                                    int16 *total_exps);
    DEPRECATED rs_bool PV_DECL pl_buf_get_img_bin(int16 himg, int16 *ibin,
                                                  int16 *jbin);
    DEPRECATED rs_bool PV_DECL pl_buf_get_img_handle(int16 hbuf, int16 exp_num,
                                                     int16 img_num,
                                                     int16 *himg);
    DEPRECATED rs_bool PV_DECL pl_buf_get_img_ofs(int16 himg, int16 *s_ofs,
                                                  int16 *p_ofs);
    DEPRECATED rs_bool PV_DECL pl_buf_get_img_ptr(int16 himg, void **img_addr);
    DEPRECATED rs_bool PV_DECL pl_buf_get_img_size(int16 himg, int16 *x_size,
                                                   int16 *y_size);
    DEPRECATED rs_bool PV_DECL pl_buf_get_img_total(int16 hbuf,
                                                    int16 *totl_imgs);
    ...
    DEPRECATED rs_bool PV_DECL pv_cam_send_debug(int16 hcam, char *debug_str,
                                                 uns16 reply_len,
                                                 char *reply_str);
    DEPRECATED rs_bool PV_DECL pv_cam_write_read(int16 hcam, uns8 c_class,
                                                 uns16 write_bytes,
                                                 uns8 *write_array,
                                                 uns8 *read_array);
    ...
    DEPRECATED rs_bool PV_DECL pv_exp_get_script(int16 hcam,
                                                 rs_bool *script_valid);
    ...
    DEPRECATED rs_bool PV_DECL pv_exp_set_bytes(int16 hcam, uns32 frame_count,
                                                uns32 seq_bytes,
                                                void *pixel_stream);
    DEPRECATED rs_bool PV_DECL pv_exp_set_script(int16 hcam,
                                                 rs_bool script_valid);
    ...
    DEPRECATED rs_bool PV_DECL pv_ccd_get_accum_capable(int16 hcam,
                                                        rs_bool *accum_capable);
    ...
    DEPRECATED rs_bool PV_DECL pm_cam_write_read(int16 hcam, uns8 c_class,
                                                 uns16 write_bytes,
                                                 uns8 *write_array,
                                                 uns8 *read_array);
    ...
    */
    ...
    /**
    @brief Decodes a metadata-enabled frame buffer into provided frame
    descriptor structure.

    This function processes the input frame buffer and calculates pointers to
    frame metadata headers, ROI headers and ROI image data and stores them to
    previously allocated @a pDstFrame structure. This function does not copy any
    pixel data. Since the @a pDstFrame stores only pointers to the @a pSrcBuf
    memory, the @a pSrcBuf must be valid for as long as the @a pDstFrame is
    accessed.

    @param[out] pDstFrame   A pre-allocated helper structure that will be filled
    with information from the given raw buffer. Create this structure with
                            #pl_md_create_frame_struct or
    #pl_md_create_frame_struct_cont functions.
    @param[in]  pSrcBuf     A pointer to a frame data as retrieved from PVCAM.
    See functions #pl_exp_get_latest_frame and #pl_exp_get_latest_frame_ex.
    @param[in]  srcBufSize  The size of the raw frame data. Size of a frame is
    obtained from #pl_exp_setup_seq and #pl_exp_setup_cont functions.

    @return #PV_OK for success, #PV_FAIL for failure. Failure sets
    #pl_error_code.
    */
    rs_bool PV_DECL pl_md_frame_decode(md_frame *pDstFrame, void *pSrcBuf,
                                       uns32 srcBufSize);

    /**
    @brief Optional function that recomposes a multi-ROI frame into a
    displayable image buffer.

    Every ROI will be copied into its appropriate location in the provided
    buffer. Please note that the function will subtract the Implied ROI position
    from each ROI position which essentially moves the entire Implied ROI to a
    [0, 0] position. Use the Offset arguments to shift all ROIs back to desired
    positions if needed. If you use the Implied ROI position for offset
    arguments, the frame will be recomposed as it appears on the full frame.

    The caller is responsible for black-filling the input buffer. Usually, this
    function is called during live/preview mode where the destination buffer is
    re-used. If the ROIs do move during acquisition, it is essential to
    black-fill the destination buffer before calling this function. This is not
    needed if the ROIs do not move. If the ROIs move during live mode, it is
    also recommended to use the offset arguments and recompose the ROI to a full
    frame - with moving ROIs the implied ROI may change with each frame and this
    may cause undesired ROI "twitching" in the displayable image.

    @param[out] pDstBuf     An output buffer; the buffer must be at least the
    size of the implied ROI that is calculated during the frame decoding
    process. The buffer must be of the same type as is the input frame - e.g. if
    the input frame format is 8-bit, the destination buffer should be 8-bit as
    well. If offset is set, the buffer dimensions must be large enough to allow
    the entire implied ROI to be positioned in the buffer.
    @param[in]  offX        Offset in the destination buffer, in pixels. If 0,
    the Implied ROI will be shifted to position 0 in the target buffer. Use
    (ImpliedRoi.s1 / ImplierRoi.sbin) as offset value to disable the shift and
    keep the ROIs in their absolute positions.
    @param[in]  offY        Offset in the destination buffer, in pixels. If 0,
    the Implied ROI will be shifted to position 0 in the target buffer. Use
    (ImpliedRoi.p1 / ImplierRoi.pbin) as offset value to disable the shift and
    keep the ROIs in their absolute positions.
    @param[in]  dstWidth    Width, in pixels of the destination image buffer.
    The buffer must be large enough to hold the entire Implied ROI, including
    the offsets (if used).
    @param[in]  dstHeight   Height, in pixels of the destination image buffer.
    @param[in]  pSrcFrame   A helper structure, previously decoded using the
    frame decoding function #pl_md_frame_decode.
    ...
    */
    ...
    /**
    @brief This method creates an empty md_frame structure for known number of
    ROIs.
    ...
    @param[out] pFrame      A pointer to frame helper structure address where
    the structure will be allocated.
    ...
    */
    ...
    /**
    @brief This method creates an empty md_frame structure from an existing
    buffer.
    ...
    For continuous acquisition where the number or ROIs is known, it is
    recommended to use the other provided method to avoid frequent memory
    allocation.

    @param[out] pFrame      A pointer address where the newly created structure
    will be stored.
    @param[in]  pSrcBuf     A raw frame data pointer as returned from the
    camera.
    ...
    */
    ...
    @param[out] pOutput     A pre-allocated structure that will be filled with
    metadata
    @param[in]  pExtMdPtr   A pointer to the ext. MD buffer, this can be
    obtained from the md_frame and md_frame_roi structures.
    @param[in]  extMdSize   Size of the ext. MD buffer, also retrievable from
