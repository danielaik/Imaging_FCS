...
void ImagePVCam::setPixelDimParam(int w, int h, int l, int t, int incamerabin)
{
    ...
    assert((hbin == vbin)
           && "error 23; Photometrics only support in camera "
              "binning of equal dimension");
    // std::cout << "inside setPixelDimParam hbin: " << hbin << ", vbin: " <<
    // vbin
    // << ", hstart: " << hstart << ", hend:  " << hend << ", vstart: " <<
    // vstart
    // << ", vend: " << vend << std::endl;
}
...
void ImagePVCam::getnoPixelXY()
{
    ...
    // std::cout << "inside getnoPixelXY x: " << nopixelX_ << ", y: " <<
    // nopixelY_
    // << std::endl;
}
...
int ImagePVCam::setAOI()
{
    ...
    // Set the CCD g_Region fo Full Frame, serial and parallel binning is set to
    // 1
    g_Region.s1 = hstart;
    g_Region.s2 = hend;
    g_Region.sbin = hbin;
...
void ImagePVCam::setupSequence()
{
    if (acqmode_ == 1)
    {
        ...
        // FramesToAcquire specifies number of frames to acquire in this
        // sequence. exposureBytes will now hold the size of the whole buffer
        // for all the images in the sequence.
        if (PV_OK
            != pl_exp_setup_seq(g_hCam, totalframe_, 1, &g_Region, TIMED_MODE,
        ...
        // std::cout << "framesToAcquire: " << totalframe_ << " frames" <<
        // std::endl; std::cout << "exposureBytes: " << exposureBytes_ << "
        // bytes"
        // << std::endl; std::cout << "size of each frames: " << oneFrameBytes_
        // << " bytes" << std::endl;

        // assert(((exposureBytes_ / sizeof(uns16)) == arraysize_) && "error 32
        // setupSequence unmatched arraysize_"); //Not applicable as some mode
        // uses 8bit, 12bit, and 16bit
    }
    else if (acqmode_ == 2)
    {
        ...
        // might hold a pointer to user-specific content that will be passed
        // with the callback once it is invoked.
        if (PV_OK
            != pl_cam_register_callback_ex3(g_hCam, PL_CALLBACK_EOF,
                                            (void *)NewFrameHandler,
                                            (void *)&dataContext))
        {
            // PrintErrorMessage(pl_error_code(), "pl_cam_register_callback()
            // error");
            CloseCameraAndUninit();
        ...
        // TRIGGER_FIRST_MODE.
        if (PV_OK
            != pl_exp_setup_cont(g_hCam, 1, &g_Region, TIMED_MODE,
...
void ImagePVCam::setFrameTransfer()
{
    ...
    if (g_IsFrameTransfer == TRUE)
    {
        ...
        if (g_IsFrameTransfer == TRUE)
            std::cout << "Camera with Frame Transfer capability sensor"
                      << std::endl;
    }
    if (g_IsFrameTransfer == FALSE)
    {
        g_IsFrameTransfer = 0;
        std::cout << "Camera without Frame Transfer capability sensor"
                  << std::endl;
    }
...
void ImagePVCam::setExposureTime(double exptime)
{
    ...
    // defines units in which the exposure time is passed to #pl_exp_setup_seq
    // and #pl_exp_setup_cont
