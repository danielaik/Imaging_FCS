...
#    define EXTRACTHIGHPACKED(SourcePtr)                                       \
        ((SourcePtr[2] << 4) + (SourcePtr[1] >> 4))
...
GainMode, IOSelector, PixelEncoding, PixelReadoutRate, ShutterMode,
SimplePreAmpGainControl, TemperatureControl, TemperatureStatus, TriggerMode,
TriggerSource

List of SOna int feature:
AOIHBin, AOIWidth, AOILeft, AOIVBin, AOIHeight, AOITop, AOIStride, Baseline,
BufferOverflowEvent, CameraImagesWaiting, EventsMissedEvent, ExposureEndEvent,
ExposureStartEvent, FrameGenFixedPixelValue, FrameCount, ImageSizeBytes,
MultitrackCount, MultitrackEnd, MultitrackSelector, MultitrackStart,
RowNExposureStartEvenet, RowNExposureEndEvenet, SensorHeight, SensorWidth,
TimestampClock, TimestampClockFrequency

List of SOna float feature:
BytesPerPixel, ExposureTime, FrameRate, MaxInterfaceTransferRate, PixelHeight,
PixelWidth, ReadoutTime, RowReadTime, SensorTemperature, ShutterTransferTime,
TargetSensorTemperature,

List of Sona boolean featuer:
CameraAcquiring, CameraPresent, EventEnable, FullAOIControl, IOInvert,
MetadataEnable, MetadataFrame, MetadataTimestamp, MultitrackBinned,
MultitrackCount, Overlap, SensorCooling, SpuriousNoiseFilter,
VerticallyCentreAOI

List of Sona String feature:
CameraFamily, CameraModel, CameraName, FirmwareVersion, InterfaceType,
SerialNumber
...
void extract2from3single(unsigned char *_buffer, short *_i_returns,
                         int _nopixperframe);
void extractMono12or16single(unsigned char *_buffer, short *_i_returns,
                             int _nopixperframe);
void setParamInfiniteLoop();
int performAcquisition_infiniteLoop(int _handle, int _numberAcquisitions,
                                    int _intervalFrame, JNIEnv *env,
                                    jshortArray _outArray,
                                    jobject framecounterObj,
                                    jmethodID increment_id, jshort *carray,
                                    jboolean isCopy);
void createBuffers(int _handle);
unsigned char *QueueBuffer(int _handle);
int doLoopOfAcquisition(int _handle, int _totalAc, int _i_interval, JNIEnv *env,
                        jshortArray _outArray, jobject framecounterObj,
                        jmethodID increment_id, jshort *carray,
                        jboolean isCopy);
void extract2from3Inf_everyInterval(unsigned char *_buffer, short *_i_returns,
                                    long long _size, int intervalnum);
void extractMono12or16Inf_everyInterval(unsigned char *_buffer,
                                        short *_i_returns, long long _size,
                                        int intervalnum);
void deleteBuffers();
...
int isSCMOSconnected()
{
    ...
    if (i_retCode != AT_SUCCESS) {
        ...
        if (iNumberDevices <= 0) {
            ...
            if (i_retCode != AT_SUCCESS) {
                std::cout << "Error condition, could not initialise camera" <<
    std::endl << std::endl;
            }
            else {
                std::cout << "Successfully initialised camera" << std::endl <<
    std::endl;
            }
            AT_WC szValue[64];
            ...
            if (i_retCode == AT_SUCCESS) {
                ...
                std::cout << "Error obtaining Serial number" << std::endl <<
    std::endl;
            }
            AT_Close(Hndl);
        }
    ...
    */
    ...
    {
        imgobj.i_retCode = AT_GetInt(AT_HANDLE_SYSTEM, L"Device Count",
                                     &imgobj.iNumberDevices);
        ...
        else
        {
            ...
            if (imgobj.i_retCode != AT_SUCCESS)
            {
                std::cout << "Error condition, could not initialise camera; "
                             "i_retCode: "
                          << imgobj.i_retCode << std::endl
                          << std::endl;
                AT_FinaliseLibrary();
                return imgobj.i_retCode; // error code 17 is due to
                                         // Solis/umanger is still open
            }
            else
            {
                imgobj.i_retCode =
                    AT_GetBool(imgobj.Hndl, L"CameraPresent", &campresent);

                if (imgobj.i_retCode != AT_SUCCESS)
                {
                    std::cout << "error CameraPresent, error code: "
                              << imgobj.i_retCode << std::endl;
                }
            }
        }
    ...
    /*
    std::cout << "isSCMOSconnectedSDK3 closing and finalising library, Hndl: "
    << imgobj.Hndl << " &Hndl: " << &imgobj.Hndl << std::endl;
...
void InitializeSystem()
{
    ...
    else
    {
        ...
        {
            imgobj.i_retCode = AT_Open(0, &imgobj.Hndl);
            // assert((imgobj.i_retCode == AT_SUCCESS) && "error Opening
            // Handle");
        }
    }
}
AT_WC *getEnumString(const wchar_t *feature)
{
    ...
    if (imgobj.i_retCode == AT_SUCCESS)
    {
        imgobj.i_retCode = AT_GetEnumStringByIndex(
            imgobj.Hndl, feature, indexenum, iFeatureValue, 64);
        if (imgobj.i_retCode != AT_SUCCESS)
        {
            std::cout << "Error get enum string" << std::endl;
            std::cout << "error code: " << imgobj.i_retCode << std::endl
                      << std::endl;
        }
        else
        {
...
void setParamSingle()
{
    ...
    if (imgobj.i_retCode != AT_SUCCESS) {
        std::cout << "error FrameGenPixelValue: " << imgobj.i_retCode <<
    std::endl;
    }

    imgobj.i_retCode = AT_SetEnumeratedString(imgobj.Hndl, L"FrameGenMode",
    imgobj.iFrameGenMode); if (imgobj.i_retCode != AT_SUCCESS) { std::cout <<
    "error FrameGenMode: " << imgobj.i_retCode << std::endl;
    }
    else {
    ...
    */
    ...
    imgobj.i_retCode =
        AT_SetEnumeratedString(imgobj.Hndl, L"TriggerMode", L"Internal");
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    // set GainMode
    imgobj.i_retCode =
        AT_GetEnumCount(imgobj.Hndl, L"GainMode", &imgobj.GMcount);

    for (int i = 0; i < imgobj.GMcount; i++)
    {
        AT_GetEnumStringByIndex(imgobj.Hndl, L"GainMode", i, imgobj.GMstring,
                                64);
        // std::cout << "GainMode index: " << i << std::endl;
        // std::wcout << "GainMode string: " << imgobj.GMstring << std::endl;
    }
    imgobj.i_retCode =
        AT_GetEnumIndex(imgobj.Hndl, L"GainMode", &imgobj.GMindex);

    imgobj.i_retCode = AT_GetEnumStringByIndex(
        imgobj.Hndl, L"GainMode", imgobj.GMindex, imgobj.GMstring, 64);

    imgobj.i_retCode =
        AT_SetEnumeratedString(imgobj.Hndl, L"GainMode", imgobj.iGainMode);
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    imgobj.i_retCode = AT_SetEnumeratedString(imgobj.Hndl, L"Pixel Encoding",
                                              imgobj.iPixelEncoding);
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    imgobj.i_retCode =
        AT_GetEnumCount(imgobj.Hndl, L"PixelReadoutRate", &imgobj.PRRcount);

    for (int i = 0; i < imgobj.PRRcount; i++)
    {
        AT_GetEnumStringByIndex(imgobj.Hndl, L"PixelReadoutRate", i,
                                imgobj.PRRstring, 64);
    }
    imgobj.i_retCode =
        AT_GetEnumIndex(imgobj.Hndl, L"PixelReadoutRate", &imgobj.PRRindex);

    imgobj.i_retCode =
        AT_GetEnumStringByIndex(imgobj.Hndl, L"PixelReadoutRate",
                                imgobj.PRRindex, imgobj.PRRstring, 64);

    imgobj.i_retCode = AT_SetEnumeratedString(
        imgobj.Hndl, L"PixelReadoutRate",
        imgobj.iPixelReadoutRate); // 100MHz or 200MHz for sona
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    // Set Rolling shutter
    imgobj.i_retCode = AT_GetEnumCount(imgobj.Hndl, L"ElectronicShutteringMode",
                                       &imgobj.ShutterModecount);

    for (int i = 0; i < imgobj.ShutterModecount; i++)
    {
        AT_GetEnumStringByIndex(imgobj.Hndl, L"ElectronicShutteringMode", i,
                                imgobj.ShutterModestring, 64);
    }
    imgobj.i_retCode = AT_GetEnumIndex(imgobj.Hndl, L"ElectronicShutteringMode",
                                       &imgobj.ShutterModeindex);

    imgobj.i_retCode = AT_GetEnumStringByIndex(
        imgobj.Hndl, L"ElectronicShutteringMode", imgobj.ShutterModeindex,
        imgobj.ShutterModestring, 64);

    imgobj.i_retCode =
        AT_SetEnumeratedString(imgobj.Hndl, L"ElectronicShutteringMode",
                               imgobj.iElectronicShutteringMode);
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    imgobj.i_retCode = AT_SetBool(imgobj.Hndl, L"SpuriousNoiseFilter",
                                  imgobj.iSpuriousNoiseFilter);
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    // Set AOI in the following order AOIHBin, AOIVBin, AOIWidth, AOIleft,
    // AOIHeight, AOITop
    ...
    imgobj.i_retCode = AT_GetFloat(imgobj.Hndl, L"MaxInterfaceTransferRate",
                                   &imgobj.maxFrameRate);
    assert((imgobj.i_retCode == 0)
           && "error void setParamSingle() MaxTransferRate");

    // note that it is important to set EXPOSURE TIME and FRAMETIME once all
    // other parameter are set such as readout rate, shutterimg mode, AOI as
    // these parameter affect how fast we can record set exposure time and frame
    // rate
    ...
    imgobj.i_retCode =
        AT_SetFloat(imgobj.Hndl, L"Exposure Time", imgobj.iExposureTime);
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    imgobj.i_retCode =
        AT_GetFloat(imgobj.Hndl, L"ExposureTime", &imgobj.iExposureTime);
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    imgobj.i_retCode =
        AT_SetFloat(imgobj.Hndl, L"Framerate", imgobj.iFrameRate);
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    imgobj.i_retCode =
        AT_GetEnumIndex(imgobj.Hndl, L"PixelReadoutRate", &imgobj.PRRindex);
    std::cout << "selected pixelreadoutrate index: " << imgobj.PRRindex
              << std::endl;
    imgobj.i_retCode =
        AT_GetEnumStringByIndex(imgobj.Hndl, L"PixelReadoutRate",
                                imgobj.PRRindex, imgobj.PRRstring, 64);
    std::wcout << "selected pixelreadoutrate string: " << imgobj.PRRstring
               << std::endl;
    ...
    imgobj.i_retCode = AT_GetEnumIndex(imgobj.Hndl, L"ElectronicShutteringMode",
                                       &imgobj.ShutterModeindex);
    std::cout << "selected ElectronicShutterMode index: "
              << imgobj.ShutterModeindex << std::endl;
    imgobj.i_retCode = AT_GetEnumStringByIndex(
        imgobj.Hndl, L"ElectronicShutteringMode", imgobj.ShutterModeindex,
        imgobj.ShutterModestring, 64);
    std::wcout << "selected ElectronicShutterMode string: "
               << imgobj.ShutterModestring << std::endl;

    // AOI Layout
    imgobj.i_retCode =
        AT_GetEnumIndex(imgobj.Hndl, L"AOILayout", &imgobj.AOILayoutindex);
    std::cout << "selected AOILayoute index: " << imgobj.AOILayoutindex
              << std::endl;
    imgobj.i_retCode = AT_GetEnumStringByIndex(imgobj.Hndl, L"AOILayout",
                                               imgobj.AOILayoutindex,
                                               imgobj.AOILayoutstring, 64);
    std::wcout << "selected AOILayout string: " << imgobj.AOILayoutstring
               << std::endl;

    // Trigger Mode
    imgobj.i_retCode =
        AT_GetEnumIndex(imgobj.Hndl, L"TriggerMode", &imgobj.TriggerModeindex);
    std::cout << "selected TriggerMode index: " << imgobj.TriggerModeindex
              << std::endl;
    imgobj.i_retCode = AT_GetEnumStringByIndex(imgobj.Hndl, L"TriggerMode",
                                               imgobj.TriggerModeindex,
                                               imgobj.TriggerModestring, 64);
    std::wcout << "selected TriggerMode string: " << imgobj.TriggerModestring
               << std::endl;

    // PixelEncoding
    imgobj.i_retCode = AT_GetEnumIndex(imgobj.Hndl, L"Pixel Encoding",
                                       &imgobj.PixelEncodingindex);
    std::cout << "selected PixelEncoding index: " << imgobj.PixelEncodingindex
              << std::endl;
    imgobj.i_retCode = AT_GetEnumStringByIndex(imgobj.Hndl, L"Pixel Encoding",
                                               imgobj.PixelEncodingindex,
                                               imgobj.PixelEncodingstring, 64);
    std::wcout << "selected PixelEncoding string: "
               << imgobj.PixelEncodingstring << std::endl;

    // CycleMode
    imgobj.i_retCode =
        AT_GetEnumIndex(imgobj.Hndl, L"CycleMode", &imgobj.CycleModeindex);
    std::cout << "selected CycleMode index: " << imgobj.CycleModeindex
              << std::endl;
    imgobj.i_retCode = AT_GetEnumStringByIndex(imgobj.Hndl, L"CycleMode",
                                               imgobj.CycleModeindex,
                                               imgobj.CycleModestring, 64);
    std::wcout << "selected CycleMode string: " << imgobj.CycleModestring
               << std::endl;
    ...
    // FrameCount
    imgobj.i_retCode =
        AT_GetInt(imgobj.Hndl, L"FrameCount", &imgobj.FrameCountval);
    ...
    // Bit depth
    imgobj.i_retCode =
        AT_GetEnumIndex(imgobj.Hndl, L"BitDepth", &imgobj.BitDepthindex);
    std::cout << "selected BitDepth index: " << imgobj.BitDepthindex
              << std::endl;
    imgobj.i_retCode =
        AT_GetEnumStringByIndex(imgobj.Hndl, L"BitDepth", imgobj.BitDepthindex,
                                imgobj.BitDepthstring, 64);
    std::wcout << "selected BitDepth string: " << imgobj.BitDepthstring
               << std::endl;
    ...
    // Spuriosnoisefilter
    AT_GetBool(imgobj.Hndl, L"SpuriousNoiseFilter",
               &imgobj.Spuriousnoisefilterval);
    std::cout << "Spurious nose filter: " << imgobj.Spuriousnoisefilterval
              << std::endl;
    ...
    // AOIHeight
    imgobj.i_retCode =
        AT_GetInt(imgobj.Hndl, L"AOIHeight", &imgobj.AOIHeightval);
                         ...
                         int _nopixperframe)
{
    ...
    _buffer = _buffer - 30; //need to subtract by 30 since previously 20 pixel
    was read out from buffer
    */
...
void setParamInfiniteLoop()
{
    ...
    imgobj.i_retCode = AT_SetEnumeratedString(
        imgobj.Hndl, L"AuxiliaryOutSource", imgobj.iAuxOutSource);
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
    ...
    // Set CycleMode
    imgobj.i_retCode =
        AT_SetEnumString(imgobj.Hndl, L"CycleMode", L"Continuous");
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        iret = 1;
    }
                                    ...
                                    jboolean isCopy)
{
    ...
    /* //debugging
    imgobj.i_retCode = AT_GetInt(imgobj.Hndl, L"CameraImagesWaiting",
    &CameraImageWaitingval); std::cout << "after sleep, images waiting: " <<
    CameraImageWaitingval << std::endl;
    */

    if (iret == 0)
    {
        if (doLoopOfAcquisition(_handle, _numberAcquisitions, _intervalFrame,
                                env, _outArray, framecounterObj, increment_id,
                                carray, isCopy)
            != 0)
        {
            iret = 1;
...
unsigned char *QueueBuffer(int _handle)
{
    ...
    imgobj.i_retCode =
        AT_GetInt(_handle, L"ImageSizeBytes", &imgobj.iImageSizeBytes);
    if (imgobj.i_retCode != AT_SUCCESS)
    {
        std::cout << "AT_GetInt failed - ImageSizeBytes - return code "
                  << imgobj.i_retCode << std::endl;
    }
    ...
    if (imgobj.i_retCode == AT_SUCCESS)
    {
        ...
        // std::cout << "Imagesizebytes: " << imgobj.iImageSizeBytes << " ;
        // BufferSize: " << BufferSize << std::endl;
        imgobj.i_retCode = AT_QueueBuffer(_handle, acqBuffer, BufferSize);
        if (imgobj.i_retCode != AT_SUCCESS)
        {
            std::cout
                << "AT_QueueBuffer failed - Image Size Bytes - return code "
                << imgobj.i_retCode << std::endl;
        }
...
int doLoopOfAcquisition(int _handle, int _totalAc, int _i_interval, JNIEnv *env,
                        jshortArray _outArray, jobject framecounterObj,
                        jmethodID increment_id, jshort *carray, jboolean isCopy)
{
    int c = _totalAc / _i_interval;
    int new_c;
    long long i_pixperinter =
        _i_interval * imgobj.iAOIHeight * imgobj.iAOIWidth;
    ...
    for (int j = 0; j < c; j++)
    {
        ...
        for (int i = 0; i < _i_interval; i++)
        {
            ...
            /* //debug
            imgobj.i_retCode = AT_GetInt(imgobj.Hndl, L"CameraImagesWaiting",
            &CameraImageWaitingval); std::cout << "Before WaitBuffer, images
            waiting: " << CameraImageWaitingval << std::endl;
            */

            imgobj.i_retCode = AT_WaitBuffer(_handle, &pBuf, &BufSize, 10000);
            assert((imgobj.i_retCode == AT_SUCCESS)
                   && "Waitbuffer insufficient code 13");
            if (imgobj.i_retCode != AT_SUCCESS)
            {
                std::cout
                    << "Error:Acquisition timeout when not expecting, retcode "
                    << imgobj.i_retCode << std::endl;
                return 1;
            }
            if (imgobj.PixelEncodingIndex == 1)
            {
                extract2from3Inf_everyInterval(
                    pBuf, imgobj.pImageArrayInf,
                    (imgobj.iAOIHeight * imgobj.iAOIWidth), i);
            }
            else
            {
                extractMono12or16Inf_everyInterval(
                    pBuf, imgobj.pImageArrayInf,
                    (imgobj.iAOIHeight * imgobj.iAOIWidth), i);
            }

            // requeue buffer
            AT_QueueBuffer(_handle, pBuf,
                           static_cast<int>(imgobj.iImageSizeBytes));
        }

        if (isCopy == true || imgobj.enableCriticalAccess == 0)
        {
            tk3.setTimeStart();
            env->SetShortArrayRegion(_outArray, (new_c * i_pixperinter),
                                     i_pixperinter, imgobj.pImageArrayInf);
            env->CallVoidMethod(framecounterObj, increment_id);
            imgobj.timeelapsed3 += tk3.getTimeElapsed();
        }
        else
        {
            std::copy(imgobj.pImageArrayInf,
                      imgobj.pImageArrayInf + i_pixperinter,
                      carray + (new_c * i_pixperinter));
            env->CallVoidMethod(framecounterObj, increment_id);
        }
                                    ...
                                    long long _size, int intervalnum)
{
    ...
    {
        *(_i_returns + (intervalnum * _size) + counter) =
            EXTRACTLOWPACKED(_buffer); // second
        counter++;
        *(_i_returns + (intervalnum * _size) + counter) =
            EXTRACTHIGHPACKED(_buffer); // first
        counter++;
        _buffer += 3;
    }
}
    ...
    JNIEnv *env, jclass cls)
{
    ...
    // library (need to be called separately)
    ...
    jint left, jint top, jint incamerabin, jint iPixelEncoding)
{
    ...
    imgobj.iAOIWidth =
        width; // for 2x2 AOI bin; max widht is 1024 / min width 25
    imgobj.iAOIHeight =
        height; // for 2x2 AOI bin; max height is 1024 / min height 1
    imgobj.iAOILeft =
        ((AT_64)left - 1) * imgobj.iAOIHBin + 1; // previously: left
                                                                   ...
                                                                   jclass cls)
{
    ...
    // Get the number of bytes required to store one frame
    imgobj.i_retCode =
        AT_GetInt(imgobj.Hndl, L"Image Size Bytes", &imgobj.iImageSizeBytes);
    assert((imgobj.i_retCode == AT_SUCCESS) && "error getting ImageSizeBytes");
    ...
    {
        /* // for debugging purpose: print first 20 pixels; remmeber to alter
        _buffer = _buffer - 30; std::cout << "Print out of first 20 pixels " <<
        std::endl;
        ...
        */
        ...
        if (imgobj.PixelEncodingIndex == 1)
        {
            // Mono12Packed
            extract2from3single(
                Buffer, imgobj.pImageArraySingle,
                (imgobj.iAOIHeight
                 * imgobj.iAOIWidth)); // took 10ms for 2560x2160 single frame
        }
        else
        {
            // Mono12 or Mono16
            extractMono12or16single(Buffer, imgobj.pImageArraySingle,
                                    (imgobj.iAOIHeight * imgobj.iAOIWidth));
        }
        ...
        std::cout << "SINGLE FRAME before copying array interval to java" <<
        std::endl; std::cout << "number element with -1: " << numnegone <<
        std::endl; std::cout << "number element large > 10,000: " << numlarge <<
        std::endl; std::cout << "number element == 0: " << numzero << std::endl;
        */
    ...
    env->SetShortArrayRegion(outArray, 0, imgobj.ArraySizeSingle,
                             imgobj.pImageArraySingle);
    imgobj.freeImArraySingle();

    // Stop the Acquisition
    AT_Command(imgobj.Hndl, L"Acquisition Stop");

    // flush any remaining buffer that has been queued using AT_QueueBuffer;
    // should only be alled after L"Acquisition Stop"
    AT_Flush(imgobj.Hndl);

    // AT_Close(imgobj.Hndl);// this should only be called after the program
    // ends AT_FinaliseLibrary(); // this should only be called after the
    // program ends
...
Java_fiji_plugin_imaging_1fcs_directCameraReadout_andorsdk3v2_AndorSDK3v2_setParameterInfiniteLoopSDK3(
    ...
    jint auxOutSource, jint isOverlap)
{
    ...
    JNIEnv *env, jclass cls, jshortArray outArray, jobject framecounterObj)
{
    ...
    static jfieldID time1_id =
        env->GetFieldID(framecounter_class, "time1", "D");
    static jfieldID time2_id =
        env->GetFieldID(framecounter_class, "time2", "D");
    static jfieldID time3_id =
        env->GetFieldID(framecounter_class, "time3", "D");
    ...
    tk2.setTimeStart();
    int iret = performAcquisition_infiniteLoop(
        imgobj.Hndl, imgobj.totalFrame, imgobj.intervalFrame, env, outArray,
        framecounterObj, increment_id, carray, isCopy);
