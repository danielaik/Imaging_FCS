...
#    define DCAMCONST_FRAMESTAMP_MISMATCH 0xFFFFFFFF
    ...
    DCAM_DECLARE_BEGIN(enum, DCAMERR){
        /* status error */
        DCAMERR_BUSY = 0x80000101, /*		API cannot process in busy
                                      state.		*/
        ...
        DCAMERR_EXCLUDED = 0x80000110, /*		some resource is exclusive and
                                          already used	*/

        DCAMERR_COOLINGTROUBLE =
            0x80000302, /*		something happens near cooler	*/
        DCAMERR_NOTRIGGER = 0x80000303, /*		no trigger when necessary. Some
                                           camera supports this error.	*/
        DCAMERR_TEMPERATURE_TROUBLE =
            0x80000304, /*		camera warns its temperature	*/
        DCAMERR_TOOFREQUENTTRIGGER =
            0x80000305, /*		input too frequent trigger. Some camera supports
                           this error.	*/
        ...
        DCAMERR_MISSINGFRAME_TROUBLE =
            0x80000f06, /*		frame is lost but reason is low lever driver's
                           bug	*/
        ...
        DCAMERR_FRAMEGRABBER_NEEDS_FIRMWAREUPDATE =
            0x80001002, /*	need to update frame grabber firmware to use the
                         * camera
                         */
        ...
        DCAMERR_NEWAPIREQUIRED =
            0x80000826, /*		old API cannot present the value because only
                           new API need to be used	*/
        DCAMERR_WRONGHANDSHAKE =
            0x80000827, /*		this error happens DCAM get error code from
                           camera unexpectedly	*/
        DCAMERR_NOPROPERTY =
            0x80000828, /*		there is no altenative or influence id, or no
                           more property id	*/
        DCAMERR_INVALIDCHANNEL =
            0x80000829, /*		the property id specifies channel but channel is
                           invalid	*/
        DCAMERR_INVALIDVIEW = 0x8000082a, /*		the property id specifies
                                             channel but channel is invalid	*/
        DCAMERR_INVALIDSUBARRAY =
            0x8000082b, /*		the combination of subarray values are invalid.
                           e.g. DCAM_IDPROP_SUBARRAYHPOS +
                           DCAM_IDPROP_SUBARRAYHSIZE is greater than the number
                           of horizontal pixel of sensor.
                         */
        DCAMERR_ACCESSDENY = 0x8000082c, /*		the property cannot access
                                            during this DCAM STATUS	*/
        DCAMERR_NOVALUETEXT =
        ...
        DCAMERR_DISHARMONY = 0x80000830, /*		the paired camera does not have
                                            same parameter	*/
        DCAMERR_FRAMEBUNDLESHOULDBEOFF =
            0x80000832, /*	framebundle mode should be OFF under current
                           property settings	*/
        ...
        DCAMERR_NOCORRECTIONDATA =
            0x80000838, /*		not take the dark and shading correction data
                           yet.	*/
        DCAMERR_CHANNELDEPENDENTVALUE =
            0x80000839, /*	each channel has own property value so can't return
                           overall property value.	*/
        DCAMERR_VIEWDEPENDENTVALUE =
            0x8000083a, /*		each view has own property value so can't return
                           overall property value.	*/
        DCAMERR_NODEVICEBUFFER =
            0x8000083b, /*		the frame count is larger than device momory
                           size on using device memory.	*/
        DCAMERR_REQUIREDSNAP = 0x8000083c, /*		the capture mode is sequence
                                              on using device memory.	*/
        DCAMERR_LESSSYSTEMMEMORY =
            0x8000083f, /*		the sysmte memory size is too small. PC doesn't
                           have enough memory or is limited memory by 32bit OS.
                         */
        ...
        DCAMERR_MISMATCH_CONFIGURATION =
            0x83001011, /*	mismatch between camera output(connection) and frame
                           grabber specs */
        ...
        DCAMERR_WRITEFULL = 0x84001006, /*		DCAMREC writes full frame of the
                                           session	*/
        DCAMERR_ALREADYOCCUPIED =
            0x84001007, /*		DCAMREC handle is already occupied by other
                           HDCAM	*/
        ...
        DCAMERR_NEWRUNTIMEREQUIRED =
            0x84002002, /*		DCAM Module Version is older than the version
                           that the camera requests	*/
        DCAMERR_VERSIONMISMATCH =
            0x84002003, /*		Camre returns the error on setting parameter to
                           limit version	*/
        ...
        DCAMERR_IMAGE_NEWRUNTIMEREQUIRED =
            0x84003002, /* version of image header is newer than version that
                           used DCAM supports	*/
        ...
        DCAMERR_DELAYEDFRAME =
            0x80000f09, /*	the frame waiting re-load from hardware buffer with
                           SNAPSHOT of DEVICEBUFFER MODE */
        ...
        /* Between DCAMERR_INITOPTION_COLLISION_BASE and
           DCAMERR_INITOPTION_COLLISION_MAX means there is collision with
           initoption in DCAMAPI_INIT. */
        /* The value "(error code) - DCAMERR_INITOPTION_COLLISION_BASE"
           indicates the index which second INITOPTION group happens. */

        DCAMERR_MISSPROP_TRIGGERSOURCE =
            0xE0100110, /*		the trigger mode is internal or syncreadout on
                           using device memory. */

        /* success */
        DCAMERR_SUCCESS = 1 /*		no error, general success code, app
        ...
        DCAM_DECLARE_BEGIN(enum, DCAMBUF_FRAME_OPTION){
            ...
            DCAM_DECLARE_BEGIN(enum, DCAMREC_FRAME_OPTION){
                ...
                DCAM_DECLARE_BEGIN(enum, DCAMBUF_METADATAOPTION){
                    DCAMBUF_METADATAOPTION__VIEW_ALL =
                        DCAMBUF_FRAME_OPTION__VIEW_ALL,
                    DCAMBUF_METADATAOPTION__VIEW_1 =
                        DCAMBUF_FRAME_OPTION__VIEW_1,
                    DCAMBUF_METADATAOPTION__VIEW_2 =
                        DCAMBUF_FRAME_OPTION__VIEW_2,
                    DCAMBUF_METADATAOPTION__VIEW_3 =
                        DCAMBUF_FRAME_OPTION__VIEW_3,
                    DCAMBUF_METADATAOPTION__VIEW_4 =
                        DCAMBUF_FRAME_OPTION__VIEW_4,
                    DCAMBUF_METADATAOPTION__VIEW__STEP =
...
#    if DCAMAPI_VER >= 4000
        ...
        DCAM_DECLARE_BEGIN(enum, DCAMDATA_KIND){
            DCAMDATA_KIND__REGION = 0x00000001, DCAMDATA_KIND__LUT = 0x00000002,
    ...
    DCAM_DECLARE_BEGIN(struct, DCAMAPI_INIT)
    {
        ...
        const int32 *initoption; // [in ptr] initialize options. Choose from
                                 // DCAMAPI_INITOPTION
    ...
    {
        DCAMDEV_CAPABILITY
        hdr; // [in] size:		size of this structure
    ...
    DCAM_DECLARE_BEGIN(struct, DCAMDEV_CAPABILITY_FRAMEOPTION)
    {
        ...
        hdr; // [in] size:		size of this structure
             ...
             // [out]capflag:	available DCAMBUF_PROCTYPE__* flags in current
             // condition. [in]	kind:		0 reserved

        int32 supportproc; // [out] support DCAMBUF_PROCTYPE__* flags in the
                           // camera. hdr.capflag may be 0 if the function
                           // doesn't work in current condition.
    }
    ...
    DCAM_DECLARE_BEGIN(struct, DCAMPROP_ATTR)
    {
        ...
        int32 iProp_NumberOfElement; //	property id to get number of elements of
                                     // this property if it is array
    ...
    {
        // copyframe() and lockframe() use this structure. Some members have
        // different direction. [i:o] means, the member is input at copyframe()
        // and output at lockframe(). [i:i] and [o:o] means always input and
        // output at both function. "input" means application has to set the
        // value before calling. "output" means function filles a value at
        // returning.
    ...
    DCAM_DECLARE_BEGIN(
        struct, DCAMREC_FRAME) // currently the structure is same as DCAM_FRAME.
                               // option flag means are different.
    {
        // copyframe() and lockframe() use this structure. Some members have
        // different direction. [i:o] means, the member is input at copyframe()
        // and output at lockframe(). [i:i] and [o:o] means always input and
...
#        endif
    ...
    DCAM_DECLARE_BEGIN(struct, DCAM_METADATABLOCKHDR)
    {
        int32 size; // [in] size of whole structure, not only this.
        int32 iKind; // [in] DCAM_METADATAKIND
        int32 option; // [in] value meaning depends on DCAMBUF_METADATAOPTION or
    ...
    DCAM_DECLARE_BEGIN(struct, DCAM_TIMESTAMPBLOCK)
    {
        ...
        hdr; // [in] size member should be size of this structure
             ...
             // [in] option should be one of DCAMBUF_METADATAOPTION or
             // DCAMREC_METADATAOPTION

        DCAM_TIMESTAMP *timestamps; // [in] pointer for TIMESTAMP block
        int32 timestampsize; // [in] sizeof(DCAM_TIMESTRAMP)
        int32 timestampvaildsize; // [o] return the written data size of
                                  // DCAM_TIMESTRAMP.
        int32 timestampkind; // [o] return timestamp kind(Hardware, Driver, DCAM
                             // etc..)
        int32 reserved;
    }
    ...
    DCAM_DECLARE_BEGIN(struct, DCAM_METADATATEXTBLOCK)
    {
        ...
        //	buf
        //// [in] pointer for filling userdatatext block 	unitsizes
        //// [in] pointer for filling each text size of METADATA 	bytesperunit
        //// [in] max bytes per unit for filling each METADATA 	textcodepage
        //// [in] pointer for filling each text codepage of METADATA

        // dcamrec_lockmetadatablock()
        //	buf
        //// [out] return DCAM internal pointer of userdatatext block unitsizes
        //// [out] return DCAM internal array pointer of each size 	bytesperunit
        //// [out] max bytes per unit which is set at DCAMREC_OPEN 	textcodepage
        //// [out] return DCAM internal array pointer of each codepage
    }
    ...
    DCAM_DECLARE_BEGIN(struct, DCAM_METADATABINBLOCK)
    {
        ...
        // dcamrec_copymetadatablock()
        //	bin
        //// [in] pointer for filling userdatabin block 	binsizes
        //// [in] pointer for filling each bin size of METADATA 	bytesperunit
        //// [in] max bytes per unit for filling each METADATA

        // dcamrec_lockmetadatablock()
        //	bin
        //// [out] return DCAM internal pointer of userdata bin block 	binsizes
        //// [out] return DCAM internal array pointer of each bin size
    ...
    DCAMERR DCAMAPI dcamprop_setgetvalue(HDCAM h, int32 iProp, double *pValue,
                                         int32 option DCAM_DEFAULT_ARG);
    DCAMERR DCAMAPI dcamprop_queryvalue(HDCAM h, int32 iProp, double *pValue,
                                        int32 option DCAM_DEFAULT_ARG);
    DCAMERR DCAMAPI dcamprop_getnextid(HDCAM h, int32 *pProp,
                                       int32 option DCAM_DEFAULT_ARG);
    DCAMERR DCAMAPI dcamprop_getname(HDCAM h, int32 iProp, char *text,
                                     int32 textbytes);
...
#        endif
    ...
    DCAM_DECLARE_BEGIN(struct, DCAM_METADATABLOCK)
    {
        ...
        // dcamrec_copymetadatablock()
        //	buf
        //// [in] pointer for filling userdata block 	unitsizes
        //// [in] pointer for filling each unit size of METADATA 	bytesperunit
        //// [in] max bytes per unit for filling each METADATA

        // dcamrec_lockmetadatablock()
        //	buf
        //// [out] return DCAM internal pointer of userdata block 	unitsizes
        //// [out] return DCAM internal array pointer of each size 	bytesperunit
