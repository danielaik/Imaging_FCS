...
int setParameter(double exptime, int oW, int oH, int oL, int oT,
                 int incamerabin, int acqmode, int totalframe, int size_b,
                 int arraysize, int outTriggerKind, double outTrigDelay,
                 double outTrigPeriod, int readoutSpeedIdx, int sensorModeIdx);
void run_acquisition(HDCAM hdcam, HDCAMWAIT hwait); // UNUSED
void run_acquisition(HDCAM hdcam, HDCAMWAIT hwait, JNIEnv *_env,
                     jshortArray _outArray);
void sample_wait_single(HDCAM hdcam, HDCAMWAIT hwait);
int readBufferSingle(const void *buf, int32 rowbytes, DCAM_PIXELTYPE type,
                     int32 width, int32 height);
int performAcquisition_infiniteLoop(HDCAM hdcam, HDCAMWAIT hwait,
                                    int totalframe, JNIEnv *_env,
                                    jshortArray _outArray,
                                    jobject framecounterObj,
                                    jmethodID increment_id, jshort *carray,
                                    jboolean isCopy);
int readBufferCpyJNI(const void *buf, int32 rowbytes, DCAM_PIXELTYPE type,
                     int32 width, int32 height, int new_c, JNIEnv *_env,
                 ...
                 double outTrigPeriod, int readoutSpeedIdx, int sensorModeIdx)
{
    ...
    imgobj.size_b_ =
        arraysize - (arraysize % (imgobj.nopixelX_ * imgobj.nopixelY_));
...
void run_acquisition(HDCAM hdcam, HDCAMWAIT hwait)
{
    ...
    std::cout << "from main thread start: " << std::this_thread::get_id()
              << std::endl;
    std::thread th([&]() {
        std::cout << "from child thread: " << std::this_thread::get_id()
                  << std::endl;
        orcajobobj.run();
    });

    th.join();
    std::cout << "from main thread end: " << std::this_thread::get_id()
              << std::endl;
...
void sample_wait_single(HDCAM hdcam, HDCAMWAIT hwait)
{
    ...
    {
        // std::cout << "pass 1 after get number of capture
        // imagecaptransferinfo.size: " << captransferinfo.size << ",
        // captransferinfo.nFrameCount: " << captransferinfo.nFrameCount << ",
        // captransferinfo.iKind: " << captransferinfo.iKind << ",
        // nNewestFrameIndex: " << captransferinfo.nNewestFrameIndex <<
        // std::endl;
    }
    ...
    int iRet = readBufferSingle(bufframe.buf, bufframe.rowbytes, bufframe.type,
                                bufframe.width, bufframe.height);
                                    ...
                                    jboolean isCopy)
{
    ...
    for (i = 0; i < totalframe; i++)
    {
        ...
        // a frame has come; read buffer and JNI transfer
        int iRet = readBufferCpyJNI(
            bufframe.buf, bufframe.rowbytes, bufframe.type, bufframe.width,
            bufframe.height, new_c, _env, _outArray, framecounterObj,
            increment_id, carray, isCopy);
        imgobj.framecounter++;
    }
                     ...
                     jmethodID increment_id, jshort *carray, jboolean isCopy)
{
    ...
    if (isCopy == true || imgobj.enableCriticalAccess == 0)
    {
        ...
        */
        ...
        //_env->SetShortArrayRegion(_outArray, (new_c * noelemperframe),
        // noelemperframe, imgobj.pImageArray1_);
    ...
    JNIEnv *env, jclass cls)
{
    ...
    std::cout << "nDevice isHAMAconnectedSDK4: " << imgobj.apiinit.iDeviceCount
              << std::endl;
    if (failed(imgobj.err))
    {
    ...
    // finalize DCAM-API
    dcamapi_uninit(); // recommended call dcamapi_uninit() when dcamapi_init()
                      // is called even if it failed.
    return iret;
}
    ...
    JNIEnv *env, jclass cls)
{
    ...
    char model[256];
    my_dcamdev_string(imgobj.err, (HDCAM)(intptr_t)iDevice, DCAM_IDSTR_MODEL,
                      model, sizeof(model));
    ...
    /*
    for (iDevice = 0; iDevice < nDevice; iDevice++) // only return info of first
    camera
    {
            char model[256];
            my_dcamdev_string(imgobj.err, (HDCAM)(intptr_t)iDevice,
    DCAM_IDSTR_MODEL, model, sizeof(model));
            // convert const char* to char*
            const char* outCStrcons = (const char*)model;

            // finalize DCAM-API
            dcamapi_uninit();	// recommended call dcamapi_uninit() when
    dcamapi_init() is called even if it failed.
    ...
    JNIEnv *env, jclass cls)
{
    ...
    {
        imgobj.iDevice =
            1; // select second in line camera if > 1 camera found //
               // umanager or HCImage will initialize first in line
    }
    else
    {
...
{ // MODEL, CAMERAID, BUS
    ...
    if (name == "MODEL")
    {
        ...
        my_dcamdev_string(imgobj.err, (HDCAM)(intptr_t)imgobj.iDevice,
                          DCAM_IDSTR_MODEL, result, sizeof(result));
    }
    else if (name == "CAMERAID")
    {
        std::cout << "cpp getting cameraid" << std::endl;
        my_dcamdev_string(imgobj.err, (HDCAM)(intptr_t)imgobj.iDevice,
                          DCAM_IDSTR_CAMERAID, result, sizeof(result));
    }
    else if (name == "BUS")
    {
        std::cout << "getting bus" << std::endl;
        my_dcamdev_string(imgobj.err, (HDCAM)(intptr_t)imgobj.iDevice,
                          DCAM_IDSTR_BUS, result, sizeof(result));
    }
    else
    {
    ...
    JNIEnv *env, jclass cls, jstring inJNIStr)
{
    ...
    {
        imgobj.err = dcamprop_getvalue(
            imgobj.hdcam, DCAM_IDPROP_CONVERSIONFACTOR_COEFF, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_CONVERSIONFACTOR_COEFF");
    }
    else if (name == "CONVERSIONFACTOR_OFFSET")
    {
        imgobj.err = dcamprop_getvalue(
            imgobj.hdcam, DCAM_IDPROP_CONVERSIONFACTOR_OFFSET, &res);
        assert(
            (!failed(imgobj.err))
            && "error dcamprop_getvalue DCAM_IDPROP_CONVERSIONFACTOR_OFFSET");
    }
    else if (name == "BITSPERCHANNEL")
    {
        imgobj.err =
            dcamprop_getvalue(imgobj.hdcam, DCAM_IDPROP_BITSPERCHANNEL, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_BITSPERCHANNEL");
    }
    else if (name == "OUTPUTTRIGGER_KIND")
    {
        imgobj.err = dcamprop_getvalue(imgobj.hdcam,
                                       DCAM_IDPROP_OUTPUTTRIGGER_KIND, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_OUTPUTTRIGGER_KIND");
    }
    else if (name == "OUTPUTTRIGGER_ACTIVE")
    {
        imgobj.err = dcamprop_getvalue(imgobj.hdcam,
                                       DCAM_IDPROP_OUTPUTTRIGGER_ACTIVE, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_OUTPUTTRIGGER_ACTIVE");
    }
    else if (name == "OUTPUTTRIGGER_SOURCE")
    {
        imgobj.err = dcamprop_getvalue(imgobj.hdcam,
                                       DCAM_IDPROP_OUTPUTTRIGGER_SOURCE, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_OUTPUTTRIGGER_SOURCE");
    }
    else if (name == "OUTPUTTRIGGER_POLARITY")
    {
        imgobj.err = dcamprop_getvalue(
            imgobj.hdcam, DCAM_IDPROP_OUTPUTTRIGGER_POLARITY, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_OUTPUTTRIGGER_POLARITY");
    }
    else if (name == "OUTPUTTRIGGER_DELAY")
    {
        imgobj.err = dcamprop_getvalue(imgobj.hdcam,
                                       DCAM_IDPROP_OUTPUTTRIGGER_DELAY, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_OUTPUTTRIGGER_DELAY");
    }
    else if (name == "OUTPUTTRIGGER_PERIOD")
    {
        imgobj.err = dcamprop_getvalue(imgobj.hdcam,
                                       DCAM_IDPROP_OUTPUTTRIGGER_PERIOD, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_OUTPUTTRIGGER_PERIOD");
    }
    else if (name == "INTERNALLINESPEED")
    {
        imgobj.err = dcamprop_getvalue(imgobj.hdcam,
                                       DCAM_IDPROP_INTERNALLINESPEED, &res);
        if (failed(imgobj.err))
        {
            res = 0;
        }
    ...
    {
        imgobj.err = dcamprop_getvalue(imgobj.hdcam,
                                       DCAM_IDPROP_INTERNAL_LINEINTERVAL, &res);
        if (failed(imgobj.err))
        {
            res = 0;
        }
    ...
    {
        imgobj.err = dcamprop_getvalue(imgobj.hdcam,
                                       DCAM_IDPROP_TIMING_READOUTTIME, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_TIMING_READOUTTIME");
    }
    else if (name == "TIMING_GLOBALEXPOSUREDELAY")
    {
        imgobj.err = dcamprop_getvalue(
            imgobj.hdcam, DCAM_IDPROP_TIMING_GLOBALEXPOSUREDELAY, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue "
                  "DCAM_IDPROP_TIMING_GLOBALEXPOSUREDELAY");
    }
    else if (name == "EXPOSURETIME")
    {
        imgobj.err =
            dcamprop_getvalue(imgobj.hdcam, DCAM_IDPROP_EXPOSURETIME, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_EXPOSURETIME");
    }
    else if (name == "READOUTSPEED")
    {
        imgobj.err =
            dcamprop_getvalue(imgobj.hdcam, DCAM_IDPROP_READOUTSPEED, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalue DCAM_IDPROP_READOUTSPEED");
    }
    else if (name == "SENSORMODE")
    {
        imgobj.err =
            dcamprop_getvalue(imgobj.hdcam, DCAM_IDPROP_SENSORMODE, &res);
        assert((!failed(imgobj.err))
               && "error dcamprop_getvalueDCAM_IDPROP_SENSORMODE");
    }
    else
    {
...
Java_fiji_plugin_imaging_1fcs_directCameraReadout_hamadcamsdk4_Hamamatsu_1DCAM_1SDK4_setParameterSDK4(
    ...
    jdouble outPeriod, jint readoutSpeedIdx, jint sensorModeIdx)
{
    int iRet;
    iRet = setParameter(exposureTime, Width, Height, Left, Top, Incamerabin,
                        acqmode, totalframe, size_b, arraysize, ouTriggerKind,
                        outDelay, outPeriod, readoutSpeedIdx, sensorModeIdx);
    return iRet;
}
        ...
        else
        {
            ...
            else
            {
                ...
                int iret = performAcquisition_infiniteLoop(
                    imgobj.hdcam, hwait, imgobj.totalframe_, env, outArray,
