...
void OrcaJob::run()
{
    ...
    case 2:
        sample_wait_and_calc(m_hdcam, m_hwait,
                             m_ptrimgobj->totalframe_); // acquisition
        break;
    case 3:
        // infinite loop
        break;
...
void OrcaJob::sample_wait_and_calc(HDCAM hdcam, HDCAMWAIT hwait, int32 nFrame)
{
    ...
    for (i = 0; i < nFrame; i++)
    {
        ...
        {
            int iRet =
                readBufferSingle(bufframe.buf, bufframe.rowbytes, bufframe.type,
                                 bufframe.width, bufframe.height);
        }
        else if (m_ptrimgobj->acqmode_ == 2)
        {
            int iRet =
                readBufferCpyJNI(bufframe.buf, bufframe.rowbytes, bufframe.type,
                                 bufframe.width, bufframe.height, new_c);
        }
        else
        {
...
double OrcaJob::readBufferSingle(const void *buf, int32 rowbytes,
                                 DCAM_PIXELTYPE type, int32 width, int32 height)
{
...
double OrcaJob::readBufferCpyJNI(const void *buf, int32 rowbytes,
                                 DCAM_PIXELTYPE type, int32 width, int32 height,
                                 int new_c)
{
    if (type != DCAM_PIXELTYPE_MONO16)
    ...
    (*penv)->SetShortArrayRegion(
        *poutArray, (new_c * noelemperframe), noelemperframe,
        m_ptrimgobj->pImageArray1_); // TODO: i previouslty use env instead of
                                     // _env; Test new version if it crashes
