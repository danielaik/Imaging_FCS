                                         ...
                                         int const num_v_coefs)
{
    ...
    REAL srn = my_sqrt(ridx * ridx - NA * NA);
    REAL psfz = 2 * (emLambda / 1e9) * ridx / (NA * NA);
    REAL szeff = my_sqrt(1.0 / (my_pow(sz, -2.0) + my_pow(psfz, -2.0)));
    REAL sdt = my_sqrt(D * x);
    ...
    REAL p00 = s;
    REAL p1x0 = ax;
    REAL pexpx0 = 2 * my_exp(- (p1x0 / p00) * (p1x0 / p00)) - 2;
    REAL perfx0 = 2 * p1x0 * my_erf(p1x0 / p00);
    REAL term_x = (p00 / SQRT_PI) * pexpx0 + perfx0;
    REAL p1y0 = ay;
    REAL pexpy0 = 2 * my_exp(- (p1y0 / p00) * (p1y0 / p00)) - 2;
    REAL perfy0 = 2 * p1y0 * my_erf(p1y0 / p00);
    REAL term_y = (p00 / SQRT_PI) * pexpy0 + perfy0;
    REAL volume3d = SQRT_PI * szeff * 4 * my_pow(ax * ay, 2) / (term_x * term_y);
    ...
    // Final Calculations
    REAL acf1 = (sum1 * 1e6) / (4 * my_pow(ax * ay, 2) / (volume3d / (SQRT_PI * sz)));
    REAL Dpspim = (sumd1 * 1e6) / (4 * my_pow(ax * ay, 2) / (volume3d / (SQRT_PI * sz)));
    REAL triplet = 1.0 + fTrip / (1.0 - fTrip) * my_exp(-x / tTrip);

    value[point_index] = (1.0 / N)
            * (((1 - parameters[5]) * acf1 + my_pow(parameters[18], 2) * parameters[5] * 0.0)
               / (my_pow(1 - parameters[5] + parameters[18] * parameters[5], 2)))
            * triplet
        + parameters[4];
