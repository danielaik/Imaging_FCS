...
https://gpufit.readthedocs.io/en/latest/ for more information. All codes
(headers and implementations), including CUDA codes, are deliberately placed
together for ease of compilation using nvcc compiler and for ease of
distribution. We added ACF and bleach correction fitting functions. Furthermore,
 ...
 *             p[3]: width (standard deviation; equal width in x and y
 * dimensions) p[4]: offset
 *
...
__device__ void calculate_gauss2d(REAL const *parameters, int const n_fits,
                                  ...
                                  int const num_v_coefs) // NEW
{
    ...
    REAL const argx =
        (point_index_x - p[1]) * (point_index_x - p[1]) / (2 * p[3] * p[3]);
    REAL const argy =
        (point_index_y - p[2]) * (point_index_y - p[2]) / (2 * p[3] * p[3]);
    ...
    current_derivative[1 * n_points] =
        p[0] * ex * (point_index_x - p[1]) / (p[3] * p[3]);
    current_derivative[2 * n_points] =
        p[0] * ex * (point_index_y - p[2]) / (p[3] * p[3]);
    current_derivative[3 * n_points] = ex * p[0]
        * ((point_index_x - p[1]) * (point_index_x - p[1])
           + (point_index_y - p[2]) * (point_index_y - p[2]))
        / (p[3] * p[3] * p[3]);
...
__device__ void calculate_linear1d(REAL const *parameters, int const n_fits,
                                   int const n_points, REAL *value,
                                   REAL *derivative, int const point_index,
                                   int const fit_index, int const chunk_index,
                                   char *user_info,
                                   ...
                                   int const num_v_coefs) // NEW
{
    ...
    // value
    // value[point_index] = parameters[0] + parameters[1]*x + parameters[2]*x*x
    // + parameters[3]*x*x*x + parameters[4]*x*x*x*x + parameters[5]*x*x*x*x*x +
    // parameters[6]*x*x*x*x*x*x;
 ...
 * =====================================================
 *
 * This function calculates the chi-square values for the weighted LSE
 * estimator.
 *
 * Parameters:
 *
 ...
 *
 * weight: An optional input vector of values for weighting the chi-square
 * values.
 *
 ...
 *        In this function it is not used. It can be used in functions
 * calculating other estimators than the LSE, such as MLE. It is passed into
 * this function to provide the same interface for all estimator functions.
 *
 * user_info: An input vector containing user information. (not used)
 *
...
__device__ void calculate_chi_square_lse(volatile REAL *chi_square,
                                         int const point_index,
                                         REAL const *data, REAL const *value,
                                         REAL const *weight, int *state,
                                         char *user_info,
                                         std::size_t const user_info_size)
{
 ...
 * ==================================================
 *
 * This function calculates the hessian matrix values of the weighted LSE
 * estimator. The calculation is performed based on previously calculated
 * fitting curve derivative values.
 *
 * Parameters:
 *
 * hessian: An output vector of values of the hessian matrix for each data
 * point.
 *
 * point_index: The data point index.
 *
 ...
 *
 * weight: An optional input vector of values for weighting the hessian matrix
 * values.
 *
 * user_info: An input vector containing user information. (not used)
 *
...
__device__ void calculate_hessian_lse(double *hessian, int const point_index,
                                      int const parameter_index_i,
                                      int const parameter_index_j,
                                      REAL const *data, REAL const *value,
                                      REAL const *derivative,
                                      REAL const *weight, char *user_info,
                                      std::size_t const user_info_size)
{
    if (weight)
    {
        *hessian += derivative[parameter_index_i]
            * derivative[parameter_index_j] * weight[point_index];
    }
    else
    {
        *hessian +=
 ...
 * Parameters:
 *
 * gradient: An output vector of values of the gradient vector for each data
 * point.
 *
 * point_index: The data point index.
 *
...
__device__ void calculate_gradient_lse(
    volatile REAL *gradient, int const point_index, int const parameter_index,
    REAL const *data, REAL const *value, REAL const *derivative,
    REAL const *weight, char *user_info, std::size_t const user_info_size)
{
 ...
 * value: An input vector of fitting curve values.
 *
 * weight: An input vector of values for weighting chi-square values. It is not
 * used in this function. It can be used in functions calculating other
 * estimators than the MLE, such as LSE.
 *
 * state: A pointer to a value which indicates whether the fitting process was
 * carreid out correctly or which problem occurred. It is set to 3 if a fitting
 * curve value is negative.
 *
 * user_info: An input vector containing user information. (not used)
 *
 * user_info_size: The number of elements in user_info. (not used)
...
__device__ void calculate_chi_square_mle(volatile REAL *chi_square,
                                         int const point_index,
                                         REAL const *data, REAL const *value,
                                         REAL const *weight, int *state,
                                         char *user_info,
                                         std::size_t const user_info_size)
{
    ...
    {
        chi_square[point_index] = 2
            * (deviation
               - data[point_index]
                   * std::log(value[point_index] / data[point_index]));
    }
    else
    {
 ...
 *             curve with respect to the fitting parameters for each data point.
 *
 * weight: An input vector of values for weighting hessian matrix values. It is
 * not used in this function. It can be used in functions calculating other
 * estimators than the MLE, such as LSE.
 *
 * user_info: An input vector containing user information. (not used)
 *
...
__device__ void calculate_hessian_mle(double *hessian, int const point_index,
                                      int const parameter_index_i,
                                      int const parameter_index_j,
                                      REAL const *data, REAL const *value,
                                      REAL const *derivative,
                                      ...
                                      std::size_t const user_info_size)
{
    *hessian += data[point_index] / (value[point_index] * value[point_index])
        * derivative[parameter_index_i] * derivative[parameter_index_j];
}
 ...
 *             curve with respect to the fitting parameters for each data point.
 *
 * weight: An input vector of values for weighting gradient vector values. It is
 * not used in this function. It can be used in functions calculating other
 * estimators than the MLE, such as LSE.
 *
 * user_info: An input vector containing user information. (not used)
...
-------------------------------------------------------------------------------------------------------
*/
__device__ void calculate_chi_square(int const estimator_id,
                                     volatile REAL *chi_square,
                                     int const point_index, REAL const *data,
                                     REAL const *value, REAL const *weight,
                                     int *state, char *user_info,
                                     std::size_t const user_info_size)
{
    switch (estimator_id)
    {
    case LSE:
        calculate_chi_square_lse(chi_square, point_index, data, value, weight,
                                 state, user_info, user_info_size);
        break;
    case MLE:
        calculate_chi_square_mle(chi_square, point_index, data, value, weight,
                                 state, user_info, user_info_size);
        break;
    default:
        break;
...
__device__ void calculate_gradient(int const estimator_id,
                                   volatile REAL *gradient,
                                   int const point_index,
                                   int const parameter_index, REAL const *data,
                                   REAL const *value, REAL const *derivative,
                                   REAL const *weight, char *user_info,
                                   std::size_t const user_info_size)
{
    switch (estimator_id)
    {
    case LSE:
        calculate_gradient_lse(gradient, point_index, parameter_index, data,
                               value, derivative, weight, user_info,
                               user_info_size);
        break;
    case MLE:
        calculate_gradient_mle(gradient, point_index, parameter_index, data,
                               value, derivative, weight, user_info,
                               user_info_size);
        break;
    default:
...
__device__ void calculate_hessian(int const estimator_id, double *hessian,
                                  int const point_index,
                                  int const parameter_index_i,
                                  int const parameter_index_j, REAL const *data,
                                  REAL const *value, REAL const *derivative,
                                  REAL const *weight, char *user_info,
                                  std::size_t const user_info_size)
{
    ...
    {
    case LSE:
        calculate_hessian_lse(hessian, point_index, parameter_index_i,
                              parameter_index_j, data, value, derivative,
                              weight, user_info, user_info_size);
        break;
    case MLE:
        calculate_hessian_mle(hessian, point_index, parameter_index_i,
                              parameter_index_j, data, value, derivative,
                              weight, user_info, user_info_size);
        break;
    default:
...
*/
/*
void configure_model(ModelID const model_id, int & n_parameters, int &
n_dimensions);
...
__global__ void
cuda_calc_curve_values(REAL const *parameters, int const n_fits,
                       int const n_points, int const n_parameters,
                       int const *finished, REAL *values, REAL *derivatives,
                       int const n_fits_per_block, int const n_blocks_per_fit,
                       ModelID const model_id, int const chunk_index,
                       char *user_info, std::size_t const user_info_size,
                       int const num_v_coefs) // NEW
{
    int const fit_in_block = threadIdx.x / n_points;
    int const fit_index =
        blockIdx.x * n_fits_per_block / n_blocks_per_fit + fit_in_block;
    int const fit_piece = blockIdx.x % n_blocks_per_fit;
    int const point_index =
        threadIdx.x - fit_in_block * n_points + fit_piece * blockDim.x;
    ...
    calculate_model(model_id, current_parameters, n_fits, n_points,
                    current_values, current_derivatives, point_index, fit_index,
                    chunk_index, user_info, user_info_size, num_v_coefs);
}
 ...
 *               on the shared memory of the GPU. The size of this vector must
 * be a power of two. Use zero padding to extend it to the next highest power of
 * 2 greater than the number of elements.
 *
 ...
 * an other __device__ function. When calling the function, the blocks and
 * threads of the __global__ function must be set up correctly, as shown in the
...
__device__ void sum_up_floats(volatile REAL *shared_array, int const size)
{
    ...
    while (current_n_points)
    {
        ...
        {
            shared_array[point_index] +=
                shared_array[point_index + current_n_points];
        }
        current_n_points >>= 1;
        __syncthreads();
    }
...
__global__ void cuda_sum_chi_square_subtotals(REAL *chi_squares,
                                              REAL const *subtotals,
                                              int const n_blocks_per_fit,
                                              int const n_fits,
                                              int const *finished)
 ...
 * currently calculated chi-square values are lower than chi-square values
 * calculated in the previous iteration and sets the iteration_failed flags.
 *
 * Parameters:
 *
 * iteration_failed: An output vector of flags which indicate whether the
 * fitting process improved the fit in the last iteration. If yes it is set to 0
 * otherwise to 1.
 *
 * chi_squares: An input vector of chi-square values for multiple fits.
 *
 * prev_chi_squares: An input vector of chi-square values for multiple fits
...
__global__ void cuda_check_fit_improvement(int *iteration_failed,
                                           REAL const *chi_squares,
                                           REAL const *prev_chi_squares,
                                           int const n_fits,
                                           int const *finished)
{
    ...
    // chi_squares[index] can be NaN which compares to false with any other
    // number
    bool const chi_square_decreased =
        (chi_squares[index] < prev_chi_squares[index]);
 ...
 * parameter estimator_id. The estimator function calculates the chi-square
 * values. The calcluation is performed for multiple fits in parallel.
 *
 * Parameters:
 *
 ...
 *
 * weights: An input vector of values for weighting chi-square, gradient and
 * hessian, while using LSE
 *
 * n_points: The number of data points per fit.
 *
 * n_fits: The number of fits.
...
__global__ void cuda_calculate_chi_squares(
    REAL *chi_squares, int *states, REAL const *data, REAL const *values,
    REAL const *weights, int const n_points, int const n_fits,
    int const estimator_id, int const *finished, int const n_fits_per_block,
    char *user_info, std::size_t const user_info_size)
{
    ...
    int const fit_index =
        blockIdx.x * n_fits_per_block + fit_in_block - fit_piece * n_fits;
    int const point_index =
        threadIdx.x - fit_in_block * shared_size + fit_piece * shared_size;
    int const first_point = fit_index * n_points;
    ...
    if (point_index < n_points)
    {
        calculate_chi_square(estimator_id, shared_chi_square, point_index,
                             current_data, current_value, current_weight,
                             current_state, user_info, user_info_size);
    }
 ...
 *
 * skip: An input vector which allows the calculation to be skipped for single
 * fits.
 *
 * finished: An input vector which allows the calculation to be skipped for
 * single fits.
...
__global__ void
cuda_sum_gradient_subtotals(REAL *gradients, REAL const *subtotals,
                            int const n_blocks_per_fit, int const n_fits,
                            int const n_parameters, int const *skip,
                            int const *finished)
{
    int const index = blockIdx.x * blockDim.x + threadIdx.x;
 ...
 * n_parameters: The number of fitting curve parameters.
 *
 * n_parameters_to_fit: The number of fitting curve parameters, that are not
 * held fixed.
 *
 * parameters_to_fit_indices: An input vector of indices of fitting curve
...
__global__ void cuda_calculate_gradients(
    REAL *gradients, REAL const *data, REAL const *values,
    REAL const *derivatives, REAL const *weights, int const n_points,
    int const n_fits, int const n_parameters, int const n_parameters_to_fit,
    int const *parameters_to_fit_indices, int const estimator_id,
    int const *finished, int const *skip, int const n_fits_per_block,
    char *user_info, std::size_t const user_info_size)
{
    int const shared_size = blockDim.x / n_fits_per_block;
    int const fit_in_block = threadIdx.x / shared_size;
    ...
    for (int parameter_index = 0; parameter_index < n_parameters_to_fit;
         parameter_index++)
    {
        if (point_index < n_points)
        {
            int const derivative_index =
                parameters_to_fit_indices[parameter_index] * n_points
                + point_index;

            calculate_gradient(estimator_id, shared_gradient, point_index,
                               derivative_index, current_data, current_value,
                               current_derivative, current_weight, user_info,
                               user_info_size);
        }
        sum_up_floats(shared_gradient + fit_piece * shared_size, shared_size);
        gradients[(fit_index * n_parameters_to_fit + parameter_index)
                  + fit_piece * n_fits * n_parameters_to_fit] =
...
__global__ void cuda_calculate_hessians(
    REAL *hessians, REAL const *data, REAL const *values,
    REAL const *derivatives, REAL const *weights, int const n_fits,
    int const n_points, int const n_parameters, int const n_parameters_to_fit,
    int const *parameters_to_fit_indices, int const estimator_id,
    int const *skip, int const *finished, char *user_info,
    std::size_t const user_info_size)
{
    int const n_unique_values =
        n_parameters_to_fit * (n_parameters_to_fit + 1) / 2;
    int const n_fits_per_block = blockDim.x * gridDim.y / n_unique_values;

    int const fit_in_block = (gridDim.y == 1)
    ...
    int const first_point = fit_index * n_points;
    int const parameter_index = (blockIdx.y * blockDim.x + threadIdx.x)
        - fit_in_block * n_unique_values;
    ...
    int const parameter_index_i = n_parameters_to_fit - 1.
        - std::floor(.5
                     * (std::sqrt(-8. * (parameter_index - n_parameters_to_fit)
                                  + 4. * n_parameters_to_fit
                                      * (n_parameters_to_fit - 1.)
                                  - 7.)
                        - 1.));

    int const parameter_index_j = parameter_index + parameter_index_i
        - parameter_index_i
            * (n_parameters_to_fit - (parameter_index_i - 1) / 2.);

    REAL *current_hessian =
        &hessians[fit_index * n_parameters_to_fit * n_parameters_to_fit];
    REAL const *current_data = &data[first_point];
    REAL const *current_weight = weights ? &weights[first_point] : NULL;
    REAL const *current_derivative = &derivatives[first_point * n_parameters];
    REAL const *current_value = &values[first_point];

    int const hessian_index_ij =
        parameter_index_i * n_parameters_to_fit + parameter_index_j;
    int const hessian_index_ji =
        parameter_index_j * n_parameters_to_fit + parameter_index_i;
    int const derivative_index_i =
        parameters_to_fit_indices[parameter_index_i] * n_points;
    int const derivative_index_j =
        parameters_to_fit_indices[parameter_index_j] * n_points;

    double sum = 0.0;
    for (int point_index = 0; point_index < n_points; point_index++)
    {
        calculate_hessian(
            estimator_id, &sum, point_index, derivative_index_i + point_index,
            derivative_index_j + point_index, current_data, current_value,
            current_derivative, current_weight, user_info, user_info_size);
    }
 ...
 * ====================================================
 *
 * This function midifies the diagonal elements of the hessian matrices by
 * multiplying them by the factor (1+ lambda). This operation controls the step
 * widths of the iteration. If the last iteration failed, befor modifying the
 * hessian, the diagonal elements of the hessian are calculated back to
 * represent unmodified values.
 *
 * hessians: An input and output vector of hessian matrices, which are modified
 * by the lambda values.
 *
 * lambdas: An input vector of values for modifying the hessians.
 *
 * n_parameters: The number of fitting curve parameters.
 *
 * iteration_failed: An input vector which indicates whether the previous
 * iteration failed.
 *
 * finished: An input vector which allows the calculation to be skipped for
 * single fits.
...
__global__ void cuda_modify_step_widths(REAL *hessians, REAL const *lambdas,
                                        REAL *scaling_vectors,
                                        unsigned int const n_parameters,
                                        int const *iteration_failed,
                                        int const *finished,
                                        int const n_fits_per_block)
{
    ...
    if (iteration_failed[fit_index])
    {
        hessian[diagonal_index] -=
            scaling_vector[parameter_index] * lambda / 10.;
    }
 ...
 * ===================================================
 *
 * This function stores the fitting curve parameter values in prev_parameters
...
__global__ void cuda_update_parameters(REAL *parameters, REAL *prev_parameters,
                                       REAL const *deltas,
                                       int const n_parameters_to_fit,
                                       int const *parameters_to_fit_indices,
                                       int const *finished,
                                       int const n_fits_per_block)
{
    ...
    current_prev_parameters[parameter_index] =
        current_parameters[parameter_index];
    ...
    current_parameters[parameters_to_fit_indices[parameter_index]] +=
        current_deltas[parameter_index];
}
...
__global__ void cuda_update_state_after_solving(int const n_fits,
 ...
 * =======================================================
 *
 * This function checks after each iteration whether the fits are converged or
 * not. It also checks whether the set maximum number of iterations is reached.
 *
 * Parameters:
 *
 * finished: An input and output vector which allows the calculation to be
 ...
 *         number of iterations is reached without converging, it is set to 1.
 * If the fit converged it keeps its initial value of 0.
 *
 * chi_squares: An input vector of chi-square values for multiple fits. Used for
 * the convergence check.
 *
 * prev_chi_squares: An input vector of chi-square values for multiple fits
 * calculated in the previous iteration. Used for the convergence check.
 *
 ...
 *
 * // NEW
 * num_v_coefs: number of coefficients to be fitted. Mainly for linear_1d, ie.
...
__global__ void cuda_check_for_convergence(int *finished, REAL const tolerance,
                                           int *states, REAL const *chi_squares,
                                           REAL const *prev_chi_squares,
                                           int const iteration,
                                           int const max_n_iterations,
                                           int const n_fits)
 ...
 *
 * finished: An input and output vector which allows the evaluation to be
 * skipped for single fits
 *
...
__global__ void cuda_evaluate_iteration(int *all_finished, int *n_iterations,
                                        int *finished, int const iteration,
                                        int const *states, int const n_fits)
{
    int const fit_index = blockIdx.x * blockDim.x + threadIdx.x;
 ...
 *
 * chi_squares: An input and output vector of chi-square values for multiple
 * fits.
 *
 * prev_chi_squares: An input and output vector of chi-square values for
 * multiple fits calculated in the previous iteration.
 *
...
__global__ void cuda_prepare_next_iteration(
    REAL *lambdas, REAL *chi_squares, REAL *prev_chi_squares, REAL *parameters,
    REAL const *prev_parameters, int const n_fits, int const n_parameters)
{
    int const fit_index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    else
    {
        ...
        {
            parameters[fit_index * n_parameters + iparameter] =
                prev_parameters[fit_index * n_parameters + iparameter];
        }
    }
}
/* -------------------------------------------------------------------------------------------------------
...
void Info::set_blocks_per_fit()
{
    ...
    if (power_of_two_n_points_ > max_threads_)
    {
        ...
        do
        {
            n_blocks_per_fit_ *= 2;
            enough_threads =
                power_of_two_n_points_ / n_blocks_per_fit_ < max_threads_;
...
{
    int one_fit_memory = sizeof(REAL)
            * (1 * n_points_ // values
    ...
    {
        one_fit_memory += sizeof(REAL) * n_parameters_to_fit_
            * n_blocks_per_fit_; // subtotals
    }
...
#ifdef USE_CUBLAS
    one_fit_memory += sizeof(REAL)
            * (2 // pointer to decomposed hessian, pointer to delta
               + 1 * n_parameters_to_fit_
                   * n_parameters_to_fit_) // decomposed hessian
        + sizeof(int) * (1 * n_parameters_to_fit_); // pivot vector
#endif // USE_CUBLAS
    ...
    tmp_chunk_size =
        (std::min)(tmp_chunk_size, max_blocks_ / n_blocks_per_fit_);
...
void Info::configure()
{
    ...
    // TODO NOTE: to address the 'too many resources requested' error due to
    // complex calculate_acf1D function. increasing the power_of_two_n_points_
    // by 4 times would reduce the number of threads required in
    // Info::set_fits_per_block function Info::set_blocks_per_fit functions
    // function would also be updated accordingly.
...
public:
    ...
    void init(int const chuk_size, int const chunk_index, REAL const *data,
              REAL const *weights, REAL const *initial_parameters,
              std::vector<int> const &parameters_to_fit_indices, int *states,
              REAL *chi_squares, int *n_iterations);
...
GPUData::GPUData(Info const &info)
    ...
    data_((info_.data_location_ == HOST)
              ? info_.max_chunk_size_ * info_.n_points_
              : 0)
    , weights_((info_.use_weights_ && info_.data_location_ == HOST)
                   ? info_.n_points_ * info_.max_chunk_size_
                   : 0)
    , parameters_((info_.data_location_ == HOST)
                      ? info_.max_chunk_size_ * info_.n_parameters_
                      : 0)
    , user_info_((info_.data_location_ == HOST) ? info_.user_info_size_ : 0)
    ,
    ...
    chi_squares_((info_.data_location_ == HOST) ? info_.max_chunk_size_ : 0)
    ,

    prev_chi_squares_(info_.max_chunk_size_)
    , gradients_(info_.max_chunk_size_ * info_.n_parameters_to_fit_)
    , hessians_(info_.max_chunk_size_ * info_.n_parameters_to_fit_
    ...
    subtotals_((info_.n_blocks_per_fit_ > 1) ? info_.max_chunk_size_
                       * info_.n_parameters_to_fit_ * info_.n_blocks_per_fit_
                                             : 0)
    ,
    ...
    states_((info_.data_location_ == HOST) ? info_.max_chunk_size_ : 0)
    ,
    ...
    n_iterations_((info_.data_location_ == HOST) ? info_.max_chunk_size_ : 0)
    ,

    solution_info_(info_.max_chunk_size_)

#ifdef USE_CUBLAS
    , decomposed_hessians_(info_.max_chunk_size_ * info_.n_parameters_to_fit_
                           * info_.n_parameters_to_fit_)
...
void GPUData::init(int const chunk_size, int const chunk_index,
                   REAL const *const data, REAL const *const weights,
                   REAL const *const initial_parameters,
                   std::vector<int> const &parameters_to_fit_indices,
                   int *states, REAL *chi_squares, int *n_iterations)
{
    ...
    if (info_.data_location_ == HOST)
    {
        ...
        if (info_.use_weights_)
            write(weights_,
                  weights
                      + chunk_index_ * info_.max_chunk_size_ * info_.n_points_,
                  chunk_size_ * info_.n_points_);
    }
    else if (info_.data_location_ == DEVICE)
    {
        data_.assign(data
                     + chunk_index_ * info_.max_chunk_size_ * info_.n_points_);
        parameters_.assign(initial_parameters
                           + chunk_index_ * info_.max_chunk_size_
                               * info_.n_parameters_);
        if (info_.use_weights_)
            weights_.assign(weights
                            + chunk_index_ * info_.max_chunk_size_
                                * info_.n_points_);
        states_.assign(states + chunk_index_ * info_.max_chunk_size_);
        chi_squares_.assign(chi_squares + chunk_index_ * info_.max_chunk_size_);
        n_iterations_.assign(n_iterations
                             + chunk_index_ * info_.max_chunk_size_);
    }
...
void GPUData::read(bool *dst, int const *src)
{
    int int_dst = 0;
    CUDA_CHECK_STATUS(
        cudaMemcpy(&int_dst, src, sizeof(int), cudaMemcpyDeviceToHost));
...
{
    CUDA_CHECK_STATUS(
        cudaMemcpy(dst, src, count * sizeof(REAL), cudaMemcpyHostToDevice));
}
...
{
    CUDA_CHECK_STATUS(
        cudaMemcpy(dst, src, count * sizeof(char), cudaMemcpyHostToDevice));
}

void GPUData::copy(REAL *dst, REAL const *src, std::size_t const count)
{
    CUDA_CHECK_STATUS(
        cudaMemcpy(dst, src, count * sizeof(REAL), cudaMemcpyDeviceToDevice));
}
...
__global__ void cuda_point_to_data_sets(REAL **pointer_to_pointers,
                                        REAL *pointer,
...
void GPUData::point_to_data_sets()
{
    ...
    threads.x =
        static_cast<unsigned int>(std::min(info_.max_chunk_size_, max_threads));
    blocks.x = static_cast<unsigned int>(
        std::ceil(REAL(info_.max_chunk_size_) / REAL(threads.x)));

    cuda_point_to_data_sets<<<blocks, threads>>>(
        pointer_decomposed_hessians_, decomposed_hessians_,
        info_.max_chunk_size_,
        info_.n_parameters_to_fit_ * info_.n_parameters_to_fit_);

    cuda_point_to_data_sets<<<blocks, threads>>>(pointer_deltas_, deltas_,
                                                 info_.max_chunk_size_,
                                                 info_.n_parameters_to_fit_);
}
 ...
 * ===============================
 *
 * This function solves a set of linear equations using the Gauss-Jordan
 * elimination method. Considering a set of N equations with N unknowns, this
 * can be written in matrix form as an NxN matrix of coefficients and a Nx1
 * column vector of right-hand side values.
 *
 * For example, consider the following problem with 3 equations and 3 unknowns
 * (N=3):
 *
 *   A x + B y + C z = MM
 *   D x + E y + F z = NN
 ...
 *   [ G H I ] [ z ] = [ PP ]
 *
 * or, [A]*[X] = [B] where [A] is the matrix of coefficients and [B] is the
 * vector of right-hand side values.
 *
 * The Gauss Jordan elimiation method solves the system of equations in the
 * following manner.  First, we form the augmented matrix (A|B):
 *
 *   [ A B C | MM ]
 *   [ D E F | NN ]
 *   [ G H I | PP ]
 *
 * and then the augmented matrix is manipulated until its left side has the
 * reduced row-echelon form.  That is to say that any individual row may be
 * multiplied by a scalar factor, and any linear combination of rows may be
 * added to another row.  Finally, two rows may be swapped without affecting the
 * solution.
 *
 * When the manipulations are complete and the left side of the matrix has the
 * desired form, the right side then corresponds to the solution of the system.
 *
 *
 * Description of the cuda_gaussjordan function
 * ============================================
 *
 * This algorithm is designed to perform many solutions of the Gauss Jordan
 * elimination method in parallel.  One limitation of the algorithm implemented
 * here is that for each solution the number of equations and unknowns (N) must
 * be identical.
 *
 * Parameters:
 *
 * alpha: Coefficients matrices.  The matrix of coefficients for a single
 * solution is a vector of NxN, where N is the number of equations.  This array
 * stores the coefficients for the entire set of M input problems, concatenated
 * end to end, and hence the total size of the array is MxNxN.
 *
 * beta: Vector of right hand side values, concatenated together for all input
 * problems. For a set of M inputs, the size of the vector is MxN.  Upon
 * completion, this vector contains the results vector X for each solution.
 *
 * skip_calculation: An input vector which allows the calculation to be skipped
 * for a particular solution.  For a set of M inputs, the size of this vector is
 * M.
 *
 * singular: An output vector used to report whether a given solution is
 * singular.  For a set of M inputs, this vector has size M.  Memory needs to be
 * allocated by the calling the function.
 *
 * n_equations: The number of equations and unknowns for a single solution. This
 * is equal to the size N.
 *
 ...
 *
 * When calling the function, the blocks and threads must be set up correctly,
 * as well as the shared memory space, as shown in the following example code.
 *
 *   dim3  threads(1, 1, 1);
 *   dim3  blocks(1, 1, 1);
 *
 ...
 *
 *   int * singular;
 *   CUDA_CHECK_STATUS(cudaMalloc((void**)&singular, n_solutions *
 * sizeof(int)));
 *
...
__global__ void cuda_gaussjordan(REAL *delta, REAL const *beta,
                                 REAL const *alpha, int const *skip_calculation,
                                 int *singular, std::size_t const n_equations,
                                 std::size_t const n_equations_pow2)
{
    extern __shared__ REAL
        extern_array[]; // shared memory between threads of a single block,
    // used for storing the calculation_matrix, the
    // abs_row vector, and the abs_row_index vector

    // In this routine we will store the augmented matrix (A|B), referred to
    // here as the calculation matrix in a shared memory space which is visible
    // to all threads within a block.  Also stored in shared memory are two
    // vectors which are used to find the largest element in each row (the
    // pivot).  These vectors are called abs_row and abs_row_index.
    //
    ...
    int const n_col =
        blockDim.x; // number of columns in calculation matrix (=threads.x)
    int const n_row =
        blockDim.y; // number of rows in calculation matrix (=threads.y)
    int const alpha_size = blockDim.y
        * blockDim
              .y; // number of entries in alpha matrix for one solution (NxN)
    ...
    REAL *abs_row = extern_array
        + n_equations
            * (n_equations
               + 1); // abs_row is located after the calculation_matrix
    // within the shared memory

    int *abs_row_index =
        (int *)(abs_row
                + n_equations_pow2); // abs_row_index is located after abs_row
    //
    ...
    // initialize abs_row and abs_row_index, using only the threads on the
    // diagonal
    if (col_index == row_index)
    {
        abs_row[col_index + (n_equations_pow2 - n_equations)] = 0.0;
        abs_row_index[col_index + (n_equations_pow2 - n_equations)] =
            col_index + (n_equations_pow2 - n_equations);
    }

    // initialize the calculation_matrix (alpha and beta, concatenated, for one
    // solution)
    if (col_index != n_equations)
        calculation_matrix[row_index * n_col + col_index] =
            alpha[solution_index * alpha_size + row_index * n_equations
                  + col_index];
    else
        calculation_matrix[row_index * n_col + col_index] =
            beta[solution_index * n_equations + row_index];
    ...
    for (int current_row = 0; current_row < n_equations; current_row++)
    {
        ...
        if (row_index == current_row && col_index != n_equations)
        {
            ...
            abs_row[col_index] =
                abs(calculation_matrix[row_index * n_col + col_index]);
            ...
            // find the largest absolute value in the current row and write its
            // index in abs_row_index[0]
            for (int n = 2; n <= n_equations_pow2; n = n * 2)
            {
                if (col_index < (n_equations_pow2 / n))
                {
                    if (abs_row[abs_row_index[col_index]]
                        < abs_row[abs_row_index[col_index
                                                + (n_equations_pow2 / n)]])
                    {
                        abs_row_index[col_index] =
                            abs_row_index[col_index + (n_equations_pow2 / n)];
                    }
                }
            }
        }
        ...
        // singularity check - if all values in the row are zero, no solution
        // exists
        if (row_index == current_row && col_index != n_equations)
        {
        ...
        if (row_index == current_row)
        {
            calculation_matrix[row_index * n_col + col_index] =
                calculation_matrix[row_index * n_col + col_index]
                / calculation_matrix[row_index * n_col + abs_row_index[0]];
        }

        __syncthreads();

        // The value of the largest element of the current row was found, and
        // then current row was divided by this value such that the largest
        // value of the current row is equal to one.
        //
        // Next, the matrix is manipulated to reduce to zero all other entries
        // in the column in which the largest value was found.   To do this, the
        // values in the current row are scaled appropriately and substracted
        // from the other rows of the matrix.
        //
        // For each element of the matrix that is not in the current row,
        // calculate the value to be subtracted and let each thread store this
        // value in the scalar variable p.

        p = calculation_matrix[current_row * n_col + col_index]
            * calculation_matrix[row_index * n_col + abs_row_index[0]];
        __syncthreads();
        ...
        {
            calculation_matrix[row_index * n_col + col_index] =
                calculation_matrix[row_index * n_col + col_index] - p;
        }
        __syncthreads();
    }

    // At this point, if the solution exists, the calculation matrix has been
    // reduced to the identity matrix on the left side, and the solution vector
    // on the right side.  However we have not swapped rows during the
    // procedure, so the identity matrix is out of order.
    //
    ...
    //   [  1  0  0 |  3 ]
    //
    // Which needs to be re-arranged to obtain the correct solution vector.  In
    // the final step, each thread checks to see if its value equals 1, and if
    // so it assigns the value in its rightmost column to the appropriate entry
    // in the beta vector.  The solution is stored in beta upon completetion.

    if (col_index != n_equations
        && calculation_matrix[row_index * n_col + col_index] == 1)
        delta[n_row * solution_index + col_index] =
            calculation_matrix[row_index * n_col + n_equations];
...
LMFit::LMFit(REAL const *const data, REAL const *const weights, Info &info,
             REAL const *const initial_parameters,
             int const *const parameters_to_fit, char *const user_info,
...
void LMFit::get_results(GPUData const &gpu_data, int const n_fits)
{
    if (info_.data_location_ == HOST)
    {
        output_parameters_ = gpu_data.parameters_.copy(
            n_fits * info_.n_parameters_, output_parameters_);
...
void LMFit::run(REAL const tolerance)
{
    ...
    while (n_fits_left_ > 0)
    {
        ...
        gpu_data.init(chunk_size_, ichunk_, data_, weights_,
                      initial_parameters_, parameters_to_fit_indices_,
                      output_states_, output_chi_squares_,
                      output_n_iterations_);

        LMFitCUDA lmfit_cuda(tolerance, info_, gpu_data, chunk_size_);
...
void LMFitCUDA::solve_equation_systems_lup()
{
    ...
    gpu_data_.copy(gpu_data_.decomposed_hessians_, gpu_data_.hessians_,
                   n_fits_ * info_.n_parameters_to_fit_
                       * info_.n_parameters_to_fit_);

    // decompose hessians
    cublasStatus_t lu_status_decopmposition = DECOMPOSE_LUP(
        gpu_data_.cublas_handle_, info_.n_parameters_to_fit_,
        gpu_data_.pointer_decomposed_hessians_, info_.n_parameters_to_fit_,
        gpu_data_.pivot_vectors_, gpu_data_.solution_info_, n_fits_);

    // initialize deltas with values of gradients
    gpu_data_.copy(gpu_data_.deltas_, gpu_data_.gradients_,
                   n_fits_ * info_.n_parameters_to_fit_);
    ...
    cublasStatus_t lu_status_solution = SOLVE_LUP(
        gpu_data_.cublas_handle_, CUBLAS_OP_N, info_.n_parameters_to_fit_, 1,
        (REAL const **)(gpu_data_.pointer_decomposed_hessians_.data()),
        info_.n_parameters_to_fit_, gpu_data_.pivot_vectors_,
        gpu_data_.pointer_deltas_, info_.n_parameters_to_fit_, &solution_info,
        n_fits_);
}
#else
void LMFitCUDA::solve_equation_systems_gj()
{
    ...
    cuda_gaussjordan<<<blocks, threads, shared_size>>>(
        gpu_data_.deltas_, gpu_data_.gradients_, gpu_data_.hessians_,
        gpu_data_.finished_, gpu_data_.solution_info_,
        info_.n_parameters_to_fit_, n_parameters_pow2);
    CUDA_CHECK_STATUS(cudaGetLastError());
}
#endif
...
void LMFitCUDA::update_states()
{
    ...
    cuda_update_state_after_solving<<<blocks, threads>>>(
        n_fits_, gpu_data_.solution_info_, gpu_data_.finished_,
        gpu_data_.states_);
    CUDA_CHECK_STATUS(cudaGetLastError());
}
...
void LMFitCUDA::scale_hessians()
{
    ...
    cuda_modify_step_widths<<<blocks, threads>>>(
        gpu_data_.hessians_, gpu_data_.lambdas_, gpu_data_.scaling_vectors_,
        info_.n_parameters_to_fit_, gpu_data_.iteration_failed_,
        gpu_data_.finished_, info_.n_fits_per_block_);
    CUDA_CHECK_STATUS(cudaGetLastError());
}
...
void LMFitCUDA::update_parameters()
{
    ...
    cuda_update_parameters<<<blocks, threads>>>(
        gpu_data_.parameters_, gpu_data_.prev_parameters_, gpu_data_.deltas_,
        info_.n_parameters_to_fit_, gpu_data_.parameters_to_fit_indices_,
        gpu_data_.finished_, info_.n_fits_per_block_);
    CUDA_CHECK_STATUS(cudaGetLastError());
}
...
void LMFitCUDA::calc_curve_values()
{
    ...
    threads.x =
        info_.n_points_ * info_.n_fits_per_block_ / info_.n_blocks_per_fit_;
    ...
    cuda_calc_curve_values<<<blocks, threads>>>(
        gpu_data_.parameters_, n_fits_, info_.n_points_, info_.n_parameters_,
        gpu_data_.finished_, gpu_data_.values_, gpu_data_.derivatives_,
        info_.n_fits_per_block_, info_.n_blocks_per_fit_, info_.model_id_,
        gpu_data_.chunk_index_, gpu_data_.user_info_, info_.user_info_size_,
...
void LMFitCUDA::calc_chi_squares()
{
    ...
    REAL *chi_squares = info_.n_blocks_per_fit_ > 1 ? gpu_data_.subtotals_
                                                    : gpu_data_.chi_squares_;

    cuda_calculate_chi_squares<<<blocks, threads, shared_size>>>(
        chi_squares, gpu_data_.states_, gpu_data_.data_, gpu_data_.values_,
        gpu_data_.weights_, info_.n_points_, n_fits_, info_.estimator_id_,
        gpu_data_.finished_, info_.n_fits_per_block_, gpu_data_.user_info_,
    ...
    {
        cuda_sum_chi_square_subtotals<<<blocks, threads>>>(
            gpu_data_.chi_squares_, gpu_data_.subtotals_,
            info_.n_blocks_per_fit_, n_fits_, gpu_data_.finished_);
        CUDA_CHECK_STATUS(cudaGetLastError());
    }

    cuda_check_fit_improvement<<<blocks, threads>>>(
        gpu_data_.iteration_failed_, gpu_data_.chi_squares_,
        gpu_data_.prev_chi_squares_, n_fits_, gpu_data_.finished_);
    CUDA_CHECK_STATUS(cudaGetLastError());
}
...
void LMFitCUDA::calc_gradients()
{
    ...
    REAL *gradients = info_.n_blocks_per_fit_ > 1 ? gpu_data_.subtotals_
                                                  : gpu_data_.gradients_;

    cuda_calculate_gradients<<<blocks, threads, shared_size>>>(
        gradients, gpu_data_.data_, gpu_data_.values_, gpu_data_.derivatives_,
        gpu_data_.weights_, info_.n_points_, n_fits_, info_.n_parameters_,
        info_.n_parameters_to_fit_, gpu_data_.parameters_to_fit_indices_,
        info_.estimator_id_, gpu_data_.finished_, gpu_data_.iteration_failed_,
        info_.n_fits_per_block_, gpu_data_.user_info_, info_.user_info_size_);
    CUDA_CHECK_STATUS(cudaGetLastError());
    ...
    if (info_.n_blocks_per_fit_ > 1)
    {
        ...
        cuda_sum_gradient_subtotals<<<blocks, threads>>>(
            gpu_data_.gradients_, gpu_data_.subtotals_, info_.n_blocks_per_fit_,
            n_fits_, info_.n_parameters_to_fit_, gpu_data_.iteration_failed_,
            gpu_data_.finished_);
        CUDA_CHECK_STATUS(cudaGetLastError());
    }
}
...
void LMFitCUDA::calc_hessians()
{
    ...
    blocks.y = temp_threads_x / info_.max_threads_
        + int((temp_threads_x % info_.max_threads_) > 0);

    blocks.x = n_fits_ / n_hessians_per_block
        + int((n_fits_ % n_hessians_per_block) > 0);

    cuda_calculate_hessians<<<blocks, threads>>>(
        gpu_data_.hessians_, gpu_data_.data_, gpu_data_.values_,
        gpu_data_.derivatives_, gpu_data_.weights_, n_fits_, info_.n_points_,
        info_.n_parameters_, info_.n_parameters_to_fit_,
        gpu_data_.parameters_to_fit_indices_, info_.estimator_id_,
        gpu_data_.iteration_failed_, gpu_data_.finished_, gpu_data_.user_info_,
        info_.user_info_size_);
    CUDA_CHECK_STATUS(cudaGetLastError());
}
...
void LMFitCUDA::evaluate_iteration(int const iteration)
{
    ...
    cuda_check_for_convergence<<<blocks, threads>>>(
        gpu_data_.finished_, tolerance_, gpu_data_.states_,
        gpu_data_.chi_squares_, gpu_data_.prev_chi_squares_, iteration,
        info_.max_n_iterations_, n_fits_);
    CUDA_CHECK_STATUS(cudaGetLastError());

    gpu_data_.set(gpu_data_.all_finished_, 1);

    cuda_evaluate_iteration<<<blocks, threads>>>(
        gpu_data_.all_finished_, gpu_data_.n_iterations_, gpu_data_.finished_,
        iteration, gpu_data_.states_, n_fits_);
    CUDA_CHECK_STATUS(cudaGetLastError());

    gpu_data_.read(&all_finished_, gpu_data_.all_finished_);

    cuda_prepare_next_iteration<<<blocks, threads>>>(
        gpu_data_.lambdas_, gpu_data_.chi_squares_, gpu_data_.prev_chi_squares_,
        gpu_data_.parameters_, gpu_data_.prev_parameters_, n_fits_,
        info_.n_parameters_);
    CUDA_CHECK_STATUS(cudaGetLastError());
}
/* -------------------------------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------------------------------
*/
LMFitCUDA::LMFitCUDA(REAL const tolerance, Info const &info, GPUData &gpu_data,
                     int const n_fits)
    : info_(info)
    , gpu_data_(gpu_data)
    , n_fits_(n_fits)
...
void LMFitCUDA::run()
{
    ...
    gpu_data_.copy(gpu_data_.prev_chi_squares_, gpu_data_.chi_squares_,
                   n_fits_);
...
{
public:
    FitInterface(REAL const *data, REAL const *weights, std::size_t n_fits,
                 int n_points, REAL tolerance, int max_n_iterations,
                 int num_v_coefs, EstimatorID estimator_id,
                 REAL const *initial_parameters, int *parameters_to_fit,
                 char *user_info, std::size_t user_info_size,
                 REAL *output_parameters, int *output_states,
                 REAL *output_chi_squares, int *output_n_iterations,
...
FitInterface::FitInterface(REAL const *data, REAL const *weights,
                           ...
                           int *parameters_to_fit, char *user_info,
                           std::size_t user_info_size, REAL *output_parameters,
                           int *output_states, REAL *output_chi_squares,
                           int *output_n_iterations, DataLocation data_location)
    : data_(data)
    , weights_(weights)
    , initial_parameters_(initial_parameters)
    , parameters_to_fit_(parameters_to_fit)
...
void FitInterface::check_sizes()
{
    ...
    if (n_fits_ > maximum_size / n_points_ / sizeof(REAL))
    {
        throw std::runtime_error(
            "maximum absolute number of data points exceeded");
    }

    if (n_fits_ > maximum_size / n_parameters_ / sizeof(REAL))
    {
        throw std::runtime_error(
            "maximum number of fits and/or parameters exceeded");
    }
}
...
void FitInterface::fit(ModelID const model_id)
{
    ...
    LMFit lmfit(data_, weights_, info, initial_parameters_, parameters_to_fit_,
                user_info_, output_parameters_, output_states_,
                output_chi_squares_, output_n_iterations_);
...
VISIBLE int gpufit_get_cuda_version(int * runtime_version, int *
driver_version);
...
int gpufit(size_t n_fits, size_t n_points, REAL *data, REAL *weights,
           int model_id, REAL *initial_parameters, REAL tolerance,
           int max_n_iterations,
           int num_v_coefs, // NEW
           int *parameters_to_fit, int estimator_id, size_t user_info_size,
           char *user_info, REAL *output_parameters, int *output_states,
           REAL *output_chi_squares, int *output_n_iterations)
try
{
    FitInterface fi(data, weights, n_fits, static_cast<int>(n_points),
                    tolerance, max_n_iterations,
                    num_v_coefs, // NEW
                    static_cast<EstimatorID>(estimator_id), initial_parameters,
                    parameters_to_fit, user_info, user_info_size,
                    output_parameters, output_states, output_chi_squares,
                    output_n_iterations, HOST);
...
int gpufit_cuda_interface(size_t n_fits, size_t n_points, REAL *gpu_data,
                          REAL *gpu_weights, int model_id, REAL tolerance,
                          int max_n_iterations,
                          int num_v_coefs, // NEW
                          int *parameters_to_fit, int estimator_id,
                          size_t user_info_size, char *gpu_user_info,
                          REAL *gpu_fit_parameters, int *gpu_output_states,
                          REAL *gpu_output_chi_squares,
                          int *gpu_output_n_iterations)
try
{
    FitInterface fi(gpu_data, gpu_weights, n_fits, static_cast<int>(n_points),
                    tolerance, max_n_iterations,
                    num_v_coefs, // NEW
                    static_cast<EstimatorID>(estimator_id), gpu_fit_parameters,
                    parameters_to_fit, gpu_user_info, user_info_size,
                    gpu_fit_parameters, gpu_output_states,
                    gpu_output_chi_squares, gpu_output_n_iterations, DEVICE);
...
int gpufit_portable_interface(int argc, void *argv[])
{
    ...
    */
    return gpufit(*((size_t *)argv[0]), *((size_t *)argv[1]), (REAL *)argv[2],
                  (REAL *)argv[3], *((int *)argv[4]), (REAL *)argv[5],
                  *((REAL *)argv[6]), *((int *)argv[7]), *((int *)argv[8]),
                  (int *)argv[9], *((int *)argv[10]), *((size_t *)argv[11]),
                  (char *)argv[12], (REAL *)argv[13], (int *)argv[14],
                  (REAL *)argv[15], (int *)argv[16]);
}
/* -------------------------------------------------------------------------------------------------------
* from gpufit.cpp END
-------------------------------------------------------------------------------------------------------
