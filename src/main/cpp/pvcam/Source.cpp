...
int setParameter(double exptime, int oW, int oH, int oL, int oT,
                 ...
                 int arraysize, int portIndex, int speedIndex);
void readBufferSingle(int pixelperframe); // read buffer (not so elegant way);
                                          // TODO:any faster way?
void readBufferSingle8bit(int pixelperframe); // read buffer (not so elegant
                                              // way); TODO:any faster way?
void readBufferCpyJNI(jshort *buffer, int pixelperframe, int new_c,
                      JNIEnv *_env, jshortArray _outArray,
                      jobject framecounterObj, jmethodID increment_id,
                      jshort *carray, jboolean isCopy);
void readBufferCpy8bitJNI(uns8 *buffer8bit, int pixelperframe, int new_c,
                          JNIEnv *_env, jshortArray _outArray,
                          jobject framecounterObj, jmethodID increment_id,
                          jshort *carray, jboolean isCopy);
                 ...
                 int arraysize, int portIndex, int speedIndex)
{
    ...
    int iRet = 0;
    // std::cout << "cpp exptime: " << exptime << ", oW: " << oW << ", oH: " <<
    // oH
    // << ", oL: " << oL << ", oT: " << oT << ", incamerabin: " << incamerabin
    // <<
    // ", acqmode: " << acqmode << ", totalfraem: " << totalframe << ", size_b:
    // "
    // << size_b << std::endl;
    ...
    // std::cout << ", arraysize: " << imgobj.arraysize_ << ", nopixelX: " <<
    // imgobj.nopixelX_ << ", nopixelY: " << imgobj.nopixelY_ << ", buffSize: "
    // << imgobj.buffSize_ << std::endl;
                      ...
                      jshort *carray, jboolean isCopy)
{
    ...
    if (isCopy == true || imgobj.enableCriticalAccess == 0)
    {
        ...
        */
        ...
        //_env->SetShortArrayRegion(_outArray, (new_c * pixelperframe),
        // pixelperframe, imgobj.pImageArrayShort_);
        _env->SetShortArrayRegion(_outArray, (new_c * pixelperframe),
                                  pixelperframe, s);
    ...
    {
        std::copy(buffer, buffer + pixelperframe,
                  carray + (new_c * pixelperframe));
        _env->CallVoidMethod(framecounterObj, increment_id);
    }
                          ...
                          jshort *carray, jboolean isCopy)
{
    ...
    if (isCopy == true || imgobj.enableCriticalAccess == 0)
    {
        ...
        */
        ...
        std::copy(buffer8bit, buffer8bit + pixelperframe,
                  imgobj.pImageArrayShort_);
...
{
    return setParameter(exposuretime, width, height, left, top, incamerabin,
                        acqmode, totalframe, size_b, arraysize, portIndex,
                        speedIndex);
}
    ...
    JNIEnv *env, jclass cls)
{
    ...
    if (PV_OK
        != pl_set_param(g_hCam, PARAM_BOF_EOF_ENABLE, (void *)&paramEofEnable))
    {
    ...
    {
        // PrintErrorMessage(pl_error_code(), "pl_set_param(PARAM_BOF_EOF_CLR)
        // error");
        CloseCameraAndUninit();
    ...
    // Start the acqusition - it is used as software trigger in TIMED trigger
    // mode. In hardware trigger mode (Strobe or Bulb) after this call camera
           ...
           && status != READOUT_COMPLETE && status != READOUT_NOT_ACTIVE)
    {
        ...
        if (PV_OK
            != pl_get_param(g_hCam, PARAM_BOF_EOF_COUNT, ATTR_CURRENT,
                            (void *)&eofCount))
        {
            // PrintErrorMessage(pl_error_code(),"pl_get_param(PARAM_BOF_EOF_COUNT)
            // error");
        }
    ...
    // after each frame before new acquisition is started with
    // pl_exp_start_seq()
    if (imgobj.is8bit)
    {
    ...
    JNIEnv *env, jclass cls, jshortArray outArray, jobject framecounterObj)
{
    ...
    if (PV_OK
        != pl_exp_start_cont(g_hCam, imgobj.circBufferInMemory_,
                             imgobj.circBufferBytes_))
    {
    ...
    /*
    if (PV_OK != pl_exp_start_cont(g_hCam, imgobj.pImageCircularBuffer_,
    imgobj.buffSize_))
    {
            //PrintErrorMessage(pl_error_code(), "pl_exp_start_cont error");
            imgobj.freeImArray();
    ...
    */
    ...
    while (framesAcquired < imgobj.totalframe_)
    {
        ...
        {
            imgobj.frameTime_ =
                (double)(g_pFrameInfo->TimeStamp * 100 - timefirstframe)
                / 1000000; // sec
        }
        ...
        if (imgobj.is8bit)
        {
            ...
            if (PV_OK
                != pl_exp_get_latest_frame(g_hCam, (void **)&frameAddress))
            {
                CloseCameraAndUninit();
                /*PrintErrorMessage(pl_error_code(), "pl_exp_get_latest_frame()
                 * error");*/
                errorOccured = TRUE;
            ...
            tk5.setTimeStart();
            // copy from uns16 buffer to short array one frame at at time and
            // make frame available to java
            readBufferCpy8bitJNI(frameAddress, imgobj.arraysize_, new_c, env,
                                 outArray, framecounterObj, increment_id,
                                 carray,
                                 isCopy); // 8bit
        }
        else
        {
            ...
            // copy from uns16 buffer to short array one frame at at time and
            // make frame available to java
            readBufferCpyJNI(frameAddress, imgobj.arraysize_, new_c, env,
                             outArray, framecounterObj, increment_id, carray,
                             isCopy); // 16bit and 12bit
        }
    ...
    JNIEnv *env, jclass cls, jstring inJNIStr)
{
    ...
    // height; ID - CPPtime1; ID - CPPtime2; ID - incamerabin;
